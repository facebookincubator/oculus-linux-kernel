	...
	struct device_attribute *attr, char *buf)
{
	...
	/* Format: <BL pulse duration>,<slice 0 scanline>,<slice 1 scanline> */
	return scnprintf(buf, PAGE_SIZE, "%u,%u,%u\n",
		bl_config->scanline_duration,
		bl_config->scanline_offset[0],
		bl_config->scanline_offset[1]);
}
...
static ssize_t blu_max_overlap_us_show(struct device *device,
	struct device_attribute *attr, char *buf)
{
	struct drm_crtc *crtc;
	struct sde_crtc *sde_crtc;
	...
	return scnprintf(buf, PAGE_SIZE, "%u\n",
			bl_config->blu_max_overlap_ns / 1000);
}

static ssize_t blu_max_overlap_us_store(struct device *device,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct drm_crtc *crtc;
	struct drm_encoder *enc;
	struct sde_crtc_state *cstate;
	u32 blu_max_overlap_us = 0;
	int res, i;
	...
	res = kstrtou32(buf, 10, &blu_max_overlap_us);
	if (res < 0)
		return res;
	...
	drm_for_each_encoder_mask(enc, crtc->dev, crtc->state->encoder_mask) {
		...
		for (i = 0; i < cstate->num_connectors; i++) {
			...
			display->panel->bl_config.blu_max_overlap_ns =
					blu_max_overlap_us * 1000;
		}
	}
...
static ssize_t blu_default_duty_show(struct device *device,
	struct device_attribute *attr, char *buf)
{
	struct drm_crtc *crtc;
	struct sde_crtc *sde_crtc;
	...
	return scnprintf(buf, PAGE_SIZE, "%u\n", bl_config->blu_default_duty);
}

static ssize_t blu_default_duty_store(struct device *device,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct drm_crtc *crtc;
	struct drm_encoder *enc;
	struct sde_crtc_state *cstate;
	u32 blu_default_duty = 0;
	int res, i;
	...
	res = kstrtou32(buf, 10, &blu_default_duty);
	if (res < 0)
		return res;
	...
	drm_for_each_encoder_mask(enc, crtc->dev, crtc->state->encoder_mask) {
		...
		for (i = 0; i < cstate->num_connectors; i++) {
			...
			display->panel->bl_config.blu_default_duty =
					blu_default_duty;
		}
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "FIFO=%u\n", bl_config->fifo_scanlines);
}

static ssize_t fifo_trim_show(struct device *device,
	struct device_attribute *attr, char *buf)
{
	struct drm_crtc *crtc;
	struct sde_crtc *sde_crtc;
...
static ssize_t fifo_trim_store(struct device *device,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct drm_crtc *crtc;
	struct sde_crtc *sde_crtc;
	...
	/* Pass an empty string to disable the FIFO trim override. */
	if (buf[0] != '\0' && buf[0] != '\n') {
		res = kstrtos32(buf, 10, &trim);
		if (res < 0)
			return res;
	}

	sde_crtc = to_sde_crtc(crtc);
	sde_crtc->fifo_trim = trim;

	return count;
}

static ssize_t backlight_temperature_override_show(struct device *device,
	struct device_attribute *attr, char *buf)
{
	struct drm_crtc *crtc;
	struct sde_crtc *sde_crtc;
	...
	return scnprintf(buf, PAGE_SIZE, "%u\n",
			bl_config->bl_temperature_override);
}

static ssize_t backlight_temperature_override_store(struct device *device,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct drm_crtc *crtc;
	struct drm_encoder *enc;
	struct sde_crtc_state *cstate;
	u32 bl_temperature_override = 0;
	int res, i;
	...
	res = kstrtou32(buf, 10, &bl_temperature_override);
	if (res < 0)
		return res;
	...
	drm_for_each_encoder_mask(enc, crtc->dev, crtc->state->encoder_mask) {
		...
		for (i = 0; i < cstate->num_connectors; i++) {
			...
			display->panel->bl_config.bl_temperature_override =
					bl_temperature_override;
		}
	}
...
static bool _sde_crtc_setup_is_quad_pipe(struct drm_crtc_state *state)
{
	...
	for (i = 0; i < cstate->num_connectors; i++) {
		...
		if (TOPOLOGY_QUADPIPE_MODE(topology))
			return true;
	}
		...
		struct drm_crtc_state *state)
{
	...
	sde_crtc_get_resolution(crtc, state, &state->adjusted_mode, &crtc_width, &crtc_height);
	if (crtc_roi->x == 0 && crtc_roi->y == 0 &&
			crtc_roi->w == crtc_width &&
			crtc_roi->h == crtc_height)
...
static void sde_crtc_lineptr_cb(void *data, u64 sample_time, int vtotal,
		int lineptr_offset, int lineptr_headroom)
{
	struct drm_crtc *crtc = (struct drm_crtc *)data;
	struct sde_crtc *sde_crtc = to_sde_crtc(crtc);
...
static void _sde_crtc_setup_quad_lm_layout(struct sde_crtc *sde_crtc,
	struct sde_crtc_mixer *mixer, int mixer_num, bool swap_mixers)
{
	...
	if (mixer_num < CRTC_DUAL_MIXERS)
		lm_cfg->lm_layout = swap_mixers ?
			SDE_LAYOUT_RIGHT : SDE_LAYOUT_LEFT;
	else
		lm_cfg->lm_layout = swap_mixers ?
			SDE_LAYOUT_LEFT : SDE_LAYOUT_RIGHT;
		...
		struct drm_encoder *enc)
{
	...
	for (i = sde_crtc->num_mixers; i < ARRAY_SIZE(sde_crtc->mixers); i++) {
		...
		if (_sde_crtc_setup_is_quad_pipe(sde_crtc->base.state))
			_sde_crtc_setup_quad_lm_layout(sde_crtc,
					mixer,
					sde_crtc->num_mixers,
					sde_kms->catalog->swap_mixers);
	...
	if (sde_kms->catalog->swap_mixers && sde_crtc->num_mixers == 2)
		swap(sde_crtc->mixers[0], sde_crtc->mixers[1]);
}
		...
		uint64_t *val)
{
	...
	if (i != -EINVAL) {
		...
		case CRTC_TIMING_PROP_BLU_SCANLINE_DURATION:
			*val = bl_config->scanline_duration;
			ret = 0;
			break;
		case CRTC_TIMING_PROP_BLU_SCANLINE_OFFSET_0:
			*val = bl_config->scanline_offset[0];
			ret = 0;
			break;
		case CRTC_TIMING_PROP_BLU_SCANLINE_OFFSET_1:
			*val = bl_config->scanline_offset[1];
			ret = 0;
			break;
...
struct drm_crtc *sde_crtc_init(struct drm_device *dev, struct drm_plane *plane)
{
	...
	/* Default to FIFO trim being configured via the device tree. */
	sde_crtc->fifo_trim = S32_MIN;
