...
static void sde_encoder_phys_cmd_autorefresh_done_irq(void *arg, int irq_idx)
{
	...
	SDE_EVT32_IRQ(DRMID(phys_enc->parent),
			...
			new_cnt, phys_enc->parent->dev->primary->index);
...
static void sde_encoder_phys_cmd_te_rd_ptr_irq(void *arg, int irq_idx)
{
	...
	SDE_EVT32_IRQ(DRMID(phys_enc->parent),
		...
		scheduler_status, phys_enc->parent->dev->primary->index);
		...
		struct sde_encoder_phys *phys_enc)
{
	...
	if (cmd_enc->pp_timeout_report_cnt == 1) {
		...
		if (sde_kms_is_secure_session_inprogress(phys_enc->sde_kms))
			SDE_DBG_DUMP(phys_enc->sde_kms->dev, SDE_DBG_BUILT_IN_ALL, "secure");
		else
			SDE_DBG_DUMP(phys_enc->sde_kms->dev, SDE_DBG_BUILT_IN_ALL);
	...
	if (recovery_events)
		...
		SDE_DBG_DUMP(phys_enc->sde_kms->dev, 0x0, "panic");
		...
		struct sde_encoder_phys *phys_enc)
{
	...
	if (ret) {
		...
		SDE_DBG_DUMP(phys_enc->sde_kms->dev, SDE_DBG_BUILT_IN_ALL, "panic");
	}
