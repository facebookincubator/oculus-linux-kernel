...
static int _reg_dma_init_dspp_feature_buf(int feature, struct sde_hw_dspp *ctx)
{
	...
	if (feature == SDE_DSPP_MEMCOLOR) {
		...
		rc = reg_dma_buf_init(
			&dspp_buf[SPR_PU_CFG][ctx->idx][ctx->dpu_idx],
			feature_reg_dma_sz[feature],
			ctx->dpu_idx);
	} else {
		rc = reg_dma_buf_init(
...
int reg_dmav1_setup_rc_datav1(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	REG_DMA_SETUP_KICKOFF(kick_off, hw_cfg->ctl,
		dspp_buf[RC_DATA][ctx->idx][ctx->dpu_idx], REG_DMA_WRITE,
	...
	struct sde_hw_scaler3_cfg *scaler3_cfg, u32 offset, u32 op_mode)
{
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg,
		offset + 0xA0, cache, sizeof(cache),
		REG_BLK_WRITE_SINGLE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg,
		offset + 0xB0, cache, sizeof(cache),
		REG_BLK_WRITE_SINGLE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg,
		offset + 0xC0, cache, sizeof(cache),
		REG_BLK_WRITE_SINGLE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
	...
	REG_DMA_SETUP_OPS(*dma_write_cfg,
	offset + 0xD0, cache, sizeof(cache),
		REG_BLK_WRITE_SINGLE, 0, 0, 0);
	rc = dma_ops->setup_payload(dma_write_cfg);
	if (rc) {
	...
	void *scaler_cfg)
{
	...
	if (scaler3_cfg->de.enable) {
		rc = reg_dmav1_setup_scaler3_de(&dma_write_cfg,
			&scaler3_cfg->de, offset, ctx->dpu_idx);
		if (!rc)
			op_mode |= BIT(8);
	}
	...
	rc = reg_dmav1_setup_cac(ctx, &dma_write_cfg, scaler3_cfg, offset,
					op_mode);
	if (rc) {
