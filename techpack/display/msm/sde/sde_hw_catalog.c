...
static struct sde_prop_type sde_prop[] = {
	...
	{SWAP_MIXERS, "qcom,swap-mixers", false, PROP_TYPE_BOOL},
};
						...
						struct sde_mdss_cfg *sde_cfg)
{
	...
	for (i = 0, mixer_count = 0, pp_idx = 0, dspp_idx = 0,
		...
		lm_pair_mask = PROP_VALUE_ACCESS(prop_value,
				MIXER_PAIR_MASK, i);
		if (lm_pair_mask)
			mixer->lm_pair_mask = lm_pair_mask;
			...
			struct sde_mdss_cfg *sde_cfg)
{
	...
	for (i = 0; i < off_count; i++) {
		...
		dsc_pair_mask = PROP_VALUE_ACCESS(prop_value,
				DSC_PAIR_MASK, i);
		if (dsc_pair_mask)
			dsc->dsc_pair_mask = dsc_pair_mask;
	}
	...
	bool prop_exists[SDE_PROP_MAX], struct sde_prop_value *prop_value)
{
	...
	cfg->swap_mixers =
		PROP_VALUE_ACCESS(prop_value, SWAP_MIXERS, 0);
	return 0;
}
