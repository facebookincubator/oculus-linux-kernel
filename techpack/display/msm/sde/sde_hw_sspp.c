		...
		enum sde_sspp_color_filter color_filter)
{
	...
	/*
	 * optionally modify the unpack to only use a single color channel;
	 * this relies on C3_ALPHA in the unpack pattern, so we'll only do
	 * it if we have enough channels (some 16-bit formats like RGB_565
	 * are not supported).
	 */
	if (fmt->unpack_count == 4) {
		switch (color_filter) {
		case SDE_SSPP_COLOR_FILTER_R:
			unpack = _sspp_unpack_filter(unpack, C2_R_Cr);
			break;
		case SDE_SSPP_COLOR_FILTER_G:
			unpack = _sspp_unpack_filter(unpack, C0_G_Y);
			break;
		case SDE_SSPP_COLOR_FILTER_B:
			unpack = _sspp_unpack_filter(unpack, C1_B_Cb);
			break;
		default:
			break;
		}
