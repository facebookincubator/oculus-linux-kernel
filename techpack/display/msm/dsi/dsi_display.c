		...
		void *display, u32 bl_lvl)
{
	...
	/* Applying backlight scaling */
	if (panel->bl_config.bl_scale_brightness)
		bl_temp = (bl_temp * panel->bl_config.bl_scale_brightness) / MAX_BL_SCALE_LEVEL_BRIGHTNESS;
	...
	if (panel->bl_config.bl_scale_brightness)
		panel->bl_config.bl_level = bl_temp;
		...
		struct dsi_cmd_desc *cmd)
{
	...
	display_for_each_ctrl(i, display) {
		...
		// Set the custom Qualcomm unicast flag if the vanilla flag is set
		if (msg->flags & MIPI_DSI_MSG_UNICAST)
			msg->flags |= MIPI_DSI_MSG_UNICAST_COMMAND;
...
int dsi_host_transfer_sub(struct mipi_dsi_host *host, struct dsi_cmd_desc *cmd)
{
	...
	if (cmd->ctrl_flags & DSI_CTRL_CMD_BROADCAST) {
		...
		if (idx < 0 || idx >= display->ctrl_count) {
			DSI_ERR("[%s] invalid dsi ctrl index (%d), ctrl_count=%d\n", display->name, idx, display->ctrl_count);
			rc = EINVAL;
			goto error;
		}
...
int dsi_display_get_num_of_displays(struct drm_device *dev)
{
	...
	for (i = 0; i < MAX_DSI_ACTIVE_DISPLAY; i++) {
		...
		if (IS_ERR_OR_NULL(display) || display->drm_dev != dev)
			continue;

		if (!IS_ERR_OR_NULL(display->panel_node) ||
				!IS_ERR_OR_NULL(display->fw))
			count++;
	}
		...
		void **display_array, u32 max_display_count)
{
	...
	for (index = 0; index < MAX_DSI_ACTIVE_DISPLAY; index++) {
		...
		if (!IS_ERR_OR_NULL(display->panel_node) ||
			...
			/* Break out if we've run out of space in the array. */
...
int dsi_display_prepare(struct dsi_display *display)
{
	...
	} else if (display->panel->cont_splash_handoff) {
		/*
		 * If the continuous splash handoff property is enabled,
		 * resend the "pre-on" command set now.
		 */
		rc = dsi_panel_pre_enable(display->panel);
		if (rc) {
			DSI_ERR("[%s] panel prepare failed, rc=%d\n",
					display->name, rc);
			goto error_ctrl_link_off;
