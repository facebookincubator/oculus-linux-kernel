...
static struct dsi_pll_clk *dsi_pllcc_5nm[] = {
				  ...
				  struct dsi_pll_resource *pll_res)
{
	...
	byteclk_idx = 2 * ndx;
	dsiclk_idx = 2 * ndx + 1;
	dsi_pllcc_5nm[byteclk_idx]->priv = pll_res;
	dsi_pllcc_5nm[dsiclk_idx]->priv = pll_res;

	/* byte clk registration */
	clk = devm_clk_register(&pdev->dev,
			&dsi_pllcc_5nm[byteclk_idx]->hw);
	if (IS_ERR(clk)) {
		DSI_PLL_ERR(pll_res,
	...
	if (ndx == 0) {
		dsi_pllcc_5nm[MDSS_0_DSIM_PLL_BYTECLK]->priv = pll_res;
		dsi_pllcc_5nm[MDSS_0_DSIM_PLL_DSICLK]->priv = pll_res;

		clk = devm_clk_register(&pdev->dev, &dsi_pllcc_5nm[MDSS_0_DSIM_PLL_BYTECLK]->hw);
		if (IS_ERR(clk)) {
		...
		clk = devm_clk_register(&pdev->dev, &dsi_pllcc_5nm[MDSS_0_DSIM_PLL_DSICLK]->hw);
		if (IS_ERR(clk)) {
					...
					struct dsi_pll_resource *rsc)
{
	...
	for (i = 0; i < MAX_DSI_PLL_SLAVE_NUM; i++) {
		if (!rsc->slave[i])
			continue;
		data = DSI_PLL_REG_R(rsc->slave[i]->phy_base, PHY_CMN_CLK_CFG1) |
			BIT(5);
...
static int dsi_pll_5nm_enable(struct dsi_pll_resource *rsc)
{
	...
	for (i = 0; i < MAX_DSI_PLL_SLAVE_NUM; i++) {
		if (!rsc->slave[i])
			continue;
		dsi_pll_phy_dig_reset(rsc->slave[i]);
	}
...
int dsi_pll_5nm_program_slave(struct dsi_pll_resource *pll, bool skip_op)
{
	...
	dsi_clk = dsi_pll_get_dsi_clk(m_pll);
