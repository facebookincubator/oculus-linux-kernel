...
static int dsi_panel_tx_cmd_set_common(struct dsi_panel *panel,
				enum dsi_cmd_set_type type, bool unicast, u32 ctrl)
{
	int rc = 0, i = 0;
	ssize_t len;
...
{
	return dsi_panel_tx_cmd_set_common(panel, type, false, 0);
}

static int dsi_panel_tx_cmd_set_unicast(struct dsi_panel *panel,
				enum dsi_cmd_set_type type, u32 ctrl)
{
	return dsi_panel_tx_cmd_set_common(panel, type, true, ctrl);
}
			...
			u32 bl_lvl)
{
	...
	u8 blu_scanline1_lsb[2] = {0xB9, 0x00}; /* BLU pulse 1 scanline offset LSB */
	u8 blu_duration1_msb[2] = {0xBA, 0x00}; /* BLU pulse 1 scanline duration MSB */
	u8 blu_duration1_lsb[2] = {0xBB, 0x00}; /* BLU pulse 1 scanline duration LSB */
	u8 blu_scanline2_msb[2] = {0xBC, 0x00}; /* BLU pulse 2 scanline offset MSB */
	...
	frame_period_ns = 1000000000 / timing->refresh_rate;
	external_1h_ns = frame_period_ns / vtotal;
	internal_1h_ns = (timing->internal_1h_time_ns > 0) ?
	...
	guardband_margin_prior = guardband_internal_scanlines * internal_1h_ns;
	guardband_margin_after = 2 * internal_1h_ns; /* Just add a couple lines of slack. */
	blu_scanline_duration = max_t(u32, blu_scanline_duration, guardband_internal_scanlines + 2);

	/* Find the latest time the backlight can fire. */
	blu_duration_ns = blu_scanline_duration * internal_1h_ns;
	blu_start_time_ns = frame_period_ns + bl_config->blu_max_overlap_ns - blu_duration_ns;

	/*
	 * Trim backlight start time down to hit the settling time target (if it
	 * is set and valid).
	 */
	if (bl_config->settling_time_target_us < 0xFFFF) {
		 ...
		 * there might not be enough time to hit the settling target.
		 */
		blu_start_time_ns = min_t(u32, blu_start_time_ns, blu_start_target_ns);
	}

	/* Force a little bit of overlap if the BLU end time is close to the frame period. */
	blu_end_time_ns = blu_start_time_ns + blu_duration_ns;

	if (blu_end_time_ns >= frame_period_ns - guardband_margin_prior &&
			blu_end_time_ns < frame_period_ns + guardband_margin_after) {
		/* Delay the BLU pulse if the end time is within the guardband margin. */
		blu_end_time_ns = frame_period_ns + guardband_margin_after;
		blu_start_time_ns = blu_end_time_ns - blu_duration_ns;
	} else if (blu_start_time_ns >= frame_period_ns - guardband_margin_prior &&
			blu_start_time_ns < frame_period_ns + guardband_margin_after) {
	...
	blu_scanline_offset = blu_start_time_ns / internal_1h_ns;
	...
	bl_config->fifo_trim = fifo_trim;
	bl_config->fifo_scanlines = fifo_time_external_scanlines;
...
int dsi_panel_jdi_nvt_update_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	*/
	...
	/* Queue the DCS writes so they can be batched together in one frame. */
	rc = mipi_dsi_dcs_write_queue(dsi, blu_set_pwm_page, sizeof(blu_set_pwm_page), MIPI_DSI_MSG_BATCH_COMMAND, 0);
	rc = mipi_dsi_dcs_write_queue(dsi, blu_right_start_msb, sizeof(blu_right_start_msb), MIPI_DSI_MSG_BATCH_COMMAND, 0);
	rc = mipi_dsi_dcs_write_queue(dsi, blu_right_start_lsb, sizeof(blu_right_start_lsb), MIPI_DSI_MSG_BATCH_COMMAND, 0);
	rc = mipi_dsi_dcs_write_queue(dsi, blu_right_level_msb, sizeof(blu_right_level_msb), MIPI_DSI_MSG_BATCH_COMMAND, 0);
	rc = mipi_dsi_dcs_write_queue(dsi, blu_right_level_lsb, sizeof(blu_right_level_lsb), MIPI_DSI_MSG_BATCH_COMMAND, 0);
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (panel->host_config.ext_bridge_mode || bl->backlight_lock)
		return 0;
				  ...
				  struct dsi_parser_utils *utils)
{
	...
	rc = utils->read_u32(utils->data, "meta,internal-1h-time-ns",
			&display_mode->priv_info->internal_1h_time_ns);
	if (rc) {
		display_mode->priv_info->internal_1h_time_ns = 0;
		rc = 0;
	}
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"meta,mdss-dsi-post-primary-panel-on-command",
	"meta,mdss-dsi-post-secondary-panel-on-command",
};
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"meta,mdss-dsi-post-primary-panel-on-command-state",
	"meta,mdss-dsi-post-secondary-panel-on-command-state",
};
...
static int dsi_panel_parse_misc_features(struct dsi_panel *panel)
{
	...
	/* Check if we need to handle continuous splash handoff. */
	panel->cont_splash_handoff = utils->read_bool(utils->data,
			"meta,cont-splash-handoff");
...
static bool dsi_panel_parse_temp_dependent_bl_config(struct dsi_panel *panel)
{
	...
	rc = utils->read_u32(utils->data, "meta,settling-time-target-us", &val);
	config->settling_time_target_us = (!rc) ? val : 0xFFFF;

	if (!utils->read_bool(utils->data, "meta,temperature-dependent-backlight"))
		return false;

	len = utils->count_u32_elems(utils->data, "meta,response-time-curve");
	if (len <= 0 || len & 1) {
		DSI_ERR("missing/malformed response time curve, len = %d\n", len);
		return false;
	}

	config->response_time = devm_kcalloc(panel->parent, len, sizeof(u32),
			GFP_KERNEL);
	if (!config->response_time)
		return false;

	rc = utils->read_u32_array(utils->data, "meta,response-time-curve",
	...
	rc = utils->read_string(utils->data, "meta,thermal-zone", &thermal_zone);
	if (rc) {
		DSI_ERR("unable to find the thermal zone, rc = %d\n", rc);
		return false;
	}

	config->bl_temp_tz = thermal_zone_get_zone_by_name(thermal_zone);
	if (IS_ERR_OR_NULL(config->bl_temp_tz)) {
		DSI_ERR("panel temperature thermal_zone_get_zone_by_name(\"%s\") error: %ld\n",
				thermal_zone, PTR_ERR(config->bl_temp_tz));
		return false;
	}
...
static int dsi_panel_parse_bl_config(struct dsi_panel *panel)
{
	...
	} else if (panel->bl_config.type == DSI_BACKLIGHT_TOKKI_A) {
		rc = utils->read_u32(utils->data,
			"meta,blu-default-duty-cycle", &val);
		if (rc) {
			DSI_DEBUG("[%s] meta,blu-default-duty-cycle unspecified, rc=%d\n",
				panel->name, rc);
			goto error;
		} else {
...
static int dsi_panel_parse_dms_info(struct dsi_panel *panel)
{
	...
	if (data && !strcmp(data, "dms-vid-seamless"))
		panel->dms_caps.type = DSI_DMS_VID_SEAMLESS;
	else if (data && !strcmp(data, "dms-vid-non-seamless"))
		panel->dms_caps.type = DSI_DMS_VID_NON_SEAMLESS;
	else
		panel->dms_caps.type = DSI_DMS_VID_DISABLED;

	panel->dms_caps.maintain_const_clk = utils->read_bool(utils->data,
				...
				bool trusted_vm_env)
{
	...
	if (is_panel_xr) {
		...
		strlcpy(new_panel_name, panel->name, size);
	...
	atomic_set(&panel->fifo_trim, S32_MIN);
...
int dsi_panel_post_enable(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set_unicast(panel, DSI_CMD_SET_POST_ON_PRIMARY, 0);
	if (rc)
		DSI_ERR("[%s] failed to send DSI_CMD_SET_POST_ON_PRIMARY cmds, rc=%d\n",
		       panel->name, rc);

	rc = dsi_panel_tx_cmd_set_unicast(panel, DSI_CMD_SET_POST_ON_SECONDARY, 1);
	if (rc)
		DSI_ERR("[%s] failed to send DSI_CMD_SET_POST_ON_PRIMARY cmds, rc=%d\n",
		       panel->name, rc);
