...
/*  add for quat  slave i2s -- begin */
static struct afe_clk_set quat_i2s_tx_clk = {
...
static struct afe_clk_set quat_i2s_rx_clk = {
	AFE_API_VERSION_I2S_CONFIG,
	Q6AFE_LPASS_CLK_ID_QUAD_MI2S_EBIT,
	Q6AFE_LPASS_IBIT_CLK_DISABLE,
	Q6AFE_LPASS_CLK_ATTRIBUTE_COUPLE_NO,
...
/*  add for quat  slave i2s -- begin */
static int quat_i2s_sclk_ctl(struct snd_pcm_substream *substream, bool enable)
{
	int ret = 0;
	...
	if (enable) {
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			quat_i2s_rx_clk.enable = enable;
			ret = afe_set_lpass_clock_v2(port_id,
					&quat_i2s_rx_clk);
		} else if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
			quat_i2s_tx_clk.enable = enable;
			ret = afe_set_lpass_clock_v2(port_id,
					&quat_i2s_tx_clk);
		} else {
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			quat_i2s_rx_clk.enable = enable;
			quat_i2s_rx_clk.clk_freq_in_hz =
					Q6AFE_LPASS_IBIT_CLK_DISABLE;
			ret = afe_set_lpass_clock_v2(port_id,
					&quat_i2s_rx_clk);
		} else if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
			quat_i2s_tx_clk.enable = enable;
			quat_i2s_tx_clk.clk_freq_in_hz =
					Q6AFE_LPASS_IBIT_CLK_DISABLE;
			ret = afe_set_lpass_clock_v2(port_id,
					&quat_i2s_tx_clk);
		} else {
...
static int msm_quat_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (pdata->vaddr_gpio_mux_mic_ctl) {
		val = ioread32(pdata->vaddr_gpio_mux_mic_ctl);
		val = (val & 0xfffcffff) | 0x00020000;
		iowrite32(val, pdata->vaddr_gpio_mux_mic_ctl);
	    iowrite32(0x1, pdata->vaddr_gpio_mux_mic_ext_clk_ctl);
	}
	ret = quat_i2s_sclk_ctl(substream, true);
	if (ret < 0) {
		pr_err("failed to enable sclk\n");
		return ret;
	}
	...
	return ret;
err:
	ret = quat_i2s_sclk_ctl(substream, false);
	if (ret < 0)
		pr_err("failed to disable sclk\n");
	return ret;
}
...
static int msm8952_asoc_machine_probe(struct platform_device *pdev)
{
	...
	muxsel = platform_get_resource_byname(pdev,
        IORESOURCE_MEM,"csr_gp_io_mux_mic_ext_clk_ctl");
	if (!muxsel) {
