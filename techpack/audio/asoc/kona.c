			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (channel < 0  || channel >= TDM_MAX_SLOTS) {
...
static int kona_tdm_be_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
...
static struct snd_soc_dai_link msm_common_misc_fe_dai_links[] = {
	...
	{/* hw:x,39 */
		.name = "PRI_MI2S_RX Hostless",
		.stream_name = "PRI_MI2S_RX Hostless",
...
static struct snd_soc_dai_link msm_tdm_be_dai_links[] = {
	{
		...
		.be_hw_params_fixup = kona_tdm_be_hw_params_fixup,
		.ops = &kona_tdm_be_ops,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
	},
...
#if (IS_ENABLED(CONFIG_SND_SOC_CM710X) || IS_ENABLED(CONFIG_SND_SOC_AK4331))
static struct snd_soc_dai_link_component cm710x_ak4331_codecs[] = {
#if IS_ENABLED(CONFIG_SND_SOC_CM710X)
	{
		.name = "cm710x.1-002c",
		.dai_name = "cm710x-aif1",
	},
#endif
...
static struct snd_soc_dai_link cm710x_pri_quin_mi2s_be_dai_links[] = {
	{
		...
		.platform_name = "msm-pcm-routing",
#if (IS_ENABLED(CONFIG_SND_SOC_CM710X) || IS_ENABLED(CONFIG_SND_SOC_AK4331))
		.codecs = cm710x_ak4331_codecs,
		.num_codecs = ARRAY_SIZE(cm710x_ak4331_codecs),
		.init = cm710x_init,
	...
	},
	{
		...
		.cpu_dai_name = "msm-dai-q6-mi2s.0",
		.platform_name = "msm-pcm-routing",
#if (IS_ENABLED(CONFIG_SND_SOC_CM710X) || defined(CONFIG_SND_SOC_AK4331))
		.codecs = cm710x_ak4331_codecs,
		.num_codecs = ARRAY_SIZE(cm710x_ak4331_codecs),
#else
		.codec_name = "msm-stub-codec.1",
...
/* mi2s_codecs_names must match order of mi2s_codec_dai_links */
...
static int add_mi2s_intf_links(struct device *dev, int total_links)
{
	int i = 0, codec_idx = 0;
	const char *codec_name = NULL;
	int links_added = 0;

	for (i = 0; i < ARRAY_SIZE(mi2s_codec_names); i++) {
		codec_name = mi2s_codec_names[i];
		codec_idx = of_property_match_string(dev->of_node,
						     "asoc-codec-names",
						     codec_name);
		if (codec_idx < 0)
			continue;
	...
	links_added += ARRAY_SIZE(msm_mi2s_be_dai_links);
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	if (!strcmp(match->data, "codec")) {
		...
		bolero_codec = of_property_match_string(dev->of_node,
							"asoc-codec-names",
							"bolero_codec");

		bolero_tx_only = of_property_read_bool(dev->of_node,
							"qcom,bolero-tx-only");
		...
		if (bolero_codec >= 0 && !bolero_tx_only) {
			memcpy(msm_kona_dai_links + total_links,
		...
		if (rc) {
			...
			if (mi2s_audio_intf) {
				total_links += add_mi2s_intf_links(dev,
						total_links);
			}
		}
...
aux_dev_register:
	...
	if (card->num_aux_devs == 0) {
		dev_info(&pdev->dev, "%s: No AUX codecs available\n", __func__);
		return 0;
	}
