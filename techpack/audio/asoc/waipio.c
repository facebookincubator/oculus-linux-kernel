...
int msm_tdm_codec_hw_params(struct snd_pcm_substream *substream,
				struct snd_pcm_hw_params *params)
{
	...
	struct tdm_cfg tdm_config;
	const char *stream_name = rtd->dai_link->stream_name;
	int i = 0, ret = 0;
	int slots, sample_width;
	unsigned int rate;
	unsigned int cnt;
	...
	if (index >= 0) {
		...
		if (atomic_read(&pdata->lpass_intf_clk_ref_cnt[index]) == 0) {
			cnt = device_property_read_u32_array(rtd->card->dev, "tdm-config", NULL, 0);
			pr_debug("%s: card tdm-config cnt %d\n", __func__, cnt);
			slots = 8; /* default */
			sample_width = 32; /* default */
			rate = params_rate(params);
			if (cnt == (sizeof(struct tdm_cfg) / sizeof(unsigned int))) {
				if (!device_property_read_u32_array(rtd->card->dev, "tdm-config", (void *)&tdm_config, cnt)) {
					pr_debug("%s: card tdm-config %d %d %d %d %d\n", __func__,
					tdm_config.rx_mask, tdm_config.tx_mask,
					tdm_config.slots, tdm_config.slot_width, tdm_config.fmt);
					slots = tdm_config.slots;
					sample_width = tdm_config.slot_width;
				}
			}
			...
			intf_clk_cfg.clk_freq_in_hz = rate * sample_width * slots;
	...
	for_each_rtd_codec_dais(rtd, i, codec_dai) {
		cnt = device_property_read_u32_array(codec_dai->component->dev, "tdm-config", NULL, 0);
		pr_debug("%s: %s tdm-config cnt %d\n", __func__,
				codec_dai->component->name, cnt);
		if (cnt == (sizeof(struct tdm_cfg) / sizeof(unsigned int))) {
			if (!device_property_read_u32_array(codec_dai->component->dev, "tdm-config", (void *)&tdm_config, cnt)) {
				pr_debug("%s: %s tdm-config %d %d %d %d %d\n", __func__,
				codec_dai->component->name, tdm_config.rx_mask, tdm_config.tx_mask,
				tdm_config.slots, tdm_config.slot_width, tdm_config.fmt);
				ret = snd_soc_dai_set_fmt(codec_dai, tdm_config.fmt);
				if (ret < 0)
					break;
				ret = snd_soc_dai_set_tdm_slot(codec_dai,
					tdm_config.tx_mask, tdm_config.rx_mask, tdm_config.slots, tdm_config.slot_width);
				if (ret < 0)
					break;
			}
		}
...
static struct snd_soc_ops msm_tdm_codec_be_ops = {
	.hw_params = msm_tdm_codec_hw_params,
	.startup = msm_common_snd_startup,
	.shutdown = msm_common_snd_shutdown,
};
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev, int wsa_max_devs)
{
	...
	if (!strcmp(match->data, "codec")) {
		...
		lpass_tx_only = of_property_read_bool(dev->of_node,
				"qcom,lpass-tx-only");
...
static int waipio_ssr_enable(struct device *dev, void *data)
{
	...
	if (pdata->wsa_max_devs > 0) {
		rtd_wsa = snd_soc_get_pcm_runtime(card,
			&card->dai_link[ARRAY_SIZE(msm_rx_cdc_dma_be_dai_links) +
				ARRAY_SIZE(msm_tx_cdc_dma_be_dai_links)]);
		if (!rtd_wsa) {
			dev_dbg(dev,
			"%s: snd_soc_get_pcm_runtime for %s failed!\n",
			__func__, card->dai_link[ARRAY_SIZE(msm_rx_cdc_dma_be_dai_links) +
				ARRAY_SIZE(msm_tx_cdc_dma_be_dai_links)].name);
		}
	}
