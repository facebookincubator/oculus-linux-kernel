...
#define VIDC_VDEV_DEC_NAME "msm-vidc-decoder"
#define VIDC_VDEV_ENC_NAME "msm-vidc-encoder"
		...
		enum msm_vidc_domain_type type, int nr)
{
	...
	if (type == MSM_VIDC_DECODER) {
		...
		strscpy(core->vdev[index].vdev.name, VIDC_VDEV_DEC_NAME,
			sizeof(core->vdev[index].vdev.name));
	} else {
		core->vdev[index].vdev.ioctl_ops = core->v4l2_ioctl_ops_enc;
		strscpy(core->vdev[index].vdev.name, VIDC_VDEV_ENC_NAME,
			sizeof(core->vdev[index].vdev.name));
	}
