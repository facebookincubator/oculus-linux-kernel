...
static bool msm_vidc_allow_image_encode_session(struct msm_vidc_inst *inst)
{
	...
	/* quick patch - adding compressed formats as one of the supported formats to unblock us for making a release*/
	allow = (is_linear_yuv_colorformat(pix_fmt) || is_ubwc_colorformat(pix_fmt));
	if (!allow) {
		i_vpr_e(inst, "%s: unsupported fmt: %#x\n", __func__, pix_fmt);
		goto exit;
	}
