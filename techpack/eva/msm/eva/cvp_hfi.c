		...
		u32 *pb_tx_req_is_set)
{
	...
	if (((packet_size_in_words << 2) <= CVP_IFACEQ_VAR_HUGE_PKT_SIZE)
		...
		/*
		 * Copy back the validated size to avoid security issues. As we read
...
static int iris_pm_qos_update(void *device, u32 latency)
{
    ...
    if (in_atomic() || irqs_disabled()) {
        dprintk(CVP_WARN,
                "Skipping PM QoS update in atomic context\n");
        return 0;
    }
...
static int __unregister_for_MMCX(struct iris_hfi_device *device)
{
	...
	if (device->notifier_registered) {
		...
		if (rc) {
			...
			device->notifier_registered = false;
		}
	}
