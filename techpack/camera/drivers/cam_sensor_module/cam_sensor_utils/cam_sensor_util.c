...
bool camera_is_mclk_no_power_down(struct cam_sensor_power_ctrl_t *power_info)
{
	struct cam_sensor_board_info *board_info = container_of(power_info,
					struct cam_sensor_board_info,
					power_info);
	return board_info->oculus_mclk_no_power_down;
}
...
power_up_failed:
	...
	for (index--; index >= 0; index--) {
		...
		switch (power_setting->seq_type) {
		case SENSOR_MCLK:
			if (camera_is_mclk_no_power_down(ctrl))
				break; // Leave MCLK on.
