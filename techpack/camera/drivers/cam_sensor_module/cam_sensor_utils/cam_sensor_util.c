...
#define CAM_SENSOR_PINCTRL_STATE_SLEEP_AO "cam_suspend_ao"
		...
		struct cam_hw_soc_info *soc_info, int gpio_en)
{
	...
	if (gpio_en) {
		for (i = 0; i < size; i++) {
			unsigned long flags = gpio_tbl[i].flags;

			rc = cam_res_mgr_gpio_request(soc_info->dev,
					gpio_tbl[i].gpio,
					flags, gpio_tbl[i].label);
			if (rc) {
				/*
	...
	struct msm_camera_gpio_num_info **pgpio_num_info)
{
	...
	*pgpio_num_info = kzalloc(sizeof(struct msm_camera_gpio_num_info),
	...
	if (rc != -EINVAL) {
		...
		gpio_num_info->always_on[SENSOR_RESET] =
					gconf->cam_gpio_aoflags[val];

		CAM_DBG(CAM_SENSOR, "gpio-reset %d, always_on %d",
			gpio_num_info->gpio_num[SENSOR_RESET],
			gpio_num_info->always_on[SENSOR_RESET]);
	}
	...
	if (rc != -EINVAL) {
		...
		gpio_num_info->always_on[SENSOR_STANDBY] =
					gconf->cam_gpio_aoflags[val];

		CAM_DBG(CAM_SENSOR, "gpio-standby %d, always_on %d",
			gpio_num_info->gpio_num[SENSOR_STANDBY],
			gpio_num_info->always_on[SENSOR_STANDBY]);
	}
	...
	if (rc != -EINVAL) {
		...
		gpio_num_info->always_on[SENSOR_CUSTOM_GPIO1] =
					gconf->cam_gpio_aoflags[val];

		CAM_DBG(CAM_SENSOR, "gpio-custom1 %d, always_on %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO1],
			gpio_num_info->always_on[SENSOR_CUSTOM_GPIO1]);
	}
	...
	if (rc != -EINVAL) {
		...
		gpio_num_info->always_on[SENSOR_CUSTOM_GPIO2] =
					gconf->cam_gpio_aoflags[val];

		CAM_DBG(CAM_SENSOR, "gpio-custom2 %d, always_on %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO2],
			gpio_num_info->always_on[SENSOR_CUSTOM_GPIO2]);
	} else {
		rc = 0;
	...
	struct msm_pinctrl_info *sensor_pctrl, struct device *dev)
{
	...
	/* Optional pin control in always on mode */
	sensor_pctrl->gpio_state_suspend_ao
		= pinctrl_lookup_state(sensor_pctrl->pinctrl,
				CAM_SENSOR_PINCTRL_STATE_SLEEP_AO);
	if (IS_ERR_OR_NULL(sensor_pctrl->gpio_state_suspend_ao))
		sensor_pctrl->gpio_state_suspend_ao = NULL;
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	if (ctrl->cam_pinctrl_status) {
		if (ctrl->always_on &&
			ctrl->pinctrl_info.gpio_state_suspend_ao) {
			ret = pinctrl_select_state(
				ctrl->pinctrl_info.pinctrl,
				ctrl->pinctrl_info.gpio_state_suspend_ao);
		} else {
