...
#if IS_ENABLED(CONFIG_META_CAMERA_SENSOR_TEMPERATURE)
static uint32_t g_cam_temperature_reg_enable = 0;
static uint32_t g_cam_temperature_reg_data = 0;
static uint32_t g_cam_temperature_measurements_active = 0;

static uint32_t g_cam_temperature_dt_sensor_id = 0;
static uint32_t g_cam_temperature_dt_en_reg = 0;
static uint32_t g_cam_temperature_dt_stat_reg = 0;
static uint32_t g_cam_temperature_dt_val_reg = 0;

void cam_sensor_temperature_measurements_init(struct cam_sensor_ctrl_t *s_ctrl)
{
	g_cam_temperature_reg_data = 0;
	g_cam_temperature_measurements_active = 0;
	if (of_property_read_u32(s_ctrl->of_node, "sensor-id",
		&g_cam_temperature_dt_sensor_id) < 0) {
		CAM_ERR(CAM_SENSOR, "Invalid camera sensor id for temperature readings in device tree");
		// whole feature is disabled if some DT value is invalid
		g_cam_temperature_dt_sensor_id = 0;
		return;
	}
	if (of_property_read_u32(s_ctrl->of_node, "sensor-temperature-en-reg",
		&g_cam_temperature_dt_en_reg) < 0) {
		CAM_ERR(CAM_SENSOR, "Invalid camera sensor en reg for temperature readings in device tree");
		// whole feature is disabled if some DT value is invalid
		g_cam_temperature_dt_sensor_id = 0;
		return;
	}
	if (of_property_read_u32(s_ctrl->of_node, "sensor-temperature-stat-reg",
		&g_cam_temperature_dt_stat_reg) < 0) {
		CAM_ERR(CAM_SENSOR, "Invalid camera sensor stat reg for temperature readings in device tree");
		// whole feature is disabled if some DT value is invalid
		g_cam_temperature_dt_sensor_id = 0;
		return;
	}
	if (of_property_read_u32(s_ctrl->of_node, "sensor-temperature-val-reg",
		&g_cam_temperature_dt_val_reg) < 0) {
		CAM_ERR(CAM_SENSOR, "Invalid camera sensor val reg for temperature readings in device tree");
		// whole feature is disabled if some DT value is invalid
		g_cam_temperature_dt_sensor_id = 0;
		return;
	}
...
void cam_set_temperature_reg_enable(uint32_t enable)
{
	g_cam_temperature_reg_enable = enable;
}

uint32_t cam_get_temperature_reg_enable(void)
{
    return g_cam_temperature_reg_enable;
}

uint32_t cam_get_temperature_reg_data(void)
{
    return g_cam_temperature_reg_data;
}

int cam_sensor_temperature_measurements_enable(
	...
	bool enable)
{
	...
	i2c_reg_array.reg_addr = g_cam_temperature_dt_en_reg;
	i2c_reg_array.reg_data = enable ? 1 : 0;
	i2c_reg_array.delay = 0;
	i2c_reg_settings.reg_setting = &i2c_reg_array;
	return cam_cci_i2c_write_table(io_master_info, &i2c_reg_settings);
}

void cam_sensor_temperature_measurements_process(
	struct camera_io_master *io_master_info)
{
    int rc = 0;
	uint32_t reg_data;

	if (!g_cam_temperature_measurements_active) {
		if (g_cam_temperature_reg_enable) {
			if (cam_sensor_temperature_measurements_enable(
					io_master_info, true)) {
				CAM_ERR(CAM_SENSOR,
					"Temperature measurements enable failed, rc=%d", rc);
				return;
			}
			g_cam_temperature_measurements_active = 1;
		}
		return;
	} else if (!g_cam_temperature_reg_enable) {
		if (cam_sensor_temperature_measurements_enable(
				io_master_info, false)) {
			CAM_ERR(CAM_SENSOR,
				"Temperature measurements disable failed, rc=%d", rc);
			return;
		}
		g_cam_temperature_measurements_active = 0;
		g_cam_temperature_reg_data = 0;
	...
	reg_data = 0;
	rc = cam_cci_i2c_read(io_master_info->cci_client, g_cam_temperature_dt_stat_reg,
		&reg_data, CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE);
	if (rc) {
		CAM_ERR(CAM_SENSOR, "Temperature CTRL_OUT read failed, rc=%d", rc);
		return;
	}
	if (reg_data & 1) {
		reg_data = 0;
		rc = cam_cci_i2c_read(io_master_info->cci_client, g_cam_temperature_dt_val_reg,
			&reg_data, CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE);
		if (rc) {
			CAM_ERR(CAM_SENSOR, "Temperature read failed, rc=%d", rc);
			return;
		}
		CAM_INFO(CAM_SENSOR,"Camera temperature: reg_data:0x%x", reg_data);
		g_cam_temperature_reg_data = reg_data & 0xff;
	}
}
#endif /* IS_ENABLED(CONFIG_META_CAMERA_SENSOR_TEMPERATURE) */

#if IS_ENABLED(CONFIG_META_CAMERA_SENSOR_ID_SYSFS)
static uint32_t g_rgbcam_chipid = 0;

uint32_t cam_get_rgbcam_chipid(void)
{
	return g_rgbcam_chipid;
}
#endif /* IS_ENABLED(CONFIG_META_CAMERA_SENSOR_ID_SYSFS) */
				...
				struct cam_control *cmd)
{
	...
	if (header.op_code == MSM_CCI_I2C_READ) {
		...
		i2c_reg_array = (struct cam_sensor_i2c_reg_array *) vzalloc(
			header.count * sizeof(struct cam_sensor_i2c_reg_array));
		if (!i2c_reg_array) {
			vfree(wr);
			return -ENOMEM;
		}
		...
		for (i = 0; i < header.count; i++) {
			i2c_reg_array[i].reg_addr = wr[i].reg_addr;
			i2c_reg_array[i].reg_data = wr[i].reg_data;
		}

		i2c_reg_settings.addr_type = header.addr_type;
		i2c_reg_settings.data_type = header.data_type;
		i2c_reg_settings.reg_setting = i2c_reg_array;
		i2c_reg_settings.size = header.count;
...
static int cam_sensor_ao_handle(struct cam_sensor_ctrl_t *s_ctrl,
				struct cam_control *cmd)
{
	int rc = 0;
	struct cam_cmd_power pwr;
	...
	if (pwr.cmd_type)
		...
		s_ctrl->ao_sensor_state = CAM_AO_SENSOR_DISABLED;

	CAM_DBG(CAM_SENSOR,
		"set always on mode %d", s_ctrl->ao_sensor_state);
	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		// When probe is called, user space must be restarted.
		// To make ao_sensor_state reset, the flag has to be cleared
		// before cam_sensor_power_down so that the cam_sensor_power_doen
		// can tear down camera so that the camera can work normal operation
		// from the next use case
		s_ctrl->ao_sensor_state = CAM_AO_SENSOR_DISABLED;

		/* Match sensor ID */
		rc = cam_sensor_match_id(s_ctrl);
		if (rc < 0) {
			if (s_ctrl->is_ignore_sensor_id_mismatch) {
				/*
				 * Ignore this error to support probe
				 * If this is real error, then the actual
				 * camera operation would fail
				 */
				CAM_ERR(CAM_SENSOR,
					"Sensor ID mismatched but ignore it!\n");
				rc = 0;
			} else {
				CAM_ERR(CAM_SENSOR,
		...
		if (s_ctrl->soc_info.index == 0)
			g_rgbcam_chipid = s_ctrl->sensordata->slave_info.sensor_id;
#endif
	...
	case CAM_SENSOR_CCI_CMD: {
		...
		break;
	case CAM_SENSOR_AO_CMD:
		rc = cam_sensor_ao_handle(s_ctrl, cmd);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "Failed ao handle");
			goto release_mutex;
		}
		break;
	default:
...
int cam_sensor_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	/* Once power up, disable always on mode */
	if (power_info->always_on) {
		power_info->always_on = false;
		s_ctrl->ao_sensor_state = CAM_AO_SENSOR_DISABLED;
	}
	...
	uint64_t req_id, enum cam_sensor_packet_opcodes opcode)
{
	...
	if (req_id == 0) {
		...
		case CAM_SENSOR_PACKET_OPCODE_SENSOR_INITIAL_CONFIG: {
#if IS_ENABLED(CONFIG_META_CAMERA_SENSOR_TEMPERATURE)
			cam_sensor_temperature_measurements_init(s_ctrl);
#endif
...
#if IS_ENABLED(CONFIG_META_CAMERA_SENSOR_TEMPERATURE)
			/* Read register temperature at frame interval of 32 frames */
			if ((req_id & 0x1f) == 0 &&
				(s_ctrl->sensordata->slave_info.sensor_id == g_cam_temperature_dt_sensor_id)) {
				cam_sensor_temperature_measurements_process(
					&(s_ctrl->io_master_info));
			}
#endif
		} else {
