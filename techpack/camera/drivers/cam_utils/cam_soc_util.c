	...
	uint16_t gpio_array_size)
{
	...
	gconf->cam_gpio_aoflags = kcalloc(count, sizeof(unsigned long),
		GFP_KERNEL);
	if (!gconf->cam_gpio_aoflags)
		return -ENOMEM;

	val_array = kcalloc(count, sizeof(uint32_t), GFP_KERNEL);
	if (!val_array) {
		rc = -ENOMEM;
		goto free_gpio_aoflags;
	}
	...
	/* Optional GPIO Always on flag */
	rc = of_property_read_u32_array(of_node, "gpio-req-tbl-aoflags",
		val_array, count);
	if (!rc) {
		for (i = 0; i < count; i++) {
			gconf->cam_gpio_aoflags[i] = val_array[i];
			CAM_DBG(CAM_UTIL, "cam_gpio_aoflags[%d] = %ld", i,
				gconf->cam_gpio_aoflags[i]);
		}
	}
