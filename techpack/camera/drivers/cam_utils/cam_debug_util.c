...
ssize_t cam_debug_sysfs_temperature_reg_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%i\n", cam_get_temperature_reg_enable());
}

ssize_t cam_debug_sysfs_temperature_reg_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int enable;
	if (kstrtouint(buf, 0, &enable))
		return -EINVAL;
	cam_set_temperature_reg_enable(enable);
	return count;
}

ssize_t cam_debug_sysfs_temperature_reg_data_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%i\n", cam_get_temperature_reg_data());
}
#endif /* IS_ENABLED(CONFIG_META_CAMERA_SENSOR_TEMPERATURE) */

#if IS_ENABLED(CONFIG_META_CAMERA_SENSOR_ID_SYSFS)
ssize_t cam_debug_sysfs_rgbcam_chipid_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "0x%x\n", cam_get_rgbcam_chipid());
}
