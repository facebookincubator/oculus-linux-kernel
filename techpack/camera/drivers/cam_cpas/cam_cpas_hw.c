	...
	struct cam_hw_info *cpas_hw, bool enable)
{
	...
	for (i = 0; i < cpas_core->num_axi_ports; i++) {
		...
		rc = cam_cpas_util_vote_bus_client_bw(&axi_port->bus_client,
			mnoc_ab_bw, mnoc_ib_bw, false, &applied_ab_bw,
			&applied_ib_bw);
		if (rc) {
			CAM_ERR(CAM_CPAS,
				"Failed in mnoc vote ab[%llu] ib[%llu] rc=%d",
	...
	uint32_t arg_size)
{
	...
	struct cam_cpas_client *cpas_client;
	struct cam_ahb_vote *ahb_vote = NULL;
	struct cam_axi_vote *axi_vote = NULL;
	struct cam_cpas_private_soc *soc_private = NULL;
	int rc = 0;
	long result;
	...
	ahb_vote = kzalloc(sizeof(*ahb_vote), GFP_KERNEL);
	if (!ahb_vote) {
		CAM_ERR(CAM_CPAS, "Failed to allocate ahb_vote");
		rc = -ENOMEM;
		goto done;
	}
	...
	axi_vote = kzalloc(sizeof(*axi_vote), GFP_KERNEL);
	if (!axi_vote) {
		CAM_ERR(CAM_CPAS, "Failed to allocate axi_vote");
		rc = -ENOMEM;
		goto done;
