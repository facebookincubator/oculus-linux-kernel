...
{
	long rc;
	struct cam_node_fastpath *node =
		(struct cam_node_fastpath *) v4l2_get_subdevdata(sd);

	switch (cmd) {
	case VIDIOC_CAM_CONTROL:
		rc = cam_node_fastpath_ioctl(node,
			(struct cam_control *) arg);
		break;
	default:
		CAM_ERR(CAM_CORE, "Invalid command %d", cmd);
		rc = -EINVAL;
	}
	return rc;
}
...
int cam_subdev_fastpath_remove(struct cam_subdev *sd)
{
	...
	cam_node_fastpath_deinit((struct cam_node_fastpath *)sd->token);
	kfree(sd->token);
...
{
	int rc;
	struct cam_node_fastpath *node = NULL;
