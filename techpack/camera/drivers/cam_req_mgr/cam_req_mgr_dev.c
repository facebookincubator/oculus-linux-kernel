...
static struct device_attribute camera_temperature_reg_enable_sysfs_attr =
	__ATTR(temperature_reg_enable, S_IWUSR | S_IRUGO,
	cam_debug_sysfs_temperature_reg_enable_show, cam_debug_sysfs_temperature_reg_enable_store);

static struct device_attribute camera_temperature_reg_data_sysfs_attr =
	__ATTR(temperature_reg_data, S_IRUGO, cam_debug_sysfs_temperature_reg_data_show, NULL);
#endif

#if IS_ENABLED(CONFIG_META_CAMERA_SENSOR_ID_SYSFS)
static struct device_attribute camera_rgbcam_chipid_sysfs_attr =
	__ATTR(rgbcam_chipid, S_IRUGO, cam_debug_sysfs_rgbcam_chipid_show, NULL);
#endif
...
#if IS_ENABLED(CONFIG_META_CAMERA_SENSOR_TEMPERATURE)
	rc = sysfs_create_file(&dev->kobj, &camera_temperature_reg_enable_sysfs_attr.attr);
	if (rc < 0) {
		CAM_ERR(CAM_CPAS,
			"Failed to create front camera temperature measurements enable attribute, rc=%d\n", rc);
		goto sysfs_camtempen_fail;
	}

	rc = sysfs_create_file(&dev->kobj, &camera_temperature_reg_data_sysfs_attr.attr);
	if (rc < 0) {
		CAM_ERR(CAM_CPAS,
			"Failed to create front camera temperature measurements result attribute, rc=%d\n", rc);
		goto sysfs_camtempdat_fail;
	}
#endif

#if IS_ENABLED(CONFIG_META_CAMERA_SENSOR_ID_SYSFS)
	rc = sysfs_create_file(&dev->kobj, &camera_rgbcam_chipid_sysfs_attr.attr);
	if (rc < 0) {
		CAM_ERR(CAM_CPAS,
			"Failed to create RGB camera chipid attribute, rc=%d\n", rc);
		goto sysfs_camid_fail;
	}
#endif
...
sysfs_camid_fail:
#if IS_ENABLED(CONFIG_META_CAMERA_SENSOR_TEMPERATURE)
	sysfs_remove_file(&dev->kobj, &camera_temperature_reg_data_sysfs_attr.attr);
#endif
#endif
#if IS_ENABLED(CONFIG_META_CAMERA_SENSOR_TEMPERATURE)
sysfs_camtempdat_fail:
	sysfs_remove_file(&dev->kobj, &camera_temperature_reg_enable_sysfs_attr.attr);
sysfs_camtempen_fail:
#endif
sysfs_fail:
...
#if IS_ENABLED(CONFIG_META_CAMERA_SENSOR_ID_SYSFS)
	sysfs_remove_file(&dev->kobj, &camera_rgbcam_chipid_sysfs_attr.attr);
#endif
#if IS_ENABLED(CONFIG_META_CAMERA_SENSOR_TEMPERATURE)
	sysfs_remove_file(&dev->kobj, &camera_temperature_reg_data_sysfs_attr.attr);
	sysfs_remove_file(&dev->kobj, &camera_temperature_reg_enable_sysfs_attr.attr);
