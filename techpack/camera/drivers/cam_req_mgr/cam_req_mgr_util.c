...
int cam_req_mgr_util_init(void)
{
	...
	hdl_tbl_local->bitmap_dev = kzalloc(bitmap_size, GFP_KERNEL);
	if (!hdl_tbl_local->bitmap_dev) {
		rc = -ENOMEM;
	...
	if (hdl_tbl) {
		...
		rc = -EEXIST;
		kfree(hdl_tbl_local->bitmap_dev);
		kfree(hdl_tbl_local);
		goto hdl_tbl_check_failed;
	}
...
static void cam_dump_tbl_info(void)
{
	...
	for (i = 0; i < CAM_REQ_MGR_MAX_HANDLES_V2; i++)
		CAM_INFO_RATE_LIMIT_CUSTOM(CAM_CRM, CAM_RATE_LIMIT_INTERVAL_5SEC,
			...
			hdl_tbl->hdl_dev[i].session_hdl,
			hdl_tbl->hdl_dev[i].hdl_value,
			hdl_tbl->hdl_dev[i].type,
			hdl_tbl->hdl_dev[i].state,
			hdl_tbl->hdl_dev[i].dev_id);
}
...
static int32_t cam_get_free_handle_index(enum hdl_type type)
{
	...
	if ((type == HDL_TYPE_DEV) || (type == HDL_TYPE_LINK)) {
...
int32_t cam_create_session_hdl(void *priv)
{
	...
	hdl_tbl->hdl_session[idx].dev_id = CAM_CRM;
	spin_unlock_bh(&hdl_tbl_lock);
...
int32_t cam_create_device_hdl(struct cam_create_dev_hdl *hdl_data)
{
	...
	hdl_tbl->hdl_dev[idx].dev_id = hdl_data->dev_id;
	spin_unlock_bh(&hdl_tbl_lock);
...
int32_t cam_create_link_hdl(struct cam_create_dev_hdl *hdl_data)
{
	...
	idx = cam_get_free_handle_index(HDL_TYPE_LINK);
	if (idx < 0) {
	...
	hdl_tbl->hdl_dev[idx].session_hdl = hdl_data->session_hdl;
	hdl_tbl->hdl_dev[idx].hdl_value = handle;
	hdl_tbl->hdl_dev[idx].type = HDL_TYPE_LINK;
	hdl_tbl->hdl_dev[idx].state = HDL_ACTIVE;
	hdl_tbl->hdl_dev[idx].priv = hdl_data->priv;
	hdl_tbl->hdl_dev[idx].ops = NULL;
	hdl_tbl->hdl_dev[idx].dev_id = hdl_data->dev_id;
	spin_unlock_bh(&hdl_tbl_lock);
...
void *cam_get_priv(int32_t dev_hdl, int handle_type)
{
	...
	if (hdl_addr[idx].state != HDL_ACTIVE) {
		CAM_ERR_RATE_LIMIT(CAM_CRM, "Invalid state: %d", hdl_tbl->hdl_dev[idx].state);
		goto device_priv_fail;
	}
