 ...
 * @hdl-dev: row of handles for devices
 * @hdl-session: row of handles for sessions
 * @bitmap_dev: bit map to get free hdl row idx for devices
 * @bitmap_session: bit map to get free hdl row idx for sessions
 */
struct cam_req_mgr_util_hdl_tbl {
	struct handle hdl_dev[CAM_REQ_MGR_MAX_HANDLES_V2];
	struct handle hdl_session[CAM_REQ_MGR_MAX_SESSION_HANDLES];
	void *bitmap_dev;
	void *bitmap_session;
};
