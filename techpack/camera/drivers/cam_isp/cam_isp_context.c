...
#define INC_VAL(head, incr, max_entries)  \
	((head) = (((head) + (incr)) % (max_entries)))
...
static void __cam_isp_ctx_ul_fastpath_restart_context(struct cam_context *ctx);
...
static const char *__cam_isp_ctx_get_substate_machine_irq_str(struct cam_isp_ctx_irq_ops *irq_substate_machine);
...
static inline bool is_request_valid(
	...
	struct cam_ctx_request *req)
{
	...
	if (req_start > ctx_start &&
	   req_start < (ctx_start + sizeof(struct cam_context))) {

		pr_crit("Ctx %d Invalid request.", ctx->ctx_id);
		if (req_start == (void *)&ctx->active_req_list) {
			pr_crit(
				"Invalid request. May be, taken from empty active list.");
		} else if (req_start == (void *)&ctx->pending_req_list) {
			pr_crit(
				"Invalid request. May be, taken from empty pending list.");
		} else if (req_start == (void *)&ctx->wait_req_list) {
			pr_crit(
				"Invalid request. May be, taken from empty wait list.");
		} else if (req_start == (void *)&ctx->free_req_list) {
			pr_crit(
				"Invalid request. May be, taken from empty free list.");
		}
		return false;
	}
	return true;
	...
	struct cam_ctx_request *req)
{
	...
	cur_time = ktime_get();
	if (req) {
		BUG_ON(is_request_valid(ctx_isp->base, req) == false);
...
static void __cam_isp_ctx_update_event_monitor_array(
	struct cam_isp_context *ctx_isp,
	enum cam_isp_ctx_activated_substate begin_state,
	uint32_t event)
{
	int iterator;
	struct cam_isp_context_event_monitor *ev_record;

	if ((!ctx_isp) || (!ctx_isp->base)) {
		CAM_ERR(CAM_ISP, "Invalid input ctx_isp %p", ctx_isp);
		return;
	}
	...
	ev_record->event = event;
	ev_record->end_state = ctx_isp->substate_activated;
	CAM_DBG(CAM_ISP, "On HW Event[ Enter %s Event %s Exit %s ] ctx:%d",
		__cam_isp_ctx_substate_val_to_type(begin_state),
		event_to_string(event),
		__cam_isp_ctx_substate_val_to_type(ctx_isp->substate_activated),
		ctx_isp->base->ctx_id);
}

static void __cam_isp_ctx_dump_event_monitor_array(
	struct cam_isp_context *ctx_isp)
{
	int i = 0;
	int64_t event_head = 0;
	...
	if (event_head == -1) {
		...
		num_entries = CAM_ISP_CTX_EVENT_MONITOR_MAX_ENTRIES;
		div_u64_rem(event_head + 1,
			CAM_ISP_CTX_EVENT_MONITOR_MAX_ENTRIES, &oldest_entry);
	}
	...
	index = oldest_entry;
	CAM_ERR(CAM_ISP,
		"  Index         time:         Enter Substate            Evtent     Exit Substate  Context");
	CAM_ERR(CAM_ISP,
		"-----------------------------------------------------------------------------------------");

	for (i = 0; i < num_entries; i++) {
		ev_record = &ctx_isp->cam_isp_ctx_event_monitor[index];
		...
		CAM_ERR_RATE_LIMIT(CAM_ISP,
			...
			__cam_isp_ctx_substate_val_to_type(ev_record->begin_state),
			event_to_string(ev_record->event),
			__cam_isp_ctx_substate_val_to_type(ev_record->end_state),
			ctx_isp->base->ctx_id);
	...
	struct cam_isp_context *ctx_isp)
{
	...
	struct timespec64 ts;
	uint64_t ms, sec, min, hrs;
	struct cam_isp_context_state_monitor *state_record;
	...
	for (i = 0; i < num_entries; i++) {
		state_record = &ctx_isp->cam_isp_ctx_state_monitor[index];
		...
		state_record->curr_state),
		state_record->frame_id,
		state_record->req_id,
		__cam_isp_hw_evt_val_to_type(
		state_record->trigger),
		ctx_isp->base->ctx_id);
		...
		if (open_cnt || ctx_isp->sensor_pd == 1 || ctx_isp->ul_path_en) {
			...
			if (!ctx_isp->mcu_enable)
				cam_isp_no_crm_apply_req_notify(ctx_isp, req_id,
					...
					&sof_notify->sensor_applied_req_id, sensor_setting_id);
	...
	uint64_t sof_irq_ts)
{
	...
	if (ctx_isp->independent_crm_en) {
		...
		if (list_empty(&ctx->pending_req_list) && !ctx_isp->ul_path_en) {
			...
			if (ctx_isp->debug_frame_drop_cnt == 8) {
		...
		if (PTR_ERR(task) == -EIO) {
			CAM_ERR_RATE_LIMIT(CAM_CRM, "worker %s paused, skip enqueue apply",
...
static void cam_isp_ctx_sof_independent_timer_cb(struct timer_list *timer_data)
{
	...
	if (PTR_ERR(task) == -EIO) {
		CAM_ERR_RATE_LIMIT(CAM_ISP, "worker %s paused, skip timer enqueue",
				ctx_isp->hw_mgr_worker->worker_name);
		return;
	}
	if (IS_ERR_OR_NULL(task)) {
...
static void __cam_isp_ctx_handle_req_reset_util(
	struct cam_isp_context *ctx_isp, struct cam_ctx_request *req)
{
	...
	CAM_DBG(CAM_REQ,
		...
		req->request_id, ctx_isp->active_req_cnt, ctx->ctx_id, req_isp->sensor_req_id);
	...
	void *evt_data)
{
	...
	stream_image->capture_timestamp = ctx_isp->sof_timestamp_val;
...
static int __cam_isp_ctx_handle_fence_signaling_util(
	bool signal_success, int32_t map_entry_idx,
	struct cam_isp_context *ctx_isp, struct cam_ctx_request *req)
{
	int32_t kernel_buf_handle, rc;
	struct cam_context *ctx = ctx_isp->base;
	struct cam_isp_ctx_req *req_isp =
		(struct cam_isp_ctx_req *)req->req_priv;
	...
	uint32_t *kernel_buf_ptr;
	uint32_t fence_status = CAM_SYNC_STATE_SIGNALED_ERROR,
		fence_cause = CAM_SYNC_ISP_EVENT_BUBBLE;

	CAM_DBG(CAM_ISP,
		"Sync with %s: req %lld res 0x%x fd 0x%x, ctx %u",
		signal_success ? "Success" : "Failure",
	...
	if (signal_success) {
		fence_status = CAM_SYNC_STATE_SIGNALED_SUCCESS;
		fence_cause = CAM_SYNC_COMMON_EVENT_SUCCESS;
	}

	param.sync_obj = req_isp->fence_map_out[map_entry_idx].sync_id;
	param.status = fence_status;
	param.event_cause = fence_cause;
	if (!ctx_isp->independent_crm_en)
		param.request_id = req->request_id;
	else
		param.request_id = req_isp->sensor_req_id;
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Sync = %u for req = %llu failed with rc = %d",
			req_isp->fence_map_out[map_entry_idx].sync_id, req->request_id, rc);
	...
	struct cam_isp_context *ctx_isp)
{
	...
	if (IS_ERR_OR_NULL(task)) {
		CAM_ERR_RATE_LIMIT(CAM_ISP, "No task = %d for worker", PTR_ERR(task));
		return -ENOMEM;
	}
...
error:
	...
	memset(ctx_isp->stream_images, 0, sizeof(ctx_isp->stream_images[0]) * ctx_isp->num_stream_images);
	...
	void *cmd)
{
	...
	for (i = 0; i < ctx_isp->num_stream_images; i++)
		cam_mem_put_cpu_buf((int32_t)ctx_isp->stream_images[i].packet_handle);
	...
	struct cam_context *ctx, struct cam_config_dev_cmd *cmd)
{
	...
	if (ctx_isp->independent_crm_en && ctx_isp->stream_type == CAM_REQ_MGR_LINK_TRIGGER_TYPE) {
		if (ctx->state == CAM_CTX_ACTIVATED && ctx_isp->rdi_only_context) {
			...
			task = cam_req_mgr_worker_get_task(ctx_isp->hw_mgr_worker);
			if (PTR_ERR(task) == -EIO) {
				CAM_ERR_RATE_LIMIT(CAM_CRM, "worker %s is paused, skip apply ctx:%u",
	...
	void *args)
{
	 ...
	 */
	if (ctx_isp->ul_path_en) {
		/* Setup circular queue to hold 16 results -> tune this */
		ctx_isp->ul_fp_results = kcalloc(MAX_IO_PACKETS,
			sizeof(struct cam_isp_context_ul_fp_results), GFP_KERNEL);
		if (!ctx_isp->ul_fp_results) {
			CAM_ERR(CAM_ISP,
				"Failed to allocate ul fastpath result queue ctx_id %d",
				ctx->ctx_id);
			rc = -ENOMEM;
			goto free_hw;
		}
	...
	struct cam_set_stream_mode *cmd)
{
	...
	for (i = 0; i < cmd->num_images; i++) {
		...
		int32_t packet_handle = (int32_t) cmd->stream_images[i].packet_handle;
		size_t offset = (size_t) cmd->stream_images[i].packet_offset;

		stream_image->packet_handle = packet_handle;
		...
		rc = cam_mem_get_cpu_buf(packet_handle, &packet_addr, &len);
		if (rc != 0) {
		...
		rc = ctx->hw_mgr_intf->hw_cmd(ctx->hw_mgr_intf->hw_mgr_priv,
			&hw_cmd_args);
		if (rc) {
			CAM_ERR(CAM_ISP, "ctx:%d Updating port mask failed",
				ctx->ctx_id);
			goto fail;
		}

		cfg.req_stream_mask = isp_hw_cmd_args.u.path_mask.path_irq_mask;
	...
	struct cam_set_stream_mode_cmd_get *cmd_get)
{
	...
	if (!stream_image) {
		CAM_DBG(CAM_ISP,
			"!stream_image true, waiting for images");
	...
	struct cam_isp_context    *ctx_isp)
{
	...
	ctx_isp->frame_drop_cnt = 0;
	ctx_isp->csid_rup_aup_mask = 0;
	ctx_isp->ul_fp_err_cnt = 0;
}

static int __cam_isp_ctx_query_primary_port_info(
	struct cam_context  *ctx)
{
	int rc = 0, i;
	struct cam_isp_context *isp_ctx = (struct cam_isp_context *)ctx->ctx_priv;
	...
	hw_cmd_args.ctxt_to_hw_map = ctx->ctxt_to_hw_map;
	hw_cmd_args.cmd_type = CAM_HW_MGR_CMD_INTERNAL;
	isp_hw_cmd_args.cmd_type = CAM_ISP_HW_MGR_GET_PRIMARY_PORT_INFO;
	hw_cmd_args.u.internal_args = (void *)&isp_hw_cmd_args;

	rc = ctx->hw_mgr_intf->hw_cmd(ctx->hw_mgr_intf->hw_mgr_priv, &hw_cmd_args);
	if (rc) {
		CAM_ERR(CAM_ISP,
			"Getting primary port info failed for ctx: %u link: 0x%x with rc: %d",
			ctx->ctx_id, ctx->link_hdl, rc);
		goto end;
	}
...
static void __cam_isp_ctx_ul_fastpath_reset_producer_queue(
	struct cam_isp_context *ctx_isp)
{
	int j;
	...
	spin_lock(&ctx_isp->ul_fp_params.fast_path_lock);
		for (j = 0; j < MAX_IO_RESOURCES; j++) {
			if (res_data[j].is_producer_q_valid) {
				...
				memset(producer_queue, 0x0, res_data[j].buf_count);
				res_data[j].curr_buf_index = 0;
			}
		}
		reinit_completion(&ctx_isp->ul_fp_params.fast_path_buf_done);
		atomic_set(&ctx_isp->ul_fp_params.read_idx, 0x0);
...
static void __cam_isp_ctx_ul_fastpath_restart_context(struct cam_context *ctx)
{
	...
	flush_req.type = CAM_REQ_MGR_FLUSH_TYPE_ALL;
	flush_req.req_id = 0;
	CAM_DBG(CAM_ISP, "Flush request lists in ctx %d", ctx->ctx_id);
	...
	if (!list_empty(&ctx->wait_req_list))
		__cam_isp_ctx_flush_req(ctx, &ctx->wait_req_list, &flush_req);

	if (!list_empty(&ctx->active_req_list))
		__cam_isp_ctx_flush_req(ctx, &ctx->active_req_list, &flush_req);

	CAM_DBG(CAM_ISP, "__cam_isp_ctx_ul_fastpath_reset_producer_queue ctx %d", ctx->ctx_id);
	__cam_isp_ctx_ul_fastpath_reset_producer_queue(ctx_isp);
	CAM_DBG(CAM_ISP, "__cam_isp_context_reset_ctx_params ctx %d", ctx->ctx_id);
	__cam_isp_context_reset_ctx_params(ctx_isp);
	CAM_INFO(CAM_ISP, " DONE restarting ctx %d ###############################", ctx->ctx_id);
	...
	struct response_buffer *response_buffers)
{
	...
	if (wr_idx == rd_idx) {
		...
		if (rc <= 0) {
			...
			__cam_isp_ctx_dump_event_monitor_array(isp_ctx);
	...
	for (i = 0; i < num_entries; i++) {
		...
		if (rc)
			CAM_WARN(CAM_ISP,
				...
				last_consumed, ctx->ctx_id, ctx->link_hdl, rd_idx);

		INC_VAL(rd_idx, 1, MAX_IO_PACKETS);
	}

	if (no_buf_error && !list_empty(&ctx->active_req_list))
		CAM_INFO(CAM_ISP, "Dropped the frame but defer the error till active list is consumed");
	...
	struct cam_start_stop_dev_cmd *cmd)
{
	...
	if (ctx_isp->substate_machine_irq == cam_isp_ctx_rdi_stream_activated_state_machine_irq) {
		ctx_isp->substate_activated = CAM_ISP_CTX_ACTIVATED_APPLIED;
		list_add_tail(&req->list, &ctx->wait_req_list);
		ctx_isp->waitlist_req_cnt++;
		CAM_DBG(CAM_REQ,
...
do_hw_start:
	...
	CAM_INFO(CAM_ISP, "Start IFE device ctx %u with irq state machine [%s] in initial SubState [%s] UL_path %d",
		ctx->ctx_id,
		__cam_isp_ctx_get_substate_machine_irq_str(ctx_isp->substate_machine_irq),
		__cam_isp_ctx_substate_val_to_type(ctx_isp->substate_activated),
		ctx_isp->ul_path_en);
...
static inline void __cam_isp_ctx_ul_fp_err_add(struct cam_context *ctx)
{
	struct cam_isp_context *ctx_isp =
		(struct cam_isp_context *) ctx->ctx_priv;

	if ((++ctx_isp->ul_fp_err_cnt) <= CAM_ISP_CONTEXT_UL_FP_ERROR_CNT_MAX)
		return;


	CAM_WARN(CAM_ISP,
		"UL FP Too much consecutive errors: ctx %u link: 0x%x. Restarting context",
		ctx->ctx_id, ctx->link_hdl);
	__cam_isp_ctx_ul_fastpath_restart_context(ctx);
}

static inline void __cam_isp_ctx_ul_fp_err_clear(struct cam_context *ctx)
{
	struct cam_isp_context *ctx_isp =
		(struct cam_isp_context *) ctx->ctx_priv;
	ctx_isp->ul_fp_err_cnt = 0;
...
static int cam_context_prepare_ul_request(struct cam_isp_context *ctx_isp)
{
	...
	if (list_empty(&cam_ctx->free_req_list)) {
		CAM_INFO(CAM_ISP, "free list empty, returning ctx:%u",
			cam_ctx->ctx_id);
		__cam_isp_ctx_ul_fp_err_add(cam_ctx);
		return -1;
	}
	...
	req_isp->num_deferred_acks        = setting_req_isp->num_deferred_acks;
	req_isp->bubble_report            = setting_req_isp->bubble_report;
	req_isp->reapply_type             = setting_req_isp->reapply_type;
	req_isp->bubble_detected          = setting_req_isp->bubble_detected;
	req_isp->cdm_reset_before_apply   = setting_req_isp->cdm_reset_before_apply;
	req_isp->path_irq_mask            = setting_req_isp->path_irq_mask;
	req_isp->intermediate_irq_mask.sof_irq_mask = 0;
	req_isp->intermediate_irq_mask.reg_up_irq_mask = 0;
	req_isp->intermediate_irq_mask.epoch_irq_mask = 0;
	req_isp->ul_fp_result_posted = false;
		...
		for (j = 0; j < MAX_IO_RESOURCES; j++) {
			...
			if (res_data[j].is_producer_q_valid) {
				...
				if (!free_buffer_found) {
					__cam_isp_ctx_ul_fp_err_add(cam_ctx);
					CAM_ERR(CAM_ISP, "Free buffer not found for res 0x%x ctx: %u",
						res_type, cam_ctx->ctx_id);
	...
	req_isp->num_cfg += ctx_isp->ul_data.rup_aup_cmd.num_rup_aup_cmd;
	...
	mutex_unlock(&ctx_isp->isp_mutex);
	__cam_isp_ctx_ul_fp_err_clear(cam_ctx);
	return 0;
}
		...
		bool check_applied_state, int *applied_req, int res_id, uint64_t sof_ts)
{
	...
	if (list_empty(&cam_ctx->pending_req_list)) {
		CAM_INFO_RATE_LIMIT(CAM_ISP, "pending list empty, returning ctx:%u",
			cam_ctx->ctx_id);
		rc = 1;
	} else
	...
	struct cam_req_mgr_apply_request *apply)
{
	...
	if ((ctx_isp->aeb_enabled) && (atomic_read(&ctx_isp->internal_recovery_set))) {
		int ret;
		ret = __cam_isp_ctx_reset_and_recover(false, ctx);
		mutex_unlock(&ctx_isp->isp_mutex);
		return ret;
	}
	mutex_unlock(&ctx_isp->isp_mutex);
	...
	uint32_t evt_id, void *evt_data)
{
	...
	__cam_isp_ctx_update_event_monitor_array(ctx_isp, begin_state, evt_id);
	mutex_unlock(&ctx_isp->isp_mutex);
	...
	int img_iommu_hdl)
{
	...
	init_completion(&ctx->ul_fp_params.fast_path_buf_done);
	spin_lock_init(&ctx->ul_fp_params.fast_path_lock);
...
static const char *__cam_isp_ctx_get_substate_machine_irq_str(struct cam_isp_ctx_irq_ops *irq_substate_machine)
{
	if (irq_substate_machine == cam_isp_ctx_rdi_only_activated_state_machine_irq)
		return "RDI Only Activated State Machine IRQ";
	else if (irq_substate_machine == cam_isp_ctx_offline_state_machine_irq)
		return "Offline State Machine IRQ";
	else if (irq_substate_machine == cam_isp_ctx_activated_state_machine_irq)
		return "Activated State Machine IRQ";
	else if (irq_substate_machine == cam_isp_ctx_fs2_state_machine_irq)
		return "FS2 State Machine IRQ";
	else if (irq_substate_machine == NULL)
		return "NULL State Machine IRQ";

	return "Unknown Machine IRQ";
}
