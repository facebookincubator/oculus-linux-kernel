...
static int dump_task(struct task_struct *p, void *arg)
{
	...
	pr_info("[%7d] %5d %5d %8lu %8lu %8ld %8lu         %5hd %s %pD\n",
		task->pid, from_kuid(&init_user_ns, task_uid(task)),
		task->tgid, task->mm->total_vm, get_mm_rss(task->mm),
		mm_pgtables_bytes(task->mm),
		get_mm_counter(task->mm, MM_SWAPENTS),
		task->signal->oom_score_adj, task->comm, exe_file);
	if (exe_file) {
		fput(exe_file);
	}
	task_unlock(task);
...
static void dump_tasks(struct oom_control *oc)
{
	...
	pr_info("[  pid  ]   uid  tgid total_vm      rss pgtables_bytes swapents oom_score_adj name exec_file \n");
...
static int render_oom_kill_events(struct seq_file *file, void *v)
{
	if (IS_ENABLED(CONFIG_META_OOM_KILL_STATS)) {
		dump_victims(file, true);
	}
	return 0;
}
...
static int __init oom_init(void)
{
	...
	proc_create_single("oom_kill_stats", 0, NULL, render_oom_kill_events);
	return 0;
}
