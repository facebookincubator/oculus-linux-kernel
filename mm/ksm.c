...
/* Smart scan age shift weight */
static unsigned long ksm_smart_scan_weight;
...
static void set_advisor_defaults(void)
{
	if (ksm_advisor == KSM_ADVISOR_NONE) {
		ksm_thread_pages_to_scan = DEFAULT_PAGES_TO_SCAN;
		   ...
		   ksm_advisor == KSM_ADVISOR_CPU_UTIL) {
...
static void scan_time_advisor(void)
{
	...
	pages = max(pages, ksm_advisor_min_pages_to_scan);
...
static void ksm_do_scan(unsigned int scan_npages)
{
	...
	while (scan_npages && likely(!freezing(current))) {
		cond_resched();
		rmap_item = scan_get_next_rmap_item(&page, &scan_npages);
		if (!rmap_item)
			return;
		scan_npages--;
...
static void advisor_update_scan(void)
{
	ktime_t now;
	unsigned long long cpu_time_after;
	unsigned long cpu_time_diff;
	unsigned long cpu_time_diff_ms;
	unsigned long scan_time;
	unsigned int cpu_percent_in_hundredths;

	if (ksm_advisor != KSM_ADVISOR_CPU_UTIL)
		return;

	now = ktime_get();

	if (!advisor_ctx.scan_time) {
		advisor_ctx.scan_time = now;
		advisor_ctx.cpu_time = task_sched_runtime(current);
		return;
	}

	scan_time = ktime_ms_delta(now, advisor_ctx.scan_time);
	if (scan_time < MSEC_PER_SEC)
		return;

	cpu_time_after = task_sched_runtime(current);
	cpu_time_diff = cpu_time_after - advisor_ctx.cpu_time;
	cpu_time_diff_ms = cpu_time_diff / NSEC_PER_MSEC;

	cpu_percent_in_hundredths = cpu_time_diff_ms * 100 * 100 / scan_time;

	if (ksm_advisor_max_cpu * 100 < cpu_percent_in_hundredths) {
		/*
		 * If cpu usage exceeded the threshold, reduce
		 * pages-to-scan by 25%
		 */
		ksm_thread_pages_to_scan >>= 1;
		ksm_thread_pages_to_scan += (ksm_thread_pages_to_scan >> 1);
	} else if ((ksm_advisor_max_cpu * 100 / 2) >
		   cpu_percent_in_hundredths) {
		/*
		 * If cpu usage was less than 50% of
		 * ksm_advisor_max_cpu, increase pages-to-scan by 25%
		 */
		ksm_thread_pages_to_scan += (ksm_thread_pages_to_scan >> 1);
	}

	ksm_thread_pages_to_scan =
		max((unsigned int)ksm_advisor_min_pages_to_scan,
		    ksm_thread_pages_to_scan);

	ksm_thread_pages_to_scan =
		min((unsigned int)ksm_advisor_max_pages_to_scan,
		    ksm_thread_pages_to_scan);

	advisor_ctx.scan_time = now;
	advisor_ctx.cpu_time = cpu_time_after;
}
...
static ssize_t smart_scan_weight_show(struct kobject *kobj,
				      struct kobj_attribute *attr, char *buf)
{
	return sysfs_emit(buf, "%u\n", ksm_smart_scan_weight);
}

static ssize_t smart_scan_weight_store(struct kobject *kobj,
				       struct kobj_attribute *attr,
				       const char *buf, size_t count)
{
	int err;
	...
	/* skip_age()'s max return age is 8 and rmap_age_t's size is 1 byte */
	if (value > 4)
		return -EINVAL;

	ksm_smart_scan_weight = value;
	return count;
}
KSM_ATTR(smart_scan_weight);
				 ...
				 struct kobj_attribute *attr, char *buf)
{
	...
	if (ksm_advisor == KSM_ADVISOR_NONE)
		output = "[none] scan-time cpu-util";
	else if (ksm_advisor == KSM_ADVISOR_SCAN_TIME)
		output = "none [scan-time] cpu-util";
	else if (ksm_advisor == KSM_ADVISOR_CPU_UTIL)
		output = "none scan-time [cpu-util]";
				  ...
				  size_t count)
{
	...
	else if (sysfs_streq("cpu-util", buf))
		ksm_advisor = KSM_ADVISOR_CPU_UTIL;
