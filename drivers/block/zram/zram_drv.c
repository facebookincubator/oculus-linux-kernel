		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = scnprintf(buf, PAGE_SIZE,
			...
			(u64)atomic64_read(&zram->stats.pages_recompressed),
			(u64)atomic64_read(&zram->stats.pages_recompressed_since));
	up_read(&zram->init_lock);
...
static void zram_free_page(struct zram *zram, size_t index)
{
	...
	if (zram_get_priority(zram, index))
		atomic64_dec(&zram->stats.pages_recompressed);
			   ...
			   u32 threshold, u32 prio, u32 prio_max)
{
	...
	atomic64_inc(&zram->stats.pages_stored);
	atomic64_inc(&zram->stats.pages_recompressed);
	atomic64_inc(&zram->stats.pages_recompressed_since);
