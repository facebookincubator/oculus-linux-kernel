...
static irqreturn_t lis2ds12_irq_management(int irq, void *private)
{
	struct timespec ts;
	...
	if (cdata->hwfifo_enabled) {
		...
		if (status[0] & (LIS2DS12_DRDY_MASK))
			     ...
			     const struct iio_trigger_ops *trigger_ops)
{
	...
	err = request_threaded_irq(cdata->irq, NULL,
					lis2ds12_irq_management,
