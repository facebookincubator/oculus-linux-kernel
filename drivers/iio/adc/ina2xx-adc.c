...
/**
 * For decreasing power consumption in off mode,
 * make ina236 enter shutdown mode when shutdown device
 * */
#define INA236_SHUTDOWN_MODE_MASK	GENMASK(2, 0)
...
static const struct ina2xx_config ina2xx_config[] = {
	 ...
	 *
	 * In this case: Imax = 3.5A, R_shunt = 20mΩ
	 *
	 * CURRENT_LSB = 107uA，we set it to 128uA
	 */
	[ina236] = {
		.config_default = INA226_CONFIG_DEFAULT,
		/* SHUNT_CAL */
		.calibration_value = 2000,
		/* CURRENT_LSB * R_shunt */
		.shunt_voltage_lsb = 2560,
		.bus_voltage_shift = 0,
		/* Fix value 1.6mV */
...
static struct iio_chan_spec ina226_channels[] = {
...
static struct iio_chan_spec ina219_channels[] = {
			...
			const struct i2c_device_id *id)
{
	...
	struct iio_chan_spec *pchan;
	int ret, index, chan_num;
	...
	if (id->driver_data == ina226 || id->driver_data == ina236) {
	...
	/* Rename extend name if defined in device tree */
	if (of_property_read_bool(client->dev.of_node, "channels")) {
		index = 0;
		if (id->driver_data == ina226 || id->driver_data == ina236) {
			pchan = ina226_channels;
			chan_num = ARRAY_SIZE(ina226_channels);
		} else {
			pchan = ina219_channels;
			chan_num = ARRAY_SIZE(ina219_channels);
		}
		of_property_for_each_string (client->dev.of_node, "channels",
					     prop, channel_name) {
			if (index < chan_num)
				pchan[index].extend_name = channel_name;
			index++;
		}
	}
...
static void ina2xx_shutdown(struct i2c_client *client)
{
	struct iio_dev *indio_dev = i2c_get_clientdata(client);
	struct ina2xx_chip_info *chip = iio_priv(indio_dev);

	/* Enter shutdown mode to decrease power consumption */
	regmap_update_bits(chip->regmap, INA2XX_CONFIG,
                                  INA236_SHUTDOWN_MODE_MASK, 0);
}
