			...
			struct rradc_chan_prop *prop, u16 *data)
{
	...
	case RR_ADC_ATEST:
		...
		rc = qpnp_misc_write_reg(chip->misc_node,
				chip->misc_adc_premux_sel_reg,
				FG_RR_MISC_PREMUX_SEL);
		if (rc < 0) {
			pr_err("Error setting ATEST MUX %d\n", rc);
			goto fail;
		}

		rc = rradc_masked_write(chip, FG_ADC_RR_ATEST_CTRL,
				FG_ADC_RR_ATEST_CTRL_CTL,
				FG_ADC_RR_ATEST_CTRL_CTL);

		if (rc < 0) {
			pr_err("Error setting ATEST CTRL Bit 0 %d\n", rc);
			goto fail;
		}

		/* Force conversion every cycle */
		rc = rradc_masked_write(chip, FG_ADC_RR_ATEST_TRIGGER,
				FG_ADC_RR_ATEST_EVERY_CYCLE_MASK,
				FG_ADC_RR_ATEST_EVERY_CYCLE);
		if (rc < 0) {
			pr_err("Force every cycle update failed:%d\n", rc);
			goto fail;
		}
		...
		if (rc < 0) {
			pr_err("Error reading in continuous mode:%d\n", rc);
			pr_err("Dump 256 registers from 0x4500\n");
			for (i = 0; i < 0x100; i++)
				reg_dump[i] = 0x0;

			rradc_read(chip, 0x0, reg_dump, 0xff);
		...
		/* Restore atest trigger */
		rc = rradc_masked_write(chip, FG_ADC_RR_ATEST_TRIGGER,
				FG_ADC_RR_ATEST_EVERY_CYCLE_MASK, 0);
		if (rc < 0) {
			pr_err("Restore every cycle update failed:%d\n", rc);
			goto fail;
		}
		...
		if (rc < 0) {
			pr_err("Error re-setting ATEST MUX %d\n", rc);
			goto fail;
		}
...
static int rradc_get_dt_data(struct rradc_chip *chip, struct device_node *node)
{
	...
	if (of_find_property(node, "qcom,pmic-misc", NULL)) {
		...
		rc = of_property_read_u32(node, "qcom,misc-adc-premux-sel-reg",
				&chip->misc_adc_premux_sel_reg);
		if (rc < 0 || !chip->misc_adc_premux_sel_reg) {
			pr_err("Invalid misc adc premux sel reg\n");
			return rc;
		}
