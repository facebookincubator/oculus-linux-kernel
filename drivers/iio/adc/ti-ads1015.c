...
static struct iio_chan_spec ads1015_channels[] = {
...
static struct iio_chan_spec ads1115_channels[] = {
...
int ads1015_get_adc_result(struct ads1015_data *data, int chan, int *val)
{
	...
	/* we need triggering sampling in single mode */
	if (data->channel_data[chan].single_mode_only) {
		data->conv_invalid = true;
		cfg = cfg | ADS1015_CFG_OS_MASK;
			    ...
			    int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW: {
		...
		/* if single mode enabled, don't need to wake device */
		if (!data->channel_data[chan->address].single_mode_only) {
...
static int ads1015_client_get_channels_config(struct i2c_client *client)
{
	...
	device_for_each_child_node(dev, node) {
		...
		if (fwnode_property_read_bool(node, "single-mode-only"))
			single_mode = true;

		data->channel_data[channel].pga = pga;
		data->channel_data[channel].data_rate = data_rate;
		data->channel_data[channel].single_mode_only = single_mode;
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (of_property_read_bool(client->dev.of_node, "channels")) {
		i = 0;
		if (chip == ADS1015) {
			pchan = ads1015_channels;
			chan_num = ARRAY_SIZE(ads1015_channels);
		} else {
			pchan = ads1115_channels;
			chan_num = ARRAY_SIZE(ads1115_channels);
		}
		of_property_for_each_string (client->dev.of_node, "channels",
					     prop, channel_name) {
			if (i < chan_num)
				pchan[i].extend_name = channel_name;
			i++;
		}
	}
	...
	if (ret && ret != -ENOTCONN) {
		dev_err(&client->dev, "set conv mode failed %d\n", ret);
		return ret;
	}

	if (ret == -ENOTCONN) {
		data->conv_invalid = false;
		dev_err(&client->dev, "ads chip not connected\n");
	} else {
		data->conv_invalid = true;
	}
...
static int ads1015_runtime_suspend(struct device *dev)
{
	...
	if (ret == -ENOTCONN) {
		dev_warn(dev, "ads chip not connected\n");
		return 0;
	}
	return ret;
}
