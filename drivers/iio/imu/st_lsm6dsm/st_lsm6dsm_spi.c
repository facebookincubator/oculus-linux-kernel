...
static int st_lsm6dsm_spi_probe(struct spi_device *spi)
{
	...
	cdata->vdd_io = regulator_get(&spi->dev, "vdd");
	regulator_set_voltage(cdata->vdd_io,1800000,1800000);
	err = regulator_enable(cdata->vdd_io);
	msleep(100);

	irq_gpio = of_get_named_gpio_flags(spi->dev.of_node, "lsm6dsm_irq-gpio",
				0, &irq_gpio_flags);
