...
#define ST_LSM6DSM_ODR_LIST_NUM				6
...
#define ST_LSM6DSM_GYRO_FS_245_GAIN			IIO_DEGREE_TO_RAD(8750)
...
#define ST_LSM6DSM_TEMP_L				0x20
#define ST_LSM6DSM_TEMP_H				0x21
...
static ssize_t st_lsm6dsm_sysfs_get_imu_temperature_celcius(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int err;
	u8 temp_l, temp_h;
	...
	err = sdata->cdata->tf->read(sdata->cdata, ST_LSM6DSM_TEMP_L,
			1, &temp_l, true);
	if (err < 0)
		return err;

	err = sdata->cdata->tf->read(sdata->cdata, ST_LSM6DSM_TEMP_H,
			1, &temp_h, true);
	if (err < 0)
		return err;
...
static IIO_DEVICE_ATTR(imu_temperature_celcius, S_IRUGO,
				st_lsm6dsm_sysfs_get_imu_temperature_celcius,
				NULL, 0);
...
static struct attribute *st_lsm6dsm_accel_attributes[] = {
	...
	&iio_dev_attr_imu_temperature_celcius.dev_attr.attr,
