...
{
	u64 current_time;
	int expected_lower_duration, expected_upper_duration, limit_time_base;
	...
	if (st->mode_1k_on) {
		if (st->sensor[i].rate == BASE_SAMPLE_RATE)
			limit_time_base = 900000;
		else
			limit_time_base = 450000;
	} else {
		limit_time_base = 450000;
	}

#if defined(CONFIG_INV_MPU_IIO_ICM20690) || defined(CONFIG_INV_MPU_IIO_ICM20648)
	if (st->chip_config.compass_enable && st->sensor[SENSOR_COMPASS].rate == MAX_COMPASS_RATE)
		limit_time_base = 900000;
#endif
	expected_upper_duration = st->eng_info[st->sensor[i].engine_base].divider * 1100000;
	expected_lower_duration = st->eng_info[st->sensor[i].engine_base].divider * limit_time_base;
#if 1
