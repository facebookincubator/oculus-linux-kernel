...
static int set_inv_ois_pwr_mgmt_mode(struct inv_mpu_state *st)
{
	int result = 0;
	u8 w;
	...
	result = inv_plat_single_write(st, REG_PWR_MGMT_1, w | BIT_SLEEP);
	if (result)
		return result;

	result = inv_plat_single_write(st, REG_PWR_MGMT_1, w);
	if (result)
		return result;
