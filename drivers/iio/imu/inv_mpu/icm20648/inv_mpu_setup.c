	...
	for (i = 0; i < SENSOR_NUM_MAX; i++) {
		if (st->sensor[i].on) {
			...
			if (SENSOR_PRESSURE == i) {
				if (st->eng_info[st->sensor[i].
					engine_base].running_rate * 100 /
					div[i] > MAX_PRESSURE_RATE * 150) {
					div[i]++;
				}
			}
	...
	for (i = 0; i < SENSOR_NUM_MAX; i++) {
		if (st->sensor[i].on) {
			...
			if (div[i] != st->sensor[i].div) {
				...
				pr_info(
"sensor %d div [%d] odr_addr [%02X] cntl [%04X]\n",
...
#define MIN_GYRO_RATE         112
	...
	if (st->smd.on)
		ped_on = true;
	/* ped engine and batch mode needs pedometer interrupt off */
	...
	st->eng_info[ENGINE_PRESSURE].base_time = base_time;
...
int inv_setup_dmp_firmware(struct inv_mpu_state *st)
{
	...
	if (result)
		return result;
	/* set DMP to 4G accel scale */
	result = write_be32_to_mem(st, DMP_4G_VALUE, ACC_SCALE);
	if (result)
		return result;
	result = write_be32_to_mem(st, 0x40000, ACC_SCALE2);
	if (result)
		return result;
