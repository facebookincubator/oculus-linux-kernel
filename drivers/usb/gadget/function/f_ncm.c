...
static int ncm_trigger_qtimer_ntb(struct gether *port)
{
	...
	if (!hrtimer_active(&ncm->task_timer) && ncm->skb_tx_data) {
		hrtimer_start_range_ns(&ncm->task_timer, TX_TIMEOUT_NSECS, 1000,
...
static struct usb_function *ncm_alloc(struct usb_function_instance *fi)
{
	...
	ncm->port.trigger_qtimer = ncm_trigger_qtimer_ntb;
