...
#define DWC3_GEVNTCOUNT_EVNTINTRPTMASK		(0x80000000)
...
#define MAX_ERROR_RECOVERY_TRIES	3
	...
	bool			notify_usb3_enumerated_event;
};
		...
		enum dwc3_notify_event event, unsigned int value)
{
	...
	case DWC3_CONTROLLER_CONNDONE_EVENT:
		...
		if (dwc->gadget->speed >= USB_SPEED_SUPER && mdwc->notify_usb3_enumerated_event) {
			usb_phy_set_event(mdwc->ss_phy, USB_EVENT_ENUMERATED);
			mdwc->ss_phy->flags |= DEVICE_IN_SS_MODE;
			usb_phy_notify_connect(mdwc->ss_phy, dwc->gadget->speed);
		}
...
static int dwc3_msm_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(node, "notify-usb3-enumerated-event"))
		mdwc->notify_usb3_enumerated_event = true;
