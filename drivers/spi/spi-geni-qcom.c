			...
			int *clk_idx, int *clk_div)
{
	...
	int se_clk_idx, se_clk_div;
	int ret = 0;

	if (speed_hz == mas->cur_speed_hz) {
		*clk_idx = rsc->se_clk_idx;
		*clk_div = rsc->se_clk_div;
		return ret;
	}

	ret = geni_se_clk_freq_match(&mas->spi_rsc,
				(speed_hz * mas->oversampling), &se_clk_idx,
				&sclk_freq, false);
	if (ret) {
	...
	se_clk_div = DIV_ROUND_UP(sclk_freq,  (mas->oversampling*speed_hz));
	...
	res_freq = (sclk_freq / (se_clk_div));

	dev_dbg(mas->dev, "%s: req %u resultant %lu sclk %lu, idx %d, div %d\n",
		__func__, speed_hz, res_freq, sclk_freq, se_clk_idx, se_clk_div);
	...
	mas->cur_speed_hz = speed_hz;
	*clk_idx = rsc->se_clk_idx = se_clk_idx;
	*clk_div = rsc->se_clk_div = se_clk_div;
