...
/* wait for complettion in busy loop */
static int wait_without_sleep(struct completion *x, unsigned long timeout)
{
	unsigned long start_fiffies = jiffies;

	while (!try_wait_for_completion(x)) {
		if ((jiffies - start_fiffies) > timeout)
			return 0;
	}
...
static bool busywait_mode;
static inline int _wait_for_completion_timeout(struct completion *x,
					       unsigned long timeout)
{
	if (busywait_mode)
		return wait_without_sleep(x, timeout);
	else
...
static int msm_spi_process_transfer(struct msm_spi *dd)
{
	...
	do {
		if (dd->write_buf &&
		    !_wait_for_completion_timeout(&dd->tx_transfer_complete,
		    timeout)) {
		...
		if (dd->read_buf &&
		    !_wait_for_completion_timeout(&dd->rx_transfer_complete,
		    timeout)) {
...
static DEVICE_BOOL_ATTR(busywait_mode, 0644, busywait_mode);

static struct attribute *dev_attrs[] = {
	&dev_attr_stats.attr,
	&dev_attr_busywait_mode.attr.attr,
	NULL,
};
