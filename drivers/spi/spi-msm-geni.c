...
#define SPI_AUTO_SUSPEND_DELAY	(500)
				...
				struct spi_transfer *xfer)
{
	...
	if (mas->cur_xfer_mode != GSI_DMA) {
		...
		if (!timeout) {
			...
			SPI_LOG_ERR(mas->ipc, true, mas->dev,
				"Xfer[len %d pm_susp %d irqst %08x dma_tx %08x dma_rx %08x] timed out.\n",
				xfer->len,
				pm_runtime_status_suspended(mas->dev),
				geni_read_reg(mas->base, SE_GENI_M_IRQ_STATUS),
				geni_read_reg(mas->base, SE_DMA_TX_IRQ_STAT),
				geni_read_reg(mas->base, SE_DMA_RX_IRQ_STAT));
			mas->cur_xfer = NULL;
			ret = -ETIMEDOUT;
			goto err_fifo_geni_transfer_one;
		}
