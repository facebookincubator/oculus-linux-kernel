...
static void spi_geni_se_select_mode(struct spi_geni_master *mas,
				    int xfer_mode)
{
	if (xfer_mode == mas->cur_xfer_mode)
		return;

	geni_se_select_mode(mas->base, xfer_mode);

	mas->cur_xfer_mode = xfer_mode;
}
					...
					struct spi_message *spi_msg)
{
	...
	xfer_mode = select_xfer_mode(spi, spi_msg);

	if (xfer_mode < 0) {
		dev_err(mas->dev, "%s: Couldn't select mode %d\n", __func__,
								xfer_mode);
		ret = -EINVAL;
	} else if (xfer_mode == GSI_DMA) {
		memset(mas->gsi, 0,
				(sizeof(struct spi_geni_gsi) * NUM_SPI_XFER));
		spi_geni_se_select_mode(mas, GSI_DMA);
		ret = spi_geni_map_buf(mas, spi_msg);
	} else {
		spi_geni_se_select_mode(mas, xfer_mode);
