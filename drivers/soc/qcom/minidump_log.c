...
static void register_pstore_info(void)
{
	...
	/*
	 * dmesg size is not read from device tree, instead dmesg will always occupy all remaining area
	 * after console, ftrace, pmsg are written. so calculate dmesg_size at runtime.
	 */
	dmesg_size = total_size;
