...
static int msm_minidump_freeze(struct device *dev)
{
	/*
	 * Save the minidump SMEM configuration for our hibernated kernel.
	 * Must run at .freeze (not .freeze_noirq) because the SMEM driver
	 * frees its ioremap mapping at .freeze_late — reading md_ss_toc
	 * after that is a use-after-free.
	 */
	if (minidump_table.md_ss_toc)
