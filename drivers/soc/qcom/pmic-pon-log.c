...
static const char * const pmic_pon_fault_reason1[MAX_REASON_COUNT] = {
	[0] = "GP_FAULT0",
	[1] = "GP_FAULT1",
	[2] = "GP_FAULT2",
	[3] = "GP_FAULT3",
...
static const char * const pmic_pon_fault_reason2[MAX_REASON_COUNT] = {
	[0] = "UNKNOWN(0)",
	[1] = "UNKNOWN(1)",
	[2] = "UNKNOWN(2)",
	[3] = "FAULT_N",
...
static const char * const pmic_pon_fault_reason3[MAX_REASON_COUNT] = {
	[0] = "GP_FAULT4",
	[1] = "GP_FAULT5",
	[2] = "GP_FAULT6",
	[3] = "GP_FAULT7",
...
static const char * const pmic_pon_s3_reset_reason[MAX_REASON_COUNT] = {
	[0] = "UNKNOWN(0)",
	[1] = "UNKNOWN(1)",
	[2] = "UNKNOWN(2)",
	[3] = "UNKNOWN(3)",
...
static const char * const pmic_pon_pon_pbl_status[MAX_REASON_COUNT] = {
	[0] = "UNKNOWN(0)",
	[1] = "UNKNOWN(1)",
	[2] = "UNKNOWN(2)",
	[3] = "UNKNOWN(3)",
		...
		struct pmic_pon_log_dev *pon_dev)
{
	...
	if (entry->state < ARRAY_SIZE(pmic_pon_state_label))
		...
		ipc_log_string(pon_dev->ipc_log, "State=Unknown (0x%02X); %s\n",
				entry->state, buf);
...
static int pmic_pon_log_parse(struct pmic_pon_log_dev *pon_dev)
{
	...
	for (i = 0; i < pon_dev->log_max_entries; i++) {
		...
		ret = pmic_pon_log_parse_entry(&entry, pon_dev);
		if (ret < 0)
			return ret;
...
static int pmic_pon_log_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(&pdev->dev, "sysfs node creation failed, pon_reason , ret:%d\n",
			ret);
		return ret;
	}

	ret = device_create_file(&pdev->dev, &dev_attr_power_off_reason);
	if (ret) {
		dev_err(&pdev->dev, "sysfs node creation failed, poff_reason , ret:%d\n",
			ret);
		return ret;
	}
