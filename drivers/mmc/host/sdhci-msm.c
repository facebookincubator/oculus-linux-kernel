...
{
	pr_debug("In wifi_card_detect: %d!\n", on);
...
struct sdhci_msm_pltfm_data *sdhci_msm_populate_pdata(struct device *dev,
						struct sdhci_msm_host *msm_host, unsigned int *quirks2)
{
	...
	if (of_get_property(np, "wifi-host", NULL)) {
		...
		if (quirks2) {
			*quirks2 |= SDHCI_QUIRK2_SDIO_IRQ_THREAD;
		}
		dev_info(mmc_dev(msm_host->mmc), "assigned as wifi host\n");
		pdata->nonremovable = false;
	}
