...
void sdhci_msm_mmc_clk_scaling_stop_busy(struct mmc_host *mhost, bool data)
{
	...
	if (host->clk_scaling.is_busy_started)
		_sdhci_msm_mmc_clk_scaling_stop_busy(host, data);
}

void sdhci_msm_mmc_clk_scaling_start_busy(struct mmc_host *mhost, bool data, u32 opcode)
{
	struct sdhci_host *shost = mmc_priv(mhost);
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(shost);
	struct sdhci_msm_host *host = sdhci_pltfm_priv(pltfm_host);

	if (sdhci_msm_mmc_is_data_request(opcode)) {
		sdhci_msm_mmc_deferred_scaling(host);
		_sdhci_msm_mmc_clk_scaling_start_busy(host, data);
	}

	if (host->clk_scaling.state == MMC_LOAD_LOW)
		mhost->cqe_ops->cqe_wait_for_idle(mhost);
}
 ...
 */
static int sdhci_msm_mmc_sd_change_bus_speed(struct sdhci_msm_host *host, unsigned long *freq)
{
	...
	mmc_set_clock(mhost, (unsigned int) (*freq));
		...
		unsigned long freq, enum sdhci_msm_mmc_load state)
{
	...
	if (mmc_card_sd(mhost->card))
		err = sdhci_msm_mmc_sd_change_bus_speed(host, &freq);
	else
