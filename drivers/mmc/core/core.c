	      ...
	      unsigned int arg)
{
	...
	/* Handle quirks requiring forced use VENDOR_ERASE mode */
	if ((card->quirks & MMC_QUIRK_CMD38_VENDOR_ERASE) &&
		(card->ext_csd.secure_removal_type != EXT_CSD_VENDOR_ERASE)) {
		err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
			/* Set Secure_Removal_Type */
			EXT_CSD_SECURE_REMOVAL_TYPE,
			EXT_CSD_VENDOR_ERASE,
			card->ext_csd.generic_cmd6_time);
		if (err) {
			pr_err("%s: set Secure_Removal_Type failed, err:%d\n",
