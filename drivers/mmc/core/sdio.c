...
void mmc_sdio_force_remove(struct mmc_host *host)
{
	...
	/*
	 * Avoid the race condition with periodic mmc_rescan which can
	 * also remove the card. Disable it first.
	 */
	...
	/*
	 * Wait for mmc_rescan to exit. Since it runs every 1000 ms,
	 * it is safe to wait only for maximum 2000 ms while trying.
	 */
	for (i = 0; i < 20; i++) {
		spin_lock_irqsave(&host->lock, flags);
		active = host->rescan_active;
		spin_unlock_irqrestore(&host->lock, flags);
		if (active == 0)
			break;
		msleep(100);
	...
	/*
	 * Assured that mmc_rescan is not running now, but it may have
	 * already removed the card. If so, skip card removal.
	 */
	if (host->card) {
...
static void mmc_sdio_detect(struct mmc_host *host)
{
	...
	/*
	 * A possible race condition will call mmc_sdio_remove()
	 * in another thread and host->card could become null
	 * in the middle of this routine.
	 */

	/* Make sure card is powered before detecting it */
	if (host->card && (host->caps & MMC_CAP_POWER_OFF_CARD)) {
...
out:
	if (err) {
		if (host->card) {
			...
			pr_info("mmc_sdio_detect: card is already removed\n");
		}
		mmc_claim_host(host);
		mmc_detach_bus(host);
		mmc_power_off(host);
