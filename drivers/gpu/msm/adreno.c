...
void adreno_touch_wake(struct kgsl_device *device, bool force)
{
	...
	if (adreno_dev->wake_on_touch && !force)
		return;
		...
		unsigned int code, int value)
{
	...
	if (type == EV_ABS)
		adreno_touch_wake(device, false);
}
		...
		struct device_node *parent)
{
	...
	if (node == NULL)
		...
		const char *status;
		int ret = of_property_read_string(node, "status", &status);
		if (ret == 0 && strcmp(status, "disabled") == 0)
			return adreno_of_get_legacy_pwrlevels(adreno_dev,
					parent);
	}
