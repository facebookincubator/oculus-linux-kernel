	...
	struct kgsl_drawobj *drawobj)
{
	...
	/*
	 * The hardware scheduler mutex should always be held while calling
	 * this function. The mutex primarily protects against letting the HW
	 * scheduler power down while the dispatcher is still running.
	 */
	...
	if (!test_and_set_bit(ADRENO_HWSCHED_ACTIVE, &hwsched->flags))
 ...
 * @adreno_dev: Pointer to the adreno device struct
 * @priority: Priority of the calling context.
 *
		...
		int priority)
{
	...
	/*
	 * If the dispatcher is busy and this is not being called for a maximum
	 * priority context then schedule the work for later. Always take the
	 * lock and issue the command immediately for maximum priority contexts.
	 */
	if (priority == KGSL_CONTEXT_PRIORITY_MAX)
		rt_mutex_lock(&hwsched->mutex);
	else if (!rt_mutex_trylock(&hwsched->mutex)) {
		adreno_hwsched_trigger(adreno_dev);
		return;
	}
	...
	struct kgsl_drawobj *drawobj)
{
	...
	if (drawobj->type & CMDOBJ_TYPE)
		kgsl_thread_queue_cmdobj(context->thread_priv,
				drawobj->timestamp);
...

{
	...
	if (_kgsl_context_get(&drawctxt->base)) {
		trace_dispatch_queue_context(drawctxt);
		llist_add(&job->node, &hwsched->jobs[drawctxt->base.priority]);
		adreno_hwsched_issuecmds(adreno_dev, drawctxt->base.priority);
	} else
		kmem_cache_free(jobs_cache, job);
...
static void hwsched_power_down_work(struct work_struct *work)
{
	struct adreno_hwsched *hwsched = container_of(work,
			struct adreno_hwsched, power_down_ws);
...
static void hwsched_power_down_timer(struct timer_list *t)
{
	struct adreno_hwsched *hwsched = container_of(t, struct adreno_hwsched,
			power_down_timer);

	/* This should never be the case, but check anyway. */
	if (!test_and_clear_bit(ADRENO_HWSCHED_POWER, &hwsched->flags)) {
		/* Don't leave the idle gate in a lurch. */
		if (test_and_clear_bit(ADRENO_HWSCHED_ACTIVE, &hwsched->flags))
			complete_all(&hwsched->idle_gate);
		return;
	}

	kgsl_schedule_work(&hwsched->power_down_ws);
}
...
static void hwsched_power_down(struct adreno_device *adreno_dev)
{
	...
	if (test_bit(ADRENO_HWSCHED_POWER, &hwsched->flags))
		mod_timer(&hwsched->power_down_timer, jiffies + HZ);
}
	...
	const struct adreno_hwsched_ops *target_hwsched_ops)
{
	struct kgsl_device *device = KGSL_DEVICE(adreno_dev);
	struct adreno_hwsched *hwsched = &adreno_dev->hwsched;
	struct sched_param sched_settings = {.sched_priority = MAX_RT_PRIO - 10};
	int i;
	...
	WARN_ON_ONCE(sched_setscheduler_nocheck(hwsched->worker->task,
			SCHED_FIFO, &sched_settings) != 0);
	...
	INIT_WORK(&hwsched->power_down_ws, hwsched_power_down_work);
	timer_setup(&hwsched->power_down_timer, hwsched_power_down_timer, 0);
...
static int hwsched_idle(struct adreno_device *adreno_dev)
{
	...
	/* Fire off the power-down timer immediately. */
	mod_timer(&hwsched->power_down_timer, 0);
