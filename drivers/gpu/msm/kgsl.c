...
void kgsl_print_ion_attachments(
		struct seq_file *s, struct kgsl_mem_entry *entry)
{
	struct kgsl_dma_buf_meta *meta = entry->priv_data;
	...
	list_for_each_entry(attachment, &dmabuf->attachments, node) {
		...
		if (attachment->dev->driver != NULL) {
			seq_printf(s,
				"\n ---> attachment [%d]: device: %pK '%s'",
				i, attachment->dev,
				attachment->dev->driver->name);
		} else if (attachment->dev == buf_attachment_dev) {
			scan_mem_entry = attachment->priv;
			seq_printf(s, "\n ---> attachment [%d]: device: %pK",
				i, attachment->dev);
			if (scan_mem_entry != NULL) {
				seq_printf(s, ", id: %d, PID: %d, comm: '%s'",
					scan_mem_entry->id,
					(scan_mem_entry->priv != NULL) ?
						scan_mem_entry->priv->pid : -1,
					(scan_mem_entry->priv != NULL) ?
						scan_mem_entry->priv->comm :
						"UNKNOWN");
			}
		} else {
			seq_printf(s, "\n ---> attachment [%d]: device: %pK",
...
static int kgsl_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (cache == KGSL_CACHEMODE_WRITEBACK
		...
		for (i = 0; i < m->page_count; i++) {
			...
			atomic_long_add(PAGE_SIZE, &entry->map_count);
		}
	}
