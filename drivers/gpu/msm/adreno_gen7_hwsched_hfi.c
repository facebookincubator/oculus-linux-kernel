...
static struct gen7_hfi *to_gen7_hfi(struct adreno_device *adreno_dev)
{
...
static void gen7_hwsched_process_msgq(struct adreno_device *adreno_dev)
{
	...
	read_lock_irqsave(&hfi->cmdq_lock, flags);
	for (;;) {
		 ...
		 * holding the hardware scheduler mutex.
		 */
	...
	read_unlock_irqrestore(&hfi->cmdq_lock, flags);
}
...
static irqreturn_t gen7_hwsched_hfi_handler(int irq, void *data)
{
	...
	gmu_core_regwrite(device, GEN7_GMU_GMU2HOST_INTR_CLR, hfi->irq_mask);
	...
	if (status & (HFI_IRQ_MSGQ_MASK | HFI_IRQ_DBGQ_MASK)) {
		if (!is_queue_empty(adreno_dev, HFI_MSG_ID))
			gen7_hwsched_process_msgq(adreno_dev);
		if (!is_queue_empty(adreno_dev, HFI_DBG_ID))
...
static int send_start_msg(struct adreno_device *adreno_dev)
{
	...
	rc = gen7_hfi_cmdq_write(adreno_dev, (u32 *)&cmd);
	if (rc)
		return rc;
...
int gen7_hwsched_hfi_start(struct adreno_device *adreno_dev)
{
	...
	ret = gen7_hfi_send_generic_req(adreno_dev, &gmu->hfi.bw_table);
	if (ret)
		goto err;
...
static int submit_lpac_raw_cmds(struct adreno_device *adreno_dev, void *cmds,
	const char *str)
{
	int ret;
...
static int cp_init(struct adreno_device *adreno_dev)
{
	...
	cmds[0] = CREATE_MSG_HDR(H2F_MSG_ISSUE_CMD_RAW,
		(GEN7_CP_INIT_DWORDS + 1) << 2, HFI_MSG_CMD);
...
int gen7_hwsched_lpac_cp_init(struct adreno_device *adreno_dev)
{
	...
	cmds[0] = CREATE_MSG_HDR(H2F_MSG_ISSUE_LPAC_CMD_RAW,
		(GEN7_CP_INIT_DWORDS + 1) << 2, HFI_MSG_CMD);

	gen7_cp_init_cmds(adreno_dev, &cmds[1]);

	return submit_lpac_raw_cmds(adreno_dev, cmds,
	...
	struct kgsl_drawobj *drawobj, void *cmdbuf)
{
	...
	ret = gen7_gmu_context_queue_write(adreno_dev, drawctxt, (u32 *)cmd, drawobj, NULL);
...
static int gen7_hfi_dispatch_queue_write(struct adreno_device *adreno_dev, u32 queue_idx,
	u32 *msg, struct kgsl_drawobj_cmd *cmdobj, struct adreno_submit_time *time)
{
	struct gen7_gmu_device *gmu = to_gen7_gmu(adreno_dev);
	struct hfi_queue_table *tbl = gmu->hfi.hfi_mem->hostptr;
	struct hfi_queue_header *hdr = &tbl->qhdr[queue_idx];
	...
	struct kgsl_drawobj_cmd *cmdobj)
{
	...
	ret = gen7_hfi_send_cmd_async(adreno_dev, cmd);
...
void gen7_hwsched_context_detach(struct adreno_context *drawctxt)
{
	...
	/*
	 * Prevent the HW scheduler from submitting more commands to the
	 * GMU until the context has been detached.
	 */
