...
static ssize_t gpubusy_accum_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct kgsl_device *device = dev_get_drvdata(dev);
...
static DEVICE_ATTR_RO(gpubusy_accum);
...
static void kgsl_idle_check(struct kthread_work *work);
...
int kgsl_pwrctrl_init(struct kgsl_device *device)
{
	...
	kthread_init_work(&device->idle_check_work, kgsl_idle_check);
		...
		if (!atomic_read(&device->active_cnt)) {
			...
			if (ret == -EBUSY) {
				...
				kgsl_pwrctrl_request_state(device,
					requested_state);
				kthread_queue_work(&kgsl_driver.worker,
					&device->idle_check_work);
			}
		}
done:
		if (!ret)
