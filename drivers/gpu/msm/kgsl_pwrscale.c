...
void kgsl_pwrscale_wake(struct kgsl_device *device)
{
	...
	spin_lock_irqsave(&psc->timer_lock, flags);
	psc->time = ktime_get();
	...
	spin_unlock_irqrestore(&psc->timer_lock, flags);
}
...
void kgsl_pwrscale_update_stats(struct kgsl_device *device)
{
	...
	if (device->state == KGSL_STATE_ACTIVE) {
		...
		int active_pwrlevel = READ_ONCE(pwrctrl->active_pwrlevel);
		...
		pwrctrl->clock_times[active_pwrlevel] +=
				stats.busy_time;
		pwrctrl->time_in_pwrlevel[active_pwrlevel] +=
...
void kgsl_pwrscale_update(struct kgsl_device *device)
{
	...
	spin_lock_irqsave(&device->pwrscale.timer_lock, flags);
	t = ktime_get();
	if (ktime_compare(t, device->pwrscale.next_governor_call) < 0) {
		spin_unlock_irqrestore(&device->pwrscale.timer_lock, flags);
		return;
	}
	...
	/* Call update_devfreq() from the workqueue if we're not slumbering. */
	if (READ_ONCE(device->state) != KGSL_STATE_SLUMBER)
		queue_work(device->pwrscale.devfreq_wq,
...
int kgsl_devfreq_target(struct device *dev, unsigned long *freq, u32 flags)
{
	...
	active_pwrlevel = READ_ONCE(pwr->active_pwrlevel);
	cur_freq = pwr->pwrlevels[active_pwrlevel].gpu_freq;
	level = active_pwrlevel;
		...
		const char *governor)
{
	...
	spin_lock_init(&pwrscale->timer_lock);
