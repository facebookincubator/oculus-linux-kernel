...
static void convert_vbat_thresh_val_to_adc(int *val)
{
...
static int bcl_set_vbat_trip(struct thermal_zone_device *tzd,
		int trip, int temp)
{
	int16_t addr = 0;
	uint8_t val = 0;
	...
	pr_debug("%s: Setting trip %d to %d\n", __func__, trip, temp);

	switch (trip + BCL_VBAT_LVL0) {
	case BCL_VBAT_LVL0:
		addr = BCL_VBAT_ADC_LOW;
		break;
	case BCL_VBAT_LVL1:
	...
	if (addr == BCL_VBAT_ADC_LOW) {
		convert_vbat_thresh_val_to_adc(&temp);
		/* ADC values are always positive, MSB should always be 0 */
		val = (temp & 0x7F);
		pr_debug("vbat trip: %d, ADC:0x%02x\n", trip, temp);
	} else {
		if (temp > BCL_VBAT_MAX_MV)
			temp = BCL_VBAT_MAX_MV;
		val = (((temp - BCL_VBAT_THRESH_BASE) / 25) & 0xFF);
		pr_debug("vbat-%s-low trip: %d mV ADC:0x%02x\n",
				(addr == BCL_VBAT_COMP_LOW) ?
				"too" : "critical",
				temp, val);
	...
	return bcl_write_register(bat_data->dev, addr, val);
}
...
static struct thermal_zone_device_ops vbat_tzd_ops = {
	...
	.set_trip_temp = bcl_set_vbat_trip,
		...
		enum bcl_dev_type type, struct bcl_device *bcl_perph)
{
	...
	vbat->tz_dev = thermal_zone_device_register("vbat", 3, 3, vbat,
