...
#define PTN5150_NO_ATTACHED                     0
#define PTN5150_DFP_ATTACHED			0x1
#define PTN5150_UFP_ATTACHED			0x2
#define PTN5150_AUDIO_ATTACHED                  0x3
#define PTN5150_DEBUG_ATTACHED                  0x4

#define PTN5150_RP_STANDBY			0
#define PTN5150_RP_STD_USB			0x1
#define PTN5150_RP_1_5_A			0x2
#define PTN5150_RP_3_0_A			0x3

#define PTN5150_PORT_STATE_UFP_MODE		0
#define PTN5150_PORT_STATE_DFP_MODE		1
#define PTN5150_PORT_STATE_DRP_MODE		2
...
#define PTN5150_REG_CC_RP_DETECTION             GENMASK(6, 5)
...
static void update_curr_capability(struct ptn5150_info *info)
{
	int ret;
	unsigned int reg_data, rp_status;
	...
	rp_status = FIELD_GET(PTN5150_REG_CC_RP_DETECTION, reg_data);
	switch (rp_status) {
	case PTN5150_RP_STANDBY:
	case PTN5150_RP_STD_USB:
		extcon_set_state_sync(info->edev, EXTCON_CHG_USB_SLOW, true);
		extcon_set_state_sync(info->edev, EXTCON_CHG_USB_FAST, false);
		break;
	case PTN5150_RP_1_5_A:
	case PTN5150_RP_3_0_A:
		extcon_set_state_sync(info->edev, EXTCON_CHG_USB_FAST, true);
		extcon_set_state_sync(info->edev, EXTCON_CHG_USB_SLOW, false);
		break;
	}
	return;
};
...
static void ptn5150_check_state(struct ptn5150_info *info)
{
	...
	gpio_val = gpio_get_value(info->dts_gpio);
	if (!gpio_val) {
		port_status = PTN5150_DEBUG_ATTACHED;
		dev_dbg(info->dev, "DTS insert\n");
	}
			     ...
			     char *buf)
{
	...
	unsigned int reg_data, port_status;
	int ret, gpio_val;
...
static ssize_t usb_type_show(struct device *dev,
			     struct device_attribute *attr,
			     char *buf)
{
	struct ptn5150_info *info = dev_get_drvdata(dev);
...
static DEVICE_ATTR_RO(usb_type);

static ssize_t cc_polarity_show(struct device *dev,
			     struct device_attribute *attr,
			     char *buf)
{
	struct ptn5150_info *info = dev_get_drvdata(dev);
	unsigned int reg_data, cc_status;
	int ret;
	...
	cc_status = FIELD_GET(PTN5150_REG_CC_POLARITY, reg_data);
...
static DEVICE_ATTR_RO(cc_polarity);

static int ptn5150_create_attrs(struct device *dev)
{
	int ret;

	ret = device_create_file(dev, &dev_attr_device_attached);
	ret |= device_create_file(dev, &dev_attr_usb_type);
	ret |= device_create_file(dev, &dev_attr_cc_polarity);
	return ret;
}
...
static int ptn5150_i2c_probe(struct i2c_client *i2c)
{
	...
	info->dts_gpio = of_get_named_gpio(dev->of_node, "dts-gpio", 0);
	if (gpio_is_valid(info->dts_gpio)) {
		if (devm_gpio_request(dev, info->dts_gpio, "dts_gpio")) {
			dev_err(dev, "failed to request dts_gpio\n");
			return -EINVAL;
		}
	}
	...
	ret = ptn5150_create_attrs(info->dev);
	if (ret) {
		dev_err(info->dev, "failed to create attributes, ret = %d\n", ret);
		return ret;
	}
