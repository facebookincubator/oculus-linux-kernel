...
megasas_fusion_update_can_queue(struct megasas_instance *instance, int fw_boot_context)
{
	...
	/* unknown6 FW does not fill outbound_scratch_pad_2 with queue depth */
 ...
 * unknown6_SERIES each set of 16 reply queues(0-15, 16-31, ..) should be
 ...
 * unknown6_SERIES to manage INVADER_SERIES as well.
 */
			  ...
			  struct megasas_cmd_fusion *cmd)
{
	...
	if (instance->adapter_type >= unknown6_SERIES) {
		...
		if (!fp_possible ||
			...
			spin_unlock_irqrestore(&instance->stream_lock,
					       spinlock_flags);
			/* In unknown6 if stream detected for a read and it is
	...
	if (fp_possible) {
		...
		} else if (instance->adapter_type >= unknown6_SERIES) {
	...
	bool fp_possible)
{
	...
	if (instance->support_seqnum_jbod_fp) {
		if (instance->use_seqnum_jbod_fp &&
			...
			/* More than 256 PD/JBOD support for unknown6 */
