...
static struct clk_vdd_class *disp_cc_neo_regulators[] = {
	&vdd_mm,
	&vdd_mxa,
};
...
static struct clk_rcg2 disp_cc_mdss_byte0_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = disp_cc_neo_regulators,
		.num_vdd_classes = ARRAY_SIZE(disp_cc_neo_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 187500000,
			[VDD_LOW] = 300000000,
...
static struct clk_regmap *disp_cc_neo_clocks[] = {
...
static const struct regmap_config disp_cc_neo_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static struct qcom_cc_desc disp_cc_neo_desc = {
	.config = &disp_cc_neo_regmap_config,
	.clks = disp_cc_neo_clocks,
	.num_clks = ARRAY_SIZE(disp_cc_neo_clocks),
	.resets = disp_cc_neo_resets,
...
static const struct of_device_id disp_cc_neo_match_table[] = {
	{ .compatible = "qcom,neo-dispcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, disp_cc_neo_match_table);

static int disp_cc_neo_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &disp_cc_neo_desc);
	if (IS_ERR(regmap))
...
static void disp_cc_neo_sync_state(struct device *dev)
{
...
static int __init disp_cc_neo_init(void)
{
	return platform_driver_register(&disp_cc_neo_driver);
}
subsys_initcall(disp_cc_neo_init);

static void __exit disp_cc_neo_exit(void)
{
	platform_driver_unregister(&disp_cc_neo_driver);
}
module_exit(disp_cc_neo_exit);

MODULE_DESCRIPTION("QTI DISP_CC NEO Driver");
