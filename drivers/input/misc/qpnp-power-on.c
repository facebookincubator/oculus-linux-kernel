...
#define QPNP_POFF_REASON_UNKNOWN		-1
...
struct qpnp_pon {
	...
	struct work_struct	charger_notif_work;
...
static ssize_t power_off_reason_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	struct qpnp_pon *pon = (struct qpnp_pon *)dev_get_drvdata(dev);
	...
	if (pon->pon_power_off_reason == QPNP_POFF_REASON_UNKNOWN) {
		ret = snprintf(buf, PAGE_SIZE, "%s\n", "Unknown");
	} else {
		ret = snprintf(buf, PAGE_SIZE, "%s\n",
...
static int qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	if (pon->cblpwr_usb_conn && (cfg->pon_type == PON_CBLPWR)) {
...
static int qpnp_pon_psy_get_property(struct power_supply *psy,
...
static void qpnp_pon_usb_conn_work(struct work_struct *work)
{
	struct qpnp_pon *pon = container_of(work, struct qpnp_pon,
					    charger_notif_work);
	struct qpnp_pon_config *pon_cfg = qpnp_get_cfg(pon, PON_CBLPWR);
	bool state = pon_cfg->old_state;

	dev_info(pon->dev, "PMIC input: CBLPWR_ON=%d\n", state);

	extcon_set_state_sync(pon->extcon, EXTCON_USB, state);
	power_supply_changed(pon->charger);
}

static int qpnp_pon_usb_conn_init(struct device *dev, struct qpnp_pon *pon)
{
	int rc;
	uint pon_rt_sts;
	struct power_supply_desc *pwr_desc;
	struct power_supply_config pwr_cfg = {
		.of_node = dev->of_node,
	};
	struct qpnp_pon_config *pon_cfg = qpnp_get_cfg(pon, PON_CBLPWR);

	pon->extcon = devm_extcon_dev_allocate(dev, usb_extcon_cable);
	if (IS_ERR(pon->extcon)) {
		rc = PTR_ERR(pon->extcon);
		dev_err(dev, "Failed to allocate extcon device ret=%d\n", rc);
		return rc;
	}

	rc = devm_extcon_dev_register(dev, pon->extcon);
	if (rc < 0) {
		dev_err(dev, "Failed to register extcon device ret=%d\n", rc);
		return rc;
	}

	INIT_WORK(&pon->charger_notif_work, qpnp_pon_usb_conn_work);
	...
	/* This is used as the USB power supply status */
	pon_cfg->old_state = !!(pon_rt_sts & QPNP_PON_CBLPWR_N_SET);

	pon->charger = devm_power_supply_register(dev, pwr_desc, &pwr_cfg);
	if (IS_ERR(pon->charger)) {
		rc = PTR_ERR(pon->charger);
		dev_err(dev, "Unable to register charger ret=%d\n", rc);
		return rc;
	}

	/* Perform the initial notification */
	schedule_work(&pon->charger_notif_work);
				       ...
				       struct device_node *node)
{
	...
	if (pon->cblpwr_usb_conn) {
		rc = qpnp_pon_usb_conn_init(&pdev->dev, pon);
		if (rc) {
			dev_err(&pdev->dev,
				"Failed to initialize extcon rc=%d\n", rc);
			return rc;
		}
	}
...
static int qpnp_pon_read_hardware_info(struct qpnp_pon *pon, bool sys_reset)
{
	...
	if (index >= ARRAY_SIZE(qpnp_poff_reason) || index < 0) {
		dev_info(dev, "PMIC@SID%d: Unknown power-off reason\n",
			 to_spmi_device(dev->parent)->usid);
		pon->pon_power_off_reason = QPNP_POFF_REASON_UNKNOWN;
	} else {
...
static int qpnp_pon_probe(struct platform_device *pdev)
{
	...
	pon->cblpwr_usb_conn =
		of_property_read_bool(dev->of_node, "qcom,cblpwr-usb-conn");
	...
	if (rc) {
		dev_err(dev,
			"sysfs power-on reason file creation failed, rc=%d\n",
			rc);
		return rc;
	}
	...
	if (rc) {
		dev_err(dev,
			"sysfs power-off reason file creation failed, rc=%d\n",
			rc);
		return rc;
	}
