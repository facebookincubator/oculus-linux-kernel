...
static int fts_input_report_b(struct fts_ts_data *data)
{
    ...
    for (i = 0; i < data->touch_point; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            ...
            if (data->pdata->ts_orientation == 0) {
                input_report_abs(data->input_dev, ABS_MT_POSITION_X, events[i].x);
                input_report_abs(data->input_dev, ABS_MT_POSITION_Y, events[i].y);
            } else if (data->pdata->ts_orientation == 180) {
                input_report_abs(data->input_dev, ABS_MT_POSITION_X,
                    data->pdata->x_max - events[i].x);
                input_report_abs(data->input_dev, ABS_MT_POSITION_Y,
                    data->pdata->y_max - events[i].y);
            }
...
#if (FTS_CHIP_TYPE == _FT8719) || (FTS_CHIP_TYPE == _FT8615)
...
#ifndef CONFIG_TOUCHSCREEN_FTS_BUS_SPI
    if (data->gesture_mode) {
...
static int fts_gpio_configure(struct fts_ts_data *data)
{
    ...
    /* request ts_select gpio */
    if (gpio_is_valid(data->pdata->ts_select_gpio)) {
        ret = gpio_request(data->pdata->ts_select_gpio, "ts_select_gpio");
        if (ret) {
            FTS_ERROR("[GPIO]ts_select gpio request failed");
            goto err_ts_select_gpio_req;
        }

        ret = gpio_direction_output(data->pdata->ts_select_gpio, 1);
        if (ret) {
            FTS_ERROR("[GPIO]set_direction for ts_select gpio failed");
            goto err_ts_select_gpio_dir;
        }
        FTS_INFO("[GPIO]ts_select gpio AP selected (1)");
    } else {
        FTS_ERROR("[GPIO]ts_select gpio not valid");
    }
...
err_ts_select_gpio_dir:
    if (gpio_is_valid(data->pdata->ts_select_gpio))
        gpio_free(data->pdata->ts_select_gpio);
err_ts_select_gpio_req:
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    /* gpio for i2c mux connecting touchscreen, High:SDM429W, Low:RT600 */
    pdata->ts_select_gpio = of_get_named_gpio_flags(np, "focaltech,ts-select-gpio",
                        0, &pdata->ts_select_gpio_flags);
    if (pdata->ts_select_gpio < 0)
        FTS_ERROR("Unable to get ts_select_gpio");

    /* touch screen orientation, 0 or 180 */
    ret = of_property_read_u32(np, "focaltech,ts-orientation", &pdata->ts_orientation);
    if (ret < 0) {
        FTS_INFO("Orientation undefined, using zero");
        pdata->ts_orientation = 0;
    } else {
        FTS_INFO("Orientation %d", pdata->ts_orientation);
    }
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    /*maybe not msm_drm_register_client(), please check your platform*/
...
#endif
    if (gpio_is_valid(ts_data->pdata->ts_select_gpio))
        gpio_free(ts_data->pdata->ts_select_gpio);
