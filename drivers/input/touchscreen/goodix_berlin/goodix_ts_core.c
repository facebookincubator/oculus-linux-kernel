...
/* reset chip */
static ssize_t goodix_ts_gesture_cmd_store(struct device *dev,
					   struct device_attribute *attr,
...
static ssize_t mcu_fab_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	struct goodix_ts_core *cd = dev_get_drvdata(dev);
	struct goodix_ts_hw_ops *hw_ops = cd->hw_ops;
...
static DEVICE_ATTR(gesture_cmd, 0220, NULL, goodix_ts_gesture_cmd_store);
...
static DEVICE_ATTR(get_mcu_fab, 0440, mcu_fab_show, NULL);

static struct attribute *sysfs_attrs[] = {
	&dev_attr_driver_info.attr, &dev_attr_chip_info.attr,
	&dev_attr_reset.attr,	    &dev_attr_gesture_cmd.attr,
	&dev_attr_send_cfg.attr,    &dev_attr_read_cfg.attr,
	&dev_attr_reg_rw.attr,	    &dev_attr_irq_info.attr,
	&dev_attr_esd_info.attr,    &dev_attr_debug_log.attr,
	&dev_attr_get_mcu_fab.attr, NULL,
};
		...
		struct goodix_ts_board_data *board_data)
{
	...
	ret = of_property_read_u32(node, "goodix,dish-panel-max-x",
				   &board_data->dish_panel_max_x);
	if (ret) {
		ts_err("failed get dish-panel-max-x");
		return ret;
	}

	ret = of_property_read_u32(node, "goodix,dish-panel-max-y",
				   &board_data->dish_panel_max_y);
	if (ret) {
		ts_err("failed get dish-panel-max-y");
		return ret;
	}

	ret = of_property_read_u32(node, "goodix,dish-panel-max-w",
				   &board_data->dish_panel_max_w);
	if (ret) {
		ts_err("failed get dish-panel-max-w");
		return ret;
	}
...
static unsigned int goodix_ts_keymap(u8 type)
{
	...
	case GESTURE_TRIPLE_TAP:
		key_code = KEY_FN_F6;
		break;
	case GESTURE_RADIAL_SWIP:
		key_code = KEY_FN_F7;
		break;
	case GESTURE_THUMBSTICK:
		key_code = KEY_FN_F8;
		break;
	case GESTURE_TWO_FINGER_CLICK:
		key_code = KEY_FN_F9;
		break;
...
static void goodix_ts_report_gesture(struct input_dev *dev,
				     struct goodix_touch_data *touch_data)
{
	static u8 pre_rt_key_code;
	u8 gesture_type;
	unsigned int key_code;

	gesture_type = touch_data->gesture_type;
	if (!gesture_type && !pre_rt_key_code)
		return;

	if (!gesture_type && pre_rt_key_code) {
		//report up event for rt time event
		input_report_key(dev, pre_rt_key_code, 0);
		input_sync(dev);
		pre_rt_key_code = 0;
		ts_info("[plat]report rt key up %d", pre_rt_key_code);
		return;
	}

	key_code = goodix_ts_keymap(gesture_type);
	if (!key_code) {
		ts_err("unsupported gesture type %d", gesture_type);
		return;
	}
	input_report_key(dev, key_code, 1);
	input_sync(dev);
	if (!touch_data->is_rt_gesture) {
		input_report_key(dev, key_code, 0);
		input_sync(dev);
	} else {
		pre_rt_key_code = key_code;
		input_report_abs(dev, ABS_RX, touch_data->gesture_value[0]);
		input_report_abs(dev, ABS_RY, touch_data->gesture_value[1]);
		input_report_abs(dev, ABS_RZ, touch_data->gesture_value[2]);
	}
}

static void goodix_ts_report_gesture_dish(struct input_dev *dev,
					  struct goodix_touch_data *touch_data)
{
	static unsigned int pre_rt_key_code;
	u8 gesture_type;
	unsigned int key_code;
	...
	if (!gesture_type && pre_rt_key_code) {
		...
		input_report_key(dev, pre_rt_key_code, 0);
		input_sync(dev);
		ts_info("rt_gst key up %d", gesture_type);
		pre_rt_key_code = 0;
		return;
	}
	...
	ts_debug("input report key %d, rt flag %d", key_code,
		 touch_data->is_rt_gesture);
	...
	if (likely(!ret)) {
		if (ts_event->event_type & EVENT_TOUCH) {
			...
			goodix_ts_report_finger(core_data->input_dev,
					&ts_event->touch_data);
			goodix_ts_report_finger(core_data->dish_input_dev,
						&ts_event->dish_touch_data);
		}
		goodix_ts_report_gesture(core_data->input_dev,
					 &ts_event->touch_data);
		goodix_ts_report_gesture_dish(core_data->dish_input_dev,
					      &ts_event->dish_touch_data);
...
static int goodix_ts_input_dev_config(struct goodix_ts_core *core_data)
{
	...
	/* set input parameters */
	input_set_abs_params(input_dev, ABS_RX, 0, 2047, 0, 0);
	input_set_abs_params(input_dev, ABS_RY, 0, 359, 0, 0);
	input_set_abs_params(input_dev, ABS_RZ, 0, 1024, 0, 0);
	//TODO: confirm gesture keycode
	input_set_capability(input_dev, EV_KEY, KEY_FN_F1);
	input_set_capability(input_dev, EV_KEY, KEY_FN_F2);
	input_set_capability(input_dev, EV_KEY, KEY_FN_F3);
	input_set_capability(input_dev, EV_KEY, KEY_FN_F4);
	input_set_capability(input_dev, EV_KEY, KEY_FN_F5);
	input_set_capability(input_dev, EV_KEY, KEY_FN_F6);
	input_set_capability(input_dev, EV_KEY, KEY_FN_F7);
	input_set_capability(input_dev, EV_KEY, KEY_FN_F8);
	input_set_capability(input_dev, EV_KEY, KEY_FN_F9);
	input_set_capability(input_dev, EV_KEY, KEY_FN_F10);
...
/**
 * goodix_ts_dish_input_dev_config - Requset and config a input device for dish area
 *  then register it to input sybsystem.
 * @core_data: pointer to touch core data
 * return: 0 ok, <0 failed
 */
static int goodix_ts_dish_input_dev_config(struct goodix_ts_core *core_data)
{
	struct goodix_ts_board_data *ts_bdata = board_data(core_data);
	struct input_dev *input_dev = NULL;
	int r;
	...
	core_data->dish_input_dev = input_dev;
	input_set_drvdata(input_dev, core_data);

	input_dev->name = "goodix_ts,dish";
	input_dev->id.product = 0xDEAD;
	input_dev->id.vendor = 0xBEEF;
	input_dev->id.version = 10427;
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0,
			     ts_bdata->dish_panel_max_x, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0,
			     ts_bdata->dish_panel_max_y, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0,
			     ts_bdata->dish_panel_max_w, 0, 0);
#ifdef INPUT_TYPE_B_PROTOCOL
#if LINUX_VERSION_CODE > KERNEL_VERSION(3, 7, 0)
...
void goodix_ts_input_dev_remove(struct goodix_ts_core *core_data)
{
	...
	/* remove dish panel dev */
	input_unregister_device(core_data->dish_input_dev);
	input_free_device(core_data->dish_input_dev);
	core_data->dish_input_dev = NULL;
}
...
int goodix_ts_stage2_init(struct goodix_ts_core *cd)
{
	...
	ret = goodix_ts_dish_input_dev_config(cd);
	if (ret) {
		ts_err("failed set input device for dish area");
		return ret;
	}
