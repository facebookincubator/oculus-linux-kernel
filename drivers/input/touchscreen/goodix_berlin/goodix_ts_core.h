...
#define GOODIX_MAX_TOUCH 3
#define GOODIX_MAX_GESTURE_NUM 2
...
enum GOODIX_GESTURE_TYP {
	GESTURE_RADIAL_SWIP = 0x2,
	GESTURE_THUMBSTICK = 0x3,
	GESTURE_TWO_FINGER_CLICK = 0x66,

	GESTURE_SWIPE_RIGHT = 0xAA,
	GESTURE_SWIPE_DOWN = 0xAB,
	GESTURE_SWIPE_UP = 0xBA,
	GESTURE_SWIPE_LEFT = 0xBB,

	GESTURE_SINGLE_TAP = 0x4C,
	GESTURE_DOUBLE_TAP = 0xCC,
	GESTURE_TRIPLE_TAP = 0xCD,
};
...
struct goodix_ts_board_data {
	...
	unsigned int dish_panel_max_x;
	unsigned int dish_panel_max_y;
	unsigned int dish_panel_max_w; /*major and minor*/
...
struct goodix_ts_event {
	...
	struct goodix_touch_data dish_touch_data;
...
struct goodix_ts_core {
	...
	struct input_dev *dish_input_dev;
...
#define ts_info(fmt, arg...) pr_info("[GTP-INF][%s] " fmt "\n", __func__, ##arg)
#define ts_err(fmt, arg...) pr_err("[GTP-ERR][%s] " fmt "\n", __func__, ##arg)
