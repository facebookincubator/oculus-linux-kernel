...
#define POINT_TYPE_DISH 0x06
static void goodix_parse_finger(struct goodix_ts_event *ts_event, u8 *buf,
				int touch_num)
{
	struct goodix_touch_data *touch_data = &ts_event->touch_data;
	struct goodix_touch_data *dish_touch_data = &ts_event->dish_touch_data;
	...
	for (i = 0; i < touch_num; i++) {
		...
		if (id >= GOODIX_MAX_TOUCH) {
			...
			dish_touch_data->touch_num = 0;
			return;
		}
		x = le16_to_cpup((__le16 *)(coor_data + 2));
		y = le16_to_cpup((__le16 *)(coor_data + 4));
		w = le16_to_cpup((__le16 *)(coor_data + 6));
		if ((coor_data[0] & 0xF) == POINT_TYPE_DISH) {
			dish_touch_data->coords[id].status = TS_TOUCH;
			dish_touch_data->coords[id].x = x;
			dish_touch_data->coords[id].y = y;
			dish_touch_data->coords[id].w = w;
			dish_touch_data->touch_num++;
		} else {
	...
	ts_debug("touch num %d:%d", dish_touch_data->touch_num,
		 touch_data->touch_num);
}

#define RT_GESTURE_FLAG 0x01
#define RT_GESTURE_PACKAGE_SIZE 9
#define GESTURE_HEADER_SIZE 5
#define GESTURE_PACKAGE_SIZE 12

static int goodix_parse_rt_gesture(struct goodix_ts_event *ts_event, u8 *buffer,
				   u8 count)
{
	int ret;
	struct goodix_touch_data *touch_data;

	ts_debug("rt gesture data %*ph", RT_GESTURE_PACKAGE_SIZE + 2, buffer);
	ret = checksum_cmp(buffer, count * RT_GESTURE_PACKAGE_SIZE + 2,
			   CHECKSUM_MODE_U8_LE);
	if (ret) {
		ts_err("rt gesture data checksum error %d",
		       count * RT_GESTURE_PACKAGE_SIZE + 2);
		ts_err("data:%*ph", count * RT_GESTURE_PACKAGE_SIZE + 2,
		       buffer);
		return -EINVAL;
	}
	...
	while (count--) {
		if ((buffer[0] & 0x0F) == POINT_TYPE_DISH)
			touch_data = &ts_event->dish_touch_data;
		else
			touch_data = &ts_event->touch_data;

		if (touch_data->gesture_type)
			ts_err("support only one gesture in the same area");

		touch_data->is_rt_gesture = 1;
		touch_data->gesture_type = buffer[2];
		/* direction/distance */
		touch_data->gesture_value[0] = buffer[3] | (buffer[4] << 8);
		/* angle [0-360) */
		touch_data->gesture_value[1] = buffer[5] | (buffer[6] << 8);
		/* speed/Reserved */
		touch_data->gesture_value[2] = buffer[7] | (buffer[8] << 8);
		buffer += RT_GESTURE_PACKAGE_SIZE;
		ts_info("rt_gst %d [%d, %d, %d] id[%d, %d]",
			touch_data->gesture_type, touch_data->gesture_value[0],
			touch_data->gesture_value[1],
			touch_data->gesture_value[2], (buffer[1] >> 4) & 0x0F,
			buffer[1] & 0x0F);
	}
	return 0;
}

static int goodix_parse_gesture(struct goodix_ts_event *ts_event, u8 *buffer,
				u8 count)
{
	int ret;
	struct goodix_touch_data *touch_data;

	ts_debug("hand up gesture data %*ph", 8, buffer);
	ret = checksum_cmp(buffer, GESTURE_HEADER_SIZE, CHECKSUM_MODE_U8_LE);
	if (ret) {
		ts_err("gesture header checksum error");
		ts_err("data:%*ph", GESTURE_HEADER_SIZE, buffer);
		return -EINVAL;
	}

	buffer += GESTURE_HEADER_SIZE;
	ret = checksum_cmp(buffer, count * GESTURE_PACKAGE_SIZE + 2,
			   CHECKSUM_MODE_U8_LE);
	if (ret) {
		ts_err("gesture data checksum error");
		ts_err("data:%*ph", count * GESTURE_PACKAGE_SIZE + 2, buffer);
		return -EINVAL;
	}
	ts_event->event_type |= EVENT_GESTURE;
	while (count--) {
		...
		touch_data->is_rt_gesture = 0;
		touch_data->gesture_type = buffer[1];
		/* gesture point 0 start X coor */
		touch_data->gesture_points[0] = buffer[4] | (buffer[5] << 8);
		/* gesture point 0 start Y coor */
		touch_data->gesture_points[1] = buffer[6] | (buffer[7] << 8);
		/* gesture point 0 stop X coor */
		touch_data->gesture_points[2] = buffer[8] | (buffer[9] << 8);
		/* gesture point 0 stop Y coor */
		touch_data->gesture_points[3] = buffer[10] | (buffer[11] << 8);
		buffer += GESTURE_PACKAGE_SIZE;
		ts_info("up_gst %d [%d, %d, %d, %d]", touch_data->gesture_type,
			touch_data->gesture_points[0],
			touch_data->gesture_points[1],
			touch_data->gesture_points[2],
			touch_data->gesture_points[3]);
	}
	return 0;
}
...
{
	int offset, ret;
	u8 touch_num = 0, rt_gst_num = 0, gst_num = 0;

	ts_debug("event header %*ph", 8, buffer);
	...
	offset = IRQ_EVENT_HEAD_LEN;
	if (buffer[0] & GOODIX_TOUCH_EVENT) {
		/* finger info */
		ts_event->event_type |= EVENT_TOUCH;
		if (touch_num > 0) {
			ret = checksum_cmp(&buffer[offset],
			...
			goodix_parse_finger(ts_event, buffer, touch_num);
			offset += touch_num * BYTES_PER_POINT + 2;
		}
	}
	if (buffer[3] & RT_GESTURE_FLAG) {
		rt_gst_num = (buffer[3] >> 4) & 0x0F;
		if (rt_gst_num > GOODIX_MAX_GESTURE_NUM) {
			ts_err("rt gesture num exceed limit %d > %d",
			       rt_gst_num, GOODIX_MAX_GESTURE_NUM);
			goto exit;
		}
		goodix_parse_rt_gesture(ts_event, &buffer[offset], rt_gst_num);
		offset += rt_gst_num * RT_GESTURE_PACKAGE_SIZE + 2;
	}
	if (buffer[0] & GOODIX_GESTURE_EVENT) {
		gst_num = buffer[offset + 1] & 0xF;
		if (gst_num > GOODIX_MAX_GESTURE_NUM) {
			ts_err("gesture num exceed limit %d > %d", gst_num,
			       GOODIX_MAX_GESTURE_NUM);
			goto exit;
		}
		goodix_parse_gesture(ts_event, &buffer[offset], gst_num);
	}
...
/* read all data once a time */
#define MAX_EVENT_BUFFER_SIZE 80
static int brl_event_handler(struct goodix_ts_core *cd,
			 struct goodix_ts_event *ts_event)
{
	struct goodix_ts_hw_ops *hw_ops = cd->hw_ops;
	struct goodix_ic_info_misc *misc = &cd->ic_info.misc;
	u8 event_buf[MAX_EVENT_BUFFER_SIZE];
	u8 event_status;
	int ret;

	ret = hw_ops->read(cd, misc->touch_data_addr, event_buf,
	...
	if (checksum_cmp(event_buf, IRQ_EVENT_HEAD_LEN, CHECKSUM_MODE_U8_LE)) {
	...
	event_status = event_buf[0];
	if (event_status & (GOODIX_TOUCH_EVENT | GOODIX_GESTURE_EVENT)) {
		goodix_touch_handler(cd, ts_event, event_buf,
				     MAX_EVENT_BUFFER_SIZE);
	} else if (event_status & GOODIX_REQUEST_EVENT) {
		ts_event->event_type |= EVENT_REQUEST;
		if (event_buf[2] == BRL_REQUEST_CODE_CONFIG)
			ts_event->request_code = REQUEST_TYPE_CONFIG;
		else if (event_buf[2] == BRL_REQUEST_CODE_RESET)
			ts_event->request_code = REQUEST_TYPE_RESET;
		else
			ts_debug("unsupported request code 0x%x", event_buf[2]);
	}
		...
		struct ts_rawdata_info *info)
{
	...
	memcpy((u8 *)(info->buff + info->used_size), cur_ptr + 8, tx * rx * 2);
