...
static void qpnp_lpg_reset_peer_ramps(struct qpnp_lpg_channel *lpg)
{
	struct qpnp_lpg_chip *lpg_chip = lpg->chip;
	struct qpnp_lpg_channel *peer_lpg;
	struct pwm_device *peer_pwm;
	int i, lpg_idx;

	if (!lpg_chip->lpg_group)
		return;

	for (i = 0; i < lpg_chip->num_lpgs; i++) {
		if (lpg_chip->lpg_group[i] == 0)
			break;

		lpg_idx = lpg_chip->lpg_group[i] - 1;
		peer_lpg = &lpg_chip->lpgs[lpg_idx];
		peer_pwm = &lpg_chip->pwm_chip.pwms[lpg_idx];

		if (peer_lpg != lpg && pwm_get_output_type(peer_pwm) == PWM_OUTPUT_MODULATED
					&& pwm_is_enabled(peer_pwm)) {
			qpnp_lpg_set_ramp_config(peer_lpg);
		}
	}
}

static int qpnp_lpg_set_ramp_config_and_reset_peers(struct qpnp_lpg_channel *lpg)
{
	unsigned long flags;
	int rc;

	/*
	 * Set ramp config and reset peer ramps with preemption and IRQs
	 * disabled so that peer channels are as synchronized as possible.
	 */
	spin_lock_irqsave(&lpg->chip->ramp_config_lock, flags);
	rc = qpnp_lpg_set_ramp_config(lpg);
	qpnp_lpg_reset_peer_ramps(lpg);
	spin_unlock_irqrestore(&lpg->chip->ramp_config_lock, flags);
		...
		struct pwm_device *pwm, enum pwm_output_type output_type)
{
	...
	if (src_sel == LUT_PATTERN) {
		...
		rc = qpnp_lpg_set_ramp_config_and_reset_peers(lpg);
		if (rc < 0) {
	...
	struct pwm_device *pwm, struct pwm_output_pattern *output_pattern)
{
	...
	memcpy(lpg->ramp_config.pattern, percentages,
			output_pattern->num_entries * sizeof(u32));
...
static int qpnp_lpg_probe(struct platform_device *pdev)
{
	...
	spin_lock_init(&chip->ramp_config_lock);
	chip->dev = &pdev->dev;
