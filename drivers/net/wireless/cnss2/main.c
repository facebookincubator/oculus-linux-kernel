...
static u8 cnss_get_board_id_override(struct cnss_plat_data *plat_priv)
{
	u8 id;

	if (!of_property_read_u8(plat_priv->plat_dev->dev.of_node,
				 "qcom,board-id-override", &id))
		return id;
...
#ifdef CONFIG_CNSS_META_BOARD_ID_OVERRIDE
	plat_priv->board_id_override = cnss_get_board_id_override(plat_priv);
#endif
