...
#define CLK_GENERATOR_GPIO		"qcom,enable-clk-gen-gpio"
#define CLK_GENERATOR_ACTIVE		"clk_generator_active"
...
#define CLK_GENERATOR_DELAY		2000
...
int cnss_get_pinctrl(struct cnss_plat_data *plat_priv)
{
	...
	if (of_find_property(dev->of_node, CLK_GENERATOR_GPIO, NULL)) {
		pinctrl_info->clk_generator_active =
			pinctrl_lookup_state(pinctrl_info->pinctrl,
					     CLK_GENERATOR_ACTIVE);
		if (IS_ERR_OR_NULL(pinctrl_info->clk_generator_active)) {
			ret = PTR_ERR(pinctrl_info->clk_generator_active);
			cnss_pr_err("Failed to get clk_generator active state, err = %d\n",
				    ret);
			goto out;
		}
	}
		...
		if (!IS_ERR_OR_NULL(pinctrl_info->clk_generator_active)) {
			ret = pinctrl_select_state
				(pinctrl_info->pinctrl,
				 pinctrl_info->clk_generator_active);
			if (ret) {
				cnss_pr_err("Failed to select clk_generator active state, err = %d\n",
					    ret);
				goto out;
			}
			udelay(CLK_GENERATOR_DELAY);
		}
		if (!IS_ERR_OR_NULL(pinctrl_info->sol_default)) {
			ret = pinctrl_select_state
				(pinctrl_info->pinctrl,
