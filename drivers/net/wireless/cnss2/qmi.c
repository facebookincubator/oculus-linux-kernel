...
int cnss_wlfw_tgt_cap_send_sync(struct cnss_plat_data *plat_priv)
{
	...
	if (resp->dev_mem_info_valid) {
		for (i = 0; i < QMI_WLFW_MAX_DEV_MEM_NUM_V01; i++) {
			...
			cnss_pr_vdbg("Device memory info[%d]: start = 0x%llx, size = 0x%llx\n",
...
static int cnss_get_bdf_file_name(struct cnss_plat_data *plat_priv,
				  ...
				  u32 filename_len)
{
	const char *cal_file_prefix = "";
	char filename_tmp[MAX_FIRMWARE_NAME_LEN];
	int ret = 0;

	of_property_read_string(plat_priv->plat_dev->dev.of_node, "cal-file-prefix", &cal_file_prefix);
		...
		if (plat_priv->board_info.board_id == 0xFF) {
			if (plat_priv->chip_info.chip_id & CHIP_ID_GF_MASK)
				snprintf(filename_tmp, filename_len,
					 "%s" ELF_BDF_FILE_NAME_GF, cal_file_prefix);
			else
				snprintf(filename_tmp, filename_len,
					 "%s" ELF_BDF_FILE_NAME, cal_file_prefix);
