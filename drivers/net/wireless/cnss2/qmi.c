				  ...
				  u32 filename_len)
{
	...
	case CNSS_BDF_ELF:
		...
		if (plat_priv->board_name) {
			snprintf(filename_tmp, filename_len, "%s_%s.elf",
				 BDF_FILE_NAME_PREFIX, plat_priv->board_name);
			break;
		}
#endif
...
#ifdef CONFIG_CNSS_META_BOARD_ID_OVERRIDE
		if (plat_priv->board_name) {
			snprintf(filename_tmp, filename_len, "%s_%s.bin",
				 BDF_FILE_NAME_PREFIX, plat_priv->board_name);
			break;
		}
#endif
