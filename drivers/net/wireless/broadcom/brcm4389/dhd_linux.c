...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	uint nd_ra_filter = 0;
#if defined(XRAPI) && defined(ROAM_ENABLE)
	uint roamvar = 0;
#endif /* XRAPI */
...
#define CUSTOM_INTR_WIDTH 100
	...
	if (dhd->up) {
		if (value && dhd->in_suspend) {
				...
				/*
				 * Enable magic filter
				 * Only allow remote wake up packet to send up
				 */
				dhd_xrapi_enable_wake_pkt_filter(dhd, TRUE);
...
#if defined(DHD_USE_EARLYSUSPEND) || defined(DHD_USE_PM_SLEEP)
#if defined(XRAPI) && defined(ROAM_ENABLE)
				/* Susepnd: Enable built-in roaming while host is suspend */
				roamvar = 0;
...
#if defined(XRAPI) && defined(DHD_MAGIC_PKT_FILTER)
				/* disable pkt filter */
				dhd_xrapi_enable_wake_pkt_filter(dhd, FALSE);
#endif /* XRAPI && DHD_MAGIC_PKT_FILTER */
...
#endif /* PASS_ALL_MCAST_PKTS */
#if defined(DHD_USE_EARLYSUSPEND) || defined(DHD_USE_PM_SLEEP)
#if defined(XRAPI) && defined(ROAM_ENABLE) && !defined(DFSPROXY_P2P)
				/*
				 * Resume: Disable built-in roaming,
				 * supplicant to take care of roaming
				 */
				roamvar = 1;
				ret = dhd_iovar(dhd, 0, "roam_off", (char *)&roamvar,
						sizeof(roamvar), NULL, 0, TRUE);
...
#endif /* XRAPI && ROAM_ENABLE && !DFSPROXY_P2P */
#ifdef NDO_CONFIG_SUPPORT
				if (dhd->ndo_enable) {
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if (dhd_xrapi_qflushlog_attach(&dhd->pub) != BCME_OK) {
		DHD_ERROR(("dhd_xrapi_qflushlog_attach has failed\n"));
		goto fail;
	}
...
#if defined(XRAPI) && !defined(DFSPROXY_P2P)
	/* Disable built-in roaming to allowed ext supplicant to take care of roaming */
	roamvar = 1;
...
#endif /* PKT_FILTER_SUPPORT */
#if defined(XRAPI) && defined(DHD_MAGIC_PKT_FILTER)
	/* Install magic filter */
	if (dhd_xrapi_install_wake_pkt_filter(dhd)) {
		DHD_ERROR(("%s wake_pkt_filter install failed\n", __FUNCTION__));
	}
#endif /* XRAPI && DHD_MAGIC_PKT_FILTER */
...
#if defined(XRAPI) && defined(QFLUSH_LOG)
	(void)dhd_xrapi_qflushlog_detach(dhdp);
#endif /* XRAPI  && QFLUSH_LOG */
...
#if defined(XRAPI) && defined(DHD_MAGIC_PKT_FILTER)
		/* disable pkt filter */
		dhd_xrapi_enable_wake_pkt_filter(dhdp, FALSE);
#endif /* XRAPI && DHD_MAGIC_PKT_FILTER */
	...
	const char *iovar, bool address_first)
{
	...
	while (copied < count) {
		size_t data_len = count - copied, rounded_data_len;
		if (data_len > sizeof(iovar_buf.data)) {
			data_len = sizeof(iovar_buf.data);
		}
		rounded_data_len = data_len + sizeof(uint32) - 1;
		rounded_data_len -= rounded_data_len % sizeof(uint32);
		iovar_buf.p1 = address_first ? (uint32)(*position) : (uint32)data_len;
		iovar_buf.p2 = address_first ? (uint32)data_len : (uint32)(*position);
		if (user_buffer_in) {
			/* SET operation */
			if (copy_from_user(iovar_buf.data, user_buffer_in + copied, data_len)) {
				return -EPERM;
			}
			/* This params/plen of NULL/0 is a 'legal fiction', imposed by
			 * strange assert in dhd_bus_iovar_op(). After this strange
			 ...
			 * inside iovar handler!
			 */
			err = dhd_bus_iovar_op(&(dhd->pub), iovar, NULL, 0, (char *)&iovar_buf,
				sizeof(iovar_buf) - sizeof(iovar_buf.data) + rounded_data_len,
				IOV_SET);
		} else {
			/* GET operation */
			 ...
			 */
			err = dhd_bus_iovar_op(&(dhd->pub), iovar, (char *)&iovar_buf,
				sizeof(iovar_buf) - sizeof(iovar_buf.data),
				(char *)iovar_buf.data,
				(rounded_data_len < (2 * sizeof(uint32))) ?
				(2 * sizeof(uint32)) : rounded_data_len, IOV_GET);
		}
		if (err) {
			return -EPERM;
		}
		*position += data_len;
		if (user_buffer_out) {
			if (copy_to_user(user_buffer_out + copied, iovar_buf.data, data_len)) {
				return -EPERM;
			}
		}
		copied += data_len;
