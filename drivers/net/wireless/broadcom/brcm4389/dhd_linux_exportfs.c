...
static ssize_t
show_sssr_support(struct dhd_info *dev, char *buf)
{
	ssize_t ret = 0;
	unsigned long onoff;

	onoff = support_sssr_dump;
	ret = scnprintf(buf, PAGE_SIZE - 1, "%lu \n",
		onoff);
	return ret;
}
...
static ssize_t
sssr_support_onoff(struct dhd_info *dev, const char *buf, size_t count)
{
	unsigned long onoff;
...
#ifdef DHD_SSSR_DUMP
static struct dhd_attr dhd_attr_sssr_support =
	__ATTR(sssr_support, 0660, show_sssr_support, sssr_support_onoff);
#endif /* DHD_SSSR_DUMP */
