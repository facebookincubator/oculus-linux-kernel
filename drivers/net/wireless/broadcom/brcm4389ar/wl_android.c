...
static int wl_android_request_tsync(struct net_device *dev, char *command,
				    int total_len);
static int wl_android_set_nextwakeup(struct net_device *dev, char *command,
				     int total_len);
...
static int wl_android_simulate_degraded_performance(struct net_device *dev,
						    char *command);
	...
	/* CMD_AMPDU_MPDU */
...
#endif /* WL_DUAL_STA */
#if defined(TSF_GSYNC)
	else if (strnicmp(command, CMD_XRAPI_SEND_PROBERESP,
			  strlen(CMD_XRAPI_SEND_PROBERESP)) == 0) {
		bytes_written = wl_android_xrapi_send_proberesp(
			net, command, priv_cmd.total_len);
	} else if (strnicmp(command, CMD_XRAPI_REQUEST_TSYNC,
			    strlen(CMD_XRAPI_REQUEST_TSYNC)) == 0) {
		bytes_written = wl_android_request_tsync(net, command,
							 priv_cmd.total_len);
	} else if (strnicmp(command, CMD_XRAPI_SET_NEXTWAKE,
			    strlen(CMD_XRAPI_SET_NEXTWAKE)) == 0) {
		bytes_written = wl_android_set_nextwakeup(net, command,
							  priv_cmd.total_len);
	} else if (strnicmp(command, CMD_XRAPI_TXDONE_INDICATION,
			    strlen(CMD_XRAPI_TXDONE_INDICATION)) == 0) {
		bytes_written = wl_android_send_txdone_indication(
			net, command, priv_cmd.total_len);
	} else if (strnicmp(command, CMD_XRAPI_GET_STATS,
			    strlen(CMD_XRAPI_GET_STATS)) == 0) {
		bytes_written = wl_android_get_xrapi_stats(net);
	} else if (strnicmp(command, CMD_XRAPI_SET_PSMODE,
		...
		bytes_written = wl_android_request_ps_mode_change(
			net, command, priv_cmd.total_len);
			  ...
			  strlen(CMD_XRAPI_SIMULATE_DEGRADED_PERFORMANCE)) ==
		 0) {
		bytes_written =
			wl_android_simulate_degraded_performance(net, command);
	}
#endif /* SIMULATE_DEGRADED_PERFORMANCE */
	else if (strnicmp(command, CMD_SETWSECINFO, strlen(CMD_SETWSECINFO)) ==
		 0) {
...
#if defined(XRAPI_IN_USER_SPACE) && defined(XRAPI)
/*
 * Send WL_XRAPI_CMD_SEND_PROBERESP cmd to notify vendor fw to
 * send a probe response frame so that both ap & sta TSF synced.
 */
static int wl_android_xrapi_send_proberesp(struct net_device *dev,
					   char *command, int total_length)
{
	...
	WL_INFORM(("WL_XRAPI_CMD_SEND_PROBERESP received\n"));
	...
	iov_buf->id = WL_XRAPI_CMD_SEND_PROBERESP;
	WL_INFORM(("wl_android_get_assoclist: ENTER\n"));
	/* Check if there is an associated STA or not */
	assoc_maclist->count = htod32(MAX_NUM_OF_ASSOCLIST);
	res = wldev_ioctl_get(dev, WLC_GET_ASSOCLIST, assoc_maclist,
			      sizeof(mac_buf));
	if (res < 0) {
		WL_ERR(("Fail to get assoc list: %d\n", res));
		goto exit;
	...
	if (assoc_maclist->count != 1) {
		...
		WL_INFORM(("Associated client=" MACDBG "\n",
			   MAC2STRDBG(assoc_maclist->ea[0].octet)));
	}

	buflen = buflen_start = WLC_IOCTL_SMLEN - sizeof(bcm_iov_buf_t);
	pxtlv = (uint8 *)&iov_buf->data[0];
	res = bcm_pack_xtlv_entry(
		&pxtlv, &buflen, WL_XRAPI_XTLV_MACADDR, ETHER_ADDR_LEN,
		(uint8 *)(struct ether_addr *)assoc_maclist->ea[0].octet,
	...
	res = wldev_iovar_setbuf(dev, "xrapi", iov_buf, iovlen, iov_resp,
				 WLC_IOCTL_MAXLEN, NULL);
	if (res != BCME_OK) {
		WL_ERR(("Fail to send xrapi WL_XRAPI_CMD_SEND_PROBERESP %d\n",
			res));
		goto exit;
	}
	WL_INFORM(("WL_XRAPI_CMD_SEND_PROBERESP sent\n"));
...
exit:
	...
	bytes_written = snprintf(command, total_length, "STATUS %d", res);
	if (unlikely(bytes_written > total_length))
		WL_ERR(("WL_XRAPI_CMD_SEND_PROBERESP return status Error, bytes_written=%d, total_len=%d",
...
#if defined(TSF_GSYNC)
/*
 * send request_sync cmd to request FW to send GPIO interrupt and TSF timestamp.
 */
static int wl_android_request_tsync(struct net_device *dev, char *command,
				    int total_len)
{
	...
	/* fill header */
	iov_buf->version = WL_XRAPI_IOV_VERSION_1_1;
	iov_buf->id = WL_XRAPI_CMD_REQUEST_TSYNC;
	buflen = buflen_start = WLC_IOCTL_SMLEN - sizeof(bcm_iov_buf_t);
	pxtlv = (uint8 *)&iov_buf->data[0];
	res = bcm_pack_xtlv_entry(&pxtlv, &buflen, WL_XRAPI_XTLV_TS_REQ_ID,
				  sizeof(req_id), (uint8 *)&req_id,
				  BCM_XTLV_OPTION_ALIGN32);
	if (res != BCME_OK) {
		WL_ERR(("bcm pack err %d\n", res));
		goto exit;
	...
	res = wldev_iovar_setbuf(dev, "xrapi", iov_buf, iovlen, iov_resp,
				 WLC_IOCTL_MAXLEN, NULL);
	if (res != BCME_OK) {
		WL_ERR(("Fail to send WL_XRAPI_CMD_REQUEST_TSYNC %d\n", res));
		goto exit;
	}
	WL_DBG(("WL_XRAPI_CMD_REQUEST_TSYNC sent"));
...
exit:
	...
	if (unlikely(bytes_written > total_len))
		WL_ERR(("WL_XRAPI_CMD_REQUEST_TSYNC return status Error, bytes_written=%d, total_len=%d",
...
static int wl_set_nextwakeup(struct net_device *dev, wl_tsf_t *wakeup_tsf)
{
	...
	res = wldev_iovar_setbuf(dev, "xrapi", iov_buf, iovlen, iov_resp,
				 WLC_IOCTL_MAXLEN, NULL);
	if (res != BCME_OK) {
		WL_ERR(("Fail to send xrapi WL_XRAPI_CMD_NEXTWAKE %d\n", res));
		goto exit;
	}
	WL_DBG(("WL_XRAPI_CMD_NEXTWAKE sent\n"));
...
static int wl_send_txdone_indication(struct net_device *dev)
{
	...
	/* fill header */
	iov_buf->version = WL_XRAPI_IOV_VERSION_1_1;
	iov_buf->id = WL_XRAPI_CMD_TXDONE_INDICATION;
	iov_buf->len = 0;
	iovlen = sizeof(bcm_iov_buf_t) + iov_buf->len;
	res = wldev_iovar_setbuf(dev, "xrapi", iov_buf, iovlen, iov_resp,
				 WLC_IOCTL_MAXLEN, NULL);
	if (res != BCME_OK) {
		WL_ERR(("Fail to send WL_XRAPI_CMD_TXDONE_INDICATION %d\n",
			res));
		goto exit;
	}
	WL_DBG(("WL_XRAPI_CMD_TXDONE_INDICATION sent\n"));
...
static int wl_android_get_xrapi_stats(struct net_device *dev)
{
	...
	memset_s(iov_resp, WLC_IOCTL_MAXLEN, 0, WLC_IOCTL_MAXLEN);

	/* fill header */
	iov_buf->version = WL_XRAPI_IOV_VERSION_1_1;
	iov_buf->id = WL_XRAPI_CMD_STATS;
	buflen = buflen_start = WLC_IOCTL_SMLEN - sizeof(bcm_iov_buf_t);
	pxtlv = (uint8 *)&iov_buf->data[0];
	res = bcm_pack_xtlv_entry(&pxtlv, &buflen, WL_XRAPI_XTLV_STATS,
				  sizeof(req_id), (uint8 *)&req_id,
				  BCM_XTLV_OPTION_ALIGN32);
	if (res != BCME_OK) {
		WL_ERR(("bcm pack err %d\n", res));
	...
	iov_buf->len = buflen_start - buflen;
	iovlen = sizeof(bcm_iov_buf_t) + iov_buf->len;
	res = wldev_iovar_getbuf(dev, "xrapi", iov_buf, iovlen, iov_resp,
				 WLC_IOCTL_MAXLEN, NULL);
	if (res != BCME_OK) {
	...
	if (iov_resp) {
		xrap_stats = (wl_xrapi_stats_t *)iov_resp;
		WL_INFORM((
			"XRAPI STATS cycle %d Total sleep dur %d Total sleep req dur %d\n",
			xrap_stats->cnt, xrap_stats->sleep_time,
			xrap_stats->sleep_req_time));
		WL_INFORM(("XRAPI STATS number of hist %d\n",
			   xrap_stats->hist_idx));
	}
...
#if defined(TSF_GSYNC)
int wl_android_xrapi_notify_time_sync_event(struct net_device *dev,
					    const void *event_data, int len)
{
	...
	wl_cfgvendor_send_async_event(
		wiphy, dev, BRCM_VENDOR_EVENT_XRAPI_TIME_SYNC, event_data, len);
	return res;
}
#endif // TSF_GSYNC

int wl_android_xrapi_notify_psmode_event(struct net_device *dev,
					 const void *event_data, int len)
{
	int res = BCME_OK;
	struct bcm_cfg80211 *cfg;
	...
	WL_ERR(("BRCM_VENDOR_EVENT_XRAPI_PSMODE event sent %d\n",
		NL80211_CMD_VENDOR));

	wl_cfgvendor_send_async_event(
		wiphy, dev, BRCM_VENDOR_EVENT_XRAPI_PSMODE, event_data, len);
	return res;
}
...
#ifdef SIMULATE_DEGRADED_PERFORMANCE
/*
 * Set the amount of delay that should happen in the TX path to simulate degraded performance
 */
static int wl_android_simulate_degraded_performance(struct net_device *dev,
						    char *command)
{
	...
	delay_us = (u32)bcm_strtoul(token, NULL, 10);
	dhdp->performance_delay_us = delay_us;
exit:
	WL_DBG(("WL_XRAPI_CMD_SIMULATE_DEGRADED_PERFORMANCE delay %d, res %d\n",
		delay_us, res));
	return res;
}
#endif /* SIMULATE_DEGRADED_PERFORMANCE */
...
static int wl_android_set_tx_chain(struct net_device *dev, char *command)
{
	...
	WL_DBG(("set tx chain cmd received: %s\n", command));
