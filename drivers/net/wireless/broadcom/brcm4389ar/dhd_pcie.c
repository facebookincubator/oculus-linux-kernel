...
dhdpcie_hybridfw_download(struct dhd_bus *bus, char *fp)
{
	...
	fsize = fwpkg_get_firmware_img_size(&bus->fwpkg);
	...
	/* Save the size of the downloaded RAM image */
	bus->fw_download_len = dnglfw_sz;
...
dhdpcie_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	 ...
	 * other errors
	 */
	bcmerror = dhdpcie_hybridfw_download(bus, filep);
	if (bcmerror != BCME_UNSUPPORTED) {
		goto err;
	}
...
BCMFASTPATH(dhd_bus_txdata)(struct dhd_bus *bus, void *txp, uint8 ifidx)
{
	...
	OSL_DELAY(bus->dhd->performance_delay_us);
                ...
                void *params, uint plen, void *arg, uint len, int val_size)
{
	...
	case IOV_SVAL(IOV_HYBRIDFW):
		{
			FWPKG_FILE *fp;
			fwpkg_info_t *fwpkg = &bus->fwpkg;
			bcmerror = fwpkg_init(fwpkg, params);
			if (bcmerror == BCME_ERROR) {
				break;
			}
			bcmerror = fwpkg_open_firmware_img(fwpkg, params, &fp);
			if (bcmerror == BCME_ERROR) {
				break;
			}
		...
		if (si_setcore(bus->sih, ARMCA7_CORE_ID, 0)) {
			...
			if (bus->hostfw_buf.va) {
				...
				dhdpcie_download_rtlv(bus,
					DNGL_RTLV_TYPE_HOST_PAGE_LOCATION,
					sizeof(host_location),
					(uint8*)&host_location);
...
#ifdef DHD_FWTRACE
			{
				...
				if (fwtrace_init(bus->dhd) == BCME_OK) {
					...
					fwtrace_get_haddr(bus->dhd, &host_buf_info);

					dhdpcie_download_rtlv(bus,
						DNGL_RTLV_HOST_FWTRACE_BUF_LOCATION,
						sizeof(host_buf_info), (uint8*)&host_buf_info);
				}
			}
#endif /* DHD_FWTRACE */
#endif /* BCMINTERNAL */
