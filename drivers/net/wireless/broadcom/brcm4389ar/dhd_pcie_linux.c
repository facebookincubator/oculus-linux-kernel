...
#if defined(DHD_DEBUG)
	uint32_t suspend_start_ms = 0, suspend_end_ms = 0;
#endif
...
#if defined(DHD_DEBUG)
	suspend_start_ms = OSL_SYSUPTIME();
#endif
...
#if defined(DHD_DEBUG)
	suspend_end_ms = OSL_SYSUPTIME();
	DHD_INFO(("Time taken for Suspend is: %d msec\n",
		  (suspend_end_ms - suspend_start_ms)));
#endif
...
static int dhdpcie_pci_resume(struct device *dev)
{
	...
	unsigned long flags;
#if defined(DHD_DEBUG)
	uint32_t resume_start_ms = 0, resume_end_ms = 0;
#endif
...
#if defined(DHD_DEBUG)
	resume_end_ms = OSL_SYSUPTIME();
	DHD_INFO(("Time taken for resume is: %d msec\n",
		(resume_end_ms - resume_start_ms)));
#endif
...
int dhdpcie_init(struct pci_dev *pdev)
{
	...
	do {
		...
		/* Set the PCIe DMA mask to 64 bit to avoid overflows */
		if (pci_set_dma_mask(pdev, DMA_BIT_MASK(64)) ||
			pci_set_consistent_dma_mask(pdev,
				DMA_BIT_MASK(64))) {
			DHD_ERROR(("%s: DMA set %d bit mask failed.\n",
