	...
	enum nl80211_channel_type channel_type)
{
	...
	if (wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP &&
		...
		if (chan->band == wl_get_nl80211_band(CHSPEC_BAND(*sta_chanspec))) {
			chspec = (
/* XRAPI support 5G/6G SoftAP only, allow non-PSC SoftAP bring up for APSTA cases */
#if defined(XRAPI) && defined(FORBID_NON_PSC)
#ifdef WL_6G_BAND
...
#else
#if defined(XRAPI) && defined(DFSPROXY_SAP)
				/* Dongle will take care of this case */
#else
