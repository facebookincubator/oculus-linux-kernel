...
/*
 * Driver for Marvell Discovery (MV643XX) and Marvell unknown5 ethernet ports
...
static const struct of_device_id mv643xx_eth_shared_ids[] = {
	{ .compatible = "marvell,unknown5-eth", },
				   ...
				   int phy_addr)
{
	...
	/* Attempt to connect to the PHY using unknown5-mdio */
	phydev = ERR_PTR(-ENODEV);
	for (i = 0; i < num; i++) {
		int addr = (start + i) & 0x1f;

		snprintf(phy_id, sizeof(phy_id), PHY_ID_FMT,
				"unknown5-mdio-mii", addr);
