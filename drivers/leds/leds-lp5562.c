...
/* If the module parameter is true, skip lp5562 device initialization and
 * keep the device state from bootloader.
 */
static bool keep_state = false;
module_param(keep_state, bool, 0444);
...
static ssize_t lp5562_engine_id_show(struct device *dev,
...
static ssize_t lp5562_engine_id_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t len)
{
	struct lp55xx_led *led = i2c_get_clientdata(to_i2c_client(dev));
...
static ssize_t lp5562_store_program_data(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t len)
{
	struct lp55xx_led *led = i2c_get_clientdata(to_i2c_client(dev));
	...
	lp5562_load_engine(chip);
	lp5562_update_firmware(chip, buf, len);

	return len;
}

static ssize_t lp5562_enable_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct lp55xx_led *led = i2c_get_clientdata(to_i2c_client(dev));
	struct lp55xx_chip *chip = led->chip;
	...
	ret = lp55xx_read(chip, LP5562_REG_ENABLE, &val);
	if (ret)
		return ret;

	return sprintf(buf, "%d\n", !!(val & LP5562_MASTER_ENABLE));
}

static ssize_t lp5562_enable_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t len)
{
	struct lp55xx_led *led = i2c_get_clientdata(to_i2c_client(dev));
	...
	if (val) {
		...
		lp55xx_write(chip, LP5562_REG_ENABLE, LP5562_DISABLE);
	}
...
static LP55XX_DEV_ATTR_RW(engine_id, lp5562_engine_id_show,
			  lp5562_engine_id_store);
static LP55XX_DEV_ATTR_WO(program_data, lp5562_store_program_data);
static LP55XX_DEV_ATTR_RW(enable, lp5562_enable_show,
			  lp5562_enable_store);

static struct attribute *lp5562_attributes[] = {
	&dev_attr_led_pattern.attr,
	&dev_attr_engine_mux.attr,
	&dev_attr_engine_id.attr,
	&dev_attr_program_data.attr,
...
static void lp5562_shutdown(struct i2c_client *client)
{
	...
	struct lp55xx_chip *chip = led->chip;
	lp55xx_write(chip, chip->cfg->reset.addr, chip->cfg->reset.val);
};
