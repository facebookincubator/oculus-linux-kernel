...
struct led_setting {
	...
	bool			blink;
	bool			breath;
	struct pwm_output_pattern breath_pattern;
	struct pwm_output_pattern *blink_pattern;
};
...
#define BREATH_STEP_NS 200000000ULL
static const u8 breath_duty_percent[] = {0, 0, 0, 14, 28, 42, 56, 70, 84, 100, 100, 84, 70, 56, 42, 28, 14, 0, 0, 0};
				...
				struct pwm_setting *pwm)
{
	...
	if (led->led_setting.breath) {
		...
		pstate.output_pattern = &led->led_setting.breath_pattern;
	} else if (led->led_setting.blink) {
		pstate.enabled = true;
		pstate.output_type = PWM_OUTPUT_MODULATED;
		pstate.output_pattern = led->led_setting.blink_pattern;
	} else {
...
static int __tri_led_set(struct qpnp_led_dev *led)
{
	...
	if (led->pwm_setting.duty_ns != 0 || led->led_setting.breath || led->led_setting.blink)
		val = mask;
	else
		val = 0;
...
static int qpnp_tri_led_set(struct qpnp_led_dev *led)
{
	...
	if (!led->led_setting.blink && !led->led_setting.breath) {
	...
	led->blinking = led->led_setting.blink;
	led->breathing = led->led_setting.breath;

	return rc;
}

static int qpnp_tri_led_update_blink_pattern(struct qpnp_led_dev *led)
{
	struct pwm_output_pattern *blink_pattern;
	u64 *duty_pattern;
	int i, num_on_entries, num_off_entries, rc = 0;
	u64 on_ns, off_ns, entry_ns, period_ns;

	on_ns = led->led_setting.on_ms * NSEC_PER_MSEC;
	off_ns = led->led_setting.off_ms * NSEC_PER_MSEC;
	entry_ns = div_u64(on_ns + off_ns, BLINK_LUT_ENTRIES);

	num_on_entries = div64_u64(on_ns + div_u64(entry_ns, 2), entry_ns);
	num_off_entries = BLINK_LUT_ENTRIES - num_on_entries;

	/*
	 * Allocate a new blink pattern, since pwm_apply_state will return early without
	 * doing anything if the output_pattern pointer is unchanged.
	 */
	blink_pattern = devm_kmalloc(led->chip->dev, sizeof(*blink_pattern), GFP_KERNEL);
	if (!blink_pattern)
		return -ENOMEM;
	duty_pattern = devm_kcalloc(led->chip->dev, BLINK_LUT_ENTRIES, sizeof(*duty_pattern), GFP_KERNEL);
	if (!duty_pattern)
		return -ENOMEM;

	period_ns = led->pwm_setting.pre_period_ns;
	for (i = 0; i < num_on_entries; i++)
		duty_pattern[i] = div_u64(period_ns * led->cdev.brightness, 255);

	blink_pattern->duty_pattern = duty_pattern;
	blink_pattern->num_entries = BLINK_LUT_ENTRIES;
	blink_pattern->cycles_per_duty = div64_u64(entry_ns, period_ns);

	if (led->led_setting.blink_pattern) {
		if (led->led_setting.blink_pattern->duty_pattern)
			devm_kfree(led->chip->dev, led->led_setting.blink_pattern->duty_pattern);

		devm_kfree(led->chip->dev, led->led_setting.blink_pattern);
	}
	led->led_setting.blink_pattern = blink_pattern;
		...
		enum led_brightness brightness)
{
	...
	if (brightness && brightness == led->led_setting.brightness && !led->breathing)
		goto unlock;

	led->led_setting.brightness = brightness;
	led->cdev.brightness = led->led_setting.brightness;

	/* leds-class.txt says we should cancel blinking if brightness is set to 0 */
	if (!brightness)
		led->led_setting.blink = false;
	led->led_setting.breath = false;

	if (led->blinking) {
		rc = qpnp_tri_led_update_blink_pattern(led);
		if (rc) {
			dev_err(led->chip->dev, "Update blink pattern brightness failed for %s, rc=%d\n",
					led->label, rc);
			goto unlock;
		}
	}
		...
		unsigned long *on_ms, unsigned long *off_ms)
{
	...
	if (*on_ms == 0) {
		...
		led->led_setting.blink = led->cdev.brightness != LED_OFF;
		led->led_setting.breath = false;
	}

	rc = qpnp_tri_led_update_blink_pattern(led);
	if (rc) {
		dev_err(led->chip->dev, "Update blink pattern failed for %s, rc=%d\n",
				led->label, rc);
		goto unlock;
	}
...
static int qpnp_tri_led_register(struct qpnp_tri_led_chip *chip)
{
	...
	for (i = 0; i < chip->num_leds; i++) {
		...
		if (pwm_get_output_type_supported(led->pwm_dev)
			...
			struct pwm_output_pattern *breath_pattern = &led->led_setting.breath_pattern;
			u64 period_ns = led->pwm_setting.pre_period_ns;

			breath_pattern->duty_pattern = devm_kmalloc(chip->dev,
				ARRAY_SIZE(breath_duty_percent) * sizeof(*breath_pattern->duty_pattern), GFP_KERNEL);
			if (!breath_pattern->duty_pattern) {
				rc = -ENOMEM;
				goto err_out;
			}

			breath_pattern->num_entries = ARRAY_SIZE(breath_duty_percent);
			breath_pattern->cycles_per_duty = div64_u64(BREATH_STEP_NS, period_ns);

			for (j = 0; j < breath_pattern->num_entries; j++)
				breath_pattern->duty_pattern[j] = div_u64(period_ns * breath_duty_percent[j], 255);
