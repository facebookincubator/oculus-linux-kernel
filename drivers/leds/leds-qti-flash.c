...
#define FLASH_LED_CURRENT_DERATE_EN		0x6D
				...
				struct device_node *node)
{
	...
	multi_strobe = of_property_read_bool(node, "qcom,multi-strobe");
	rc = qti_flash_led_masked_write(led, FLASH_LED_MULTI_STROBE_CTRL,
				FLASH_LED_FLASH_ONCE_ONLY, multi_strobe ? 0 : 1);
	if (rc < 0) {
		pr_err("Failed to %s multi-strobe support rc=%d\n", multi_strobe ? "enable" : "disable", rc);
		return rc;
	}

	rc = of_property_read_u8(node, "qcom,cur-derate-en-val", &val);
	if (rc == 0) {
...
static int qti_flash_led_probe(struct platform_device *pdev)
{
	...
	rc = qti_flash_led_setup(led, node);
	if (rc < 0) {
