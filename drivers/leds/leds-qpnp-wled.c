...
extern int get_panel_name(char *panel_name);
#endif
...
static int qpnp_wled_parse_dt(struct qpnp_wled *wled)
{
	...
	rc = get_panel_name(panel_name);
	pr_info("%s: panel_name: %s\n", __func__, panel_name);

	if (!rc) {
		/* dsi panel cfg not present */
		pr_warn("%s: %d: Panel name not present\n", __func__, __LINE__);
	}
#endif
...
#ifdef CONFIG_VS1_BOARD
		rc = of_property_read_string(pdev->dev.of_node,
				"qcom,panel-0-name", &temp_str);
		if (!rc) {
			rc = of_property_read_u32(pdev->dev.of_node,
					"qcom,panel-0-avdd-target-voltage-mv",
					&temp_val);
			if (!rc) {
				if (strcmp(temp_str, panel_name) == 0)
					wled->avdd_target_voltage_mv = temp_val;
			} else if (rc != -EINVAL) {
				dev_err(&pdev->dev,
						"Unable to read avdd target voltage\n");
				return rc;
			}
		} else if (rc != -EINVAL) {
			dev_err(&pdev->dev, "Unable to read panel 0 name\n");
			return rc;
		}

		rc = of_property_read_string(pdev->dev.of_node,
				"qcom,panel-1-name", &temp_str);
		if (!rc) {
			rc = of_property_read_u32(pdev->dev.of_node,
					"qcom,panel-1-avdd-target-voltage-mv",
					&temp_val);
			if (!rc) {
				if (strcmp(temp_str, panel_name) == 0)
					wled->avdd_target_voltage_mv = temp_val;
			...
			dev_err(&pdev->dev, "Unable to read panel 1 name\n");
			return rc;
		}
#endif
		rc = of_property_read_u32(pdev->dev.of_node,
