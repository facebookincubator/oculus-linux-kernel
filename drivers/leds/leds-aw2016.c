				...
				struct device_attribute *attr, char *buf)
{
	...
	struct aw2016_led *led =
		container_of(led_cdev, struct aw2016_led, cdev);
	/* imax has 4 level:
	 * 0x00:15mA  0x01:30mA  0x10:5mA 0x11:10mA
	 * led_current: 0 ~ 15
	 * led output current = imax * led_current / 15 (mA)
	 * when PWM is 255
	 */
				 ...
				 size_t len)
{
	...
	if (rc != 2) {
		dev_err(&led->client->dev, "invalid input: %s\n", buf);
		return len;
	}

	mutex_lock(&led->pdata->led->lock);
	led->pdata->imax = (imax > MAX_IMAX_VAL) ? MAX_IMAX_VAL : imax;
