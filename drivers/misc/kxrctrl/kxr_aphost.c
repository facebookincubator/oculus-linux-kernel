...
static int kxr_aphost_init_gpios(struct kxr_aphost *aphost)
{
	...
	aphost->gpio_dfu = devm_gpiod_get_from_of_node(dev, dev->of_node,
			"nordic,nordic-dfu", 0, GPIOD_OUT_HIGH, "nordic-dfu");
...
static int kxr_aphost_driver_remove(struct spi_device *spi)
{
