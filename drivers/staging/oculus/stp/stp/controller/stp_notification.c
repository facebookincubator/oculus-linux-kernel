...
void stp_controller_set_notification(uint8_t channel, uint32_t notification)
{
    ...
    if (_stp_controller_data->channels[channel].pending_tx_notification != STP_IN_NONE)
        STP_LOG_ERROR("STP controller overwrite notif 0x%zx with 0x%zx",
                      (size_t)_stp_controller_data->channels[channel].pending_tx_notification,
                      (size_t)notification);

    _stp_controller_data->channels[channel].pending_tx_notification = notification;
...
void stp_controller_prepare_tx_notification(bool *do_transaction)
{
    ...
    *do_transaction = false;
    ...
    int channel_index = stp_get_channel_with_notification(_stp_controller_data->channels);
    if (channel_index != -1)
    {
        stp_prepare_tx_notification_packet(
            (uint8_t)channel_index,
            _stp_controller_data->tx_buffer,
            _stp_controller_data->channels[channel_index].pending_tx_notification);

        *do_transaction = true;

        _stp_controller_data->channels[channel_index].pending_tx_notification = STP_IN_NONE;
    }
