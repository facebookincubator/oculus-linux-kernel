...
static struct stp_master_handshake_table stella_handshake_table = {
...
static int spi_stp_probe(struct spi_device *spi)
{
	...
	struct stp_init_t stp_init = {
		...
		.handshake = &stella_handshake_table,
...
int spi_stp_remove(struct spi_device *spi)
{
	...
	dev_err(&spi->dev, "SPI_STP: spi_stp_remove\n");
...
MODULE_AUTHOR("Eugen Pirvu <epirvu@fb.com>");
