		...
		size_t count, loff_t *f_pos)
{
	...
	while (pending_count > 0) {
		...
		if (missing != 0) {
			pr_err("stp_dev_write: missing = %d\n", (int)missing);
			status = -EFAULT;
			goto error;
		}
		...
		if (real_count != len) {
			pr_err("stp_dev_write: writing error\n");
			status = -EFAULT;
			goto error;
		}
...
static ssize_t stp_start_get(struct device *dev,
...
static ssize_t stp_start_set(struct device *dev,
	struct device_attribute *attr,
...
static DEVICE_ATTR(stp_start, 0660, stp_start_get, stp_start_set);
