...
static int top_panel_get_temp(void *data,
					int *temperature)
{
	...
	struct top_panel_sensor_data *fp = data;
...
static const struct thermal_zone_of_device_ops top_panel_thermal_ops = {
	.get_temp = top_panel_get_temp,
};
...
{
	struct top_panel_sensor_data *fp =
		(struct top_panel_sensor_data *) dev_get_drvdata(dev);
	ssize_t ret;
...
static struct attribute *top_panel_sensor_attrs[] = {
...
ATTRIBUTE_GROUPS(top_panel_sensor);

static int top_panel_sensor_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct top_panel_sensor_data *fp;
	...
	ret = sysfs_create_groups(&pdev->dev.kobj, top_panel_sensor_groups);
	if (ret < 0)
...
static int top_panel_sensor_remove(struct platform_device *pdev)
{
	struct top_panel_sensor_data *fp = platform_get_drvdata(pdev);

	thermal_zone_of_sensor_unregister(&pdev->dev, fp->tzd);

	sysfs_remove_groups(&pdev->dev.kobj, top_panel_sensor_groups);
...
static const struct of_device_id top_panel_sensor_table[] = {
	{ .compatible = "oculus,top-panel-sensor" },
	{}
};
MODULE_DEVICE_TABLE(of, top_panel_sensor_table);

static struct platform_driver top_panel_sensor_driver = {
	.probe = top_panel_sensor_probe,
	.remove = top_panel_sensor_remove,
	.driver = {
		.name = "top-panel-sensor",
		.of_match_table = top_panel_sensor_table,
	},
};

static int __init top_panel_sensor_init(void)
{
	pr_info("%s: Initializing\n", __func__);
	return platform_driver_register(&top_panel_sensor_driver);
}
subsys_initcall(top_panel_sensor_init);

static void __exit top_panel_sensor_deinit(void)
{
	platform_driver_unregister(&top_panel_sensor_driver);
}
module_exit(top_panel_sensor_deinit);

MODULE_ALIAS("top_panel_sensor");
