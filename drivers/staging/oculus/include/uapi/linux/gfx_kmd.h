...
enum gfx_kmd_event_type {
	GFX_KMD_EVENT_UNKNOWN,
	GFX_KMD_EVENT_LINKDOWN,
	GFX_KMD_EVENT_LINKUP,
};

// Events themsevles, wrapped in case we need extra context.
struct __packed gfx_kmd_event {
	enum gfx_kmd_event_type type;
};

// Increase this if more event types are added.
#define GFX_KMD_EVENT_BATCH_MAX 5

// Allows libgfx to fetch multiple events from the queue.
// Cuts on syscall roundtrips.
struct __packed gfx_kmd_event_batch {
	uint16_t size;
	struct gfx_kmd_event events[GFX_KMD_EVENT_BATCH_MAX];
};
...
// there is no control queue as in ARFW to send them outside of
