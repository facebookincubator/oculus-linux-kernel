...
struct swd_mcu_data {
	...
	/* Firmware to flash loaded from userspace */
	const u8 *fw_data;
	u32 fw_size;
...
struct swd_dev_data {
	...
	/* Whether manual FW loading via userspace is enabled */
	bool manual_fw_loading;

	/* Whether the DAM reset quirk using the dam_ctrl driver is enabled */
	bool dam_reset_quirk;

	/* Handle for the DAM control device */
	struct platform_device *dam_ctrl_pdev;
};
