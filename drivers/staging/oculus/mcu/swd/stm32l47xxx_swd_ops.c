...
static int stm32l47xxx_swd_get_errors(struct device *dev)
{
	...
	if (devid != DEVID_STM32L47XXX && devid != DEVID_STM32L44XXX) {
		dev_err(dev, "(stm324l7xxx SWD) Unrecognized devid: 0x%x", devid);
		return -EINVAL;
	}
...
int stm32l47xxx_swd_prepare(struct device *dev)
{
	...
	/* Verify if it's already unlocked */
...
int stm32l47xxx_swd_erase_app(struct device *dev)
{
	...
	swd_memory_write(dev, SWD_STM32L47XXX_FLASH_CR, SWD_STM32L47XXX_FLASH_CR_LOCK);
...
int stm32l47xxx_swd_write_chunk(struct device *dev, int addr, const u8 *data, size_t len)
{
	...
	status = stm32l47xxx_swd_prepare(dev);
	if (status != 0)
		return status;
