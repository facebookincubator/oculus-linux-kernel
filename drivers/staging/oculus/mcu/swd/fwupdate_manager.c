...
extern void dam_ctrl_reset(struct device *dev, int val);
...
static int fwupdate_update_write_single_app(struct device *dev, struct swd_mcu_data *mcudata, int bytes_written)
{
	...
	if (mcudata->fw_data) {
		fw_data = mcudata->fw_data;
		fw_size = mcudata->fw_size;
	} else {
		fw_data = mcudata->fw->data;
		fw_size = mcudata->fw->size;
	}
...
static int fwupdate_update_single_app(struct device *dev, struct swd_mcu_data *mcudata, bool erase_all)
{
	...
	dev_info(dev,
		 ...
		 mcudata->target_flavor, mcudata->fw_path, fw_size);
...
void fwupdate_release_all_firmware(struct device *dev)
{
	...
	kfree(devdata->mcu_data.fw_data);
	devdata->mcu_data.fw_data = NULL;
	devdata->mcu_data.fw_size = 0;
...
static int fwupdate_update_firmware(struct device *dev)
{
	...
	if (devdata->dam_reset_quirk) {
		// Connect under reset to ensure the DAM is in a known state
		dam_ctrl_reset(&devdata->dam_ctrl_pdev->dev, 1); // 1 here means active
		msleep(RESET_GPIO_TIME_MS);
	} else if (gpio_is_valid(devdata->gpio_reset)) {
		gpio_direction_output(devdata->gpio_reset, 1); // 1 here means inactive
		msleep(RESET_GPIO_TIME_MS);
	}
	...
	// De-assert reset for the DAM
	if (devdata->dam_reset_quirk) {
		dam_ctrl_reset(&devdata->dam_ctrl_pdev->dev, 0);
		msleep(RESET_GPIO_TIME_MS);
	}
	...
	if (!devdata->dam_reset_quirk && gpio_is_valid(devdata->gpio_reset)) {
...
/**
 * fwupdate_load_firmware_write() - handle a write to the fwupdate_load_firmware file
 * @dev: This device
 * @buf: Data buffer written from userspace. Expected to be part of the firmware image.
 * @off: The offset in the firmware file to write to
 * @count: The number of bytes in buffer
 *
 * Return: @count upon success, else a negative value to indicate an error.
 */
ssize_t fwupdate_load_firmware_write(struct device *dev,
				     ...
				     size_t count)
{
	...
	if (off + count > max_fw_size) {
		dev_err(dev, "Firmware image too large: %lld > %zu",
			off, max_fw_size);
		status = -EINVAL;
		goto error;
	}
	...
	if (off == 0) {
		kfree(mcudata->fw_data);
		mcudata->fw_data = kzalloc(max_fw_size, GFP_KERNEL);
		mcudata->fw_size = 0;
	}

	memcpy((void*)(mcudata->fw_data + off), buf, count);
	mcudata->fw_size += count;
