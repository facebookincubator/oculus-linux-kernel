...
static ssize_t swd_driver_load_firmware_write(struct file *fp, struct kobject *kobj,
					      struct bin_attribute *attr, char *buf,
					      loff_t off, size_t count)
{
	struct device *dev = kobj_to_dev(kobj);

	return fwupdate_load_firmware_write(dev, buf, off, count);
}
...
static int swd_driver_init_dev_data(struct swd_dev_data *devdata, struct device *dev)
{
	...
	struct device_node *dam_node = NULL;
	...
	devdata->manual_fw_loading = of_property_read_bool(node, "oculus,manual-fw-loading");
	devdata->dam_reset_quirk = of_property_read_bool(node, "oculus,dam-reset-quirk");

	if (devdata->dam_reset_quirk) {
		dam_node = of_parse_phandle(node, "oculus,dam-ctrl", 0);
		if (!dam_node) {
			dev_err(dev, "Failed to get the dam-ctrl phandle\n");
			return -ENODEV;
		}
		devdata->dam_ctrl_pdev = of_find_device_by_node(dam_node);
		of_node_put(dam_node);
		if (!devdata->dam_ctrl_pdev) {
			dev_warn(dev, "Failed to get the dam-ctrl device\n");
			return -EPROBE_DEFER;
		}
	}
...
static struct bin_attribute dev_attr_load_firmware = {
	.attr =	{
		.name = "load_firmware",
		.mode = 0660,
	},
	.size = 0,
	.write = swd_driver_load_firmware_write,
};


static struct bin_attribute *swd_bin_attr[] = {
	&dev_attr_load_firmware,
	NULL
};

static const struct attribute_group m_swd_gr = {
	.attrs = swd_attr,
	.bin_attrs = swd_bin_attr
};
