...
static void handle_display_event(struct nsync_dev_data *devdata, const struct syncboss_data *packet)
{
	...
	if (devdata->nsync_irq_timestamp_us <= 0) {
		dev_err_ratelimited(devdata->dev, "Dropping display frame event message received before nsync IRQ\n");
		return;
	}
