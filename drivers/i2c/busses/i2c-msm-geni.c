...
static bool geni_i2c_is_bus_recovery_required(struct geni_i2c_dev *gi2c)
{
	 ...
	 * is pulled high.
	 */
	return (((geni_ios & 1) == 0 || (geni_ios & 2) == 0) &&
					(gi2c->err == -EPROTO ||
					gi2c->err == -EBUSY ||
					gi2c->err == -ETIMEDOUT));
}
			 ...
			 int num)
{
	...
	for (i = 0; i < RECOVERY_TRY_COUNT; i++) {
		geni_ios = geni_read_reg_nolog(gi2c->base, SE_GENI_IOS);
		if ((geni_ios & 0x3) != 0x3) { //SCL:b'1, SDA:b'0
			I2C_LOG_ERR(
				gi2c->ipcl, true, gi2c->dev,
				"try %d: IO lines in bad state (SCL %d, SDA %d)"
				", Power the slave n: %d addr: 0x%x\n",
				i + 1, geni_ios & 2, geni_ios & 1, num,
				msgs[0].addr);

			// Re-use one of valid error codes to enable recovery
			gi2c->err = -EPROTO;
			...
			if (ret)
				...
				// Recovery success, not enabled or not required
				break;
		} else
			// Idle state is good
			break;
	};
	// If all recovery attempts failed, we will exit anyway with -ENXIO
	...
	if ((geni_ios & 0x3) != 0x3) { //SCL:b'1, SDA:b'0
		I2C_LOG_ERR(gi2c->ipcl, true, gi2c->dev,
			"IO lines in bad state (SCL %d, SDA %d),"
			" Power the slave n: %d addr: 0x%x\n",
			geni_ios & 2, geni_ios & 1, num, msgs[0].addr);
		/* for levm skip auto suspend timer */
		if (!gi2c->is_le_vm) {
			pm_runtime_mark_last_busy(gi2c->dev);
			pm_runtime_put_autosuspend(gi2c->dev);
	...
	for (i = 0; i < num; i++) {
		...
		if (!timeout) {
			...
			I2C_LOG_ERR(gi2c->ipcl, true, gi2c->dev,
				"I2C xfer timeout: %d, n:%d addr: 0x%x\n",
				gi2c->xfer_timeout, num, msgs[0].addr);
			geni_i2c_err(gi2c, GENI_TIMEOUT);
		...
		if (gi2c->err) {
			I2C_LOG_ERR(gi2c->ipcl, true, gi2c->dev,
				"i2c error :%d  n: %d  addr: %x\n",
				gi2c->err, num, msgs[0].addr);
