	...
	if (dma_rx_status) {
		...
		if (dma_rx_status & UART_DMA_RX_FRAMING_ERR) {
			...
			geni_se_dump_dbg_regs(&msm_port->serial_rsc, uport->membase,
					      msm_port->ipc_log_misc);
			drop_rx = true;
			// META smartglasses Begin: Print framing errors to better track any MCU related issues.
			dev_err(uport->dev, "%s:dma_rx_status & UART_DMA_RX_FRAMING_ERR\n", __func__);
			// META smartglasses End: Print framing errors to better track any MCU related issues.
		}
		...
		if (dma_rx_status & UART_DMA_RX_BREAK) {
			...
			geni_se_dump_dbg_regs(&msm_port->serial_rsc, uport->membase,
					      msm_port->ipc_log_misc);
			drop_rx = true;
			// META smartglasses Begin: Print break errors to better track any MCU related issues.
			dev_err(uport->dev, "%s:dma_rx_status & UART_DMA_RX_BREAK\n", __func__);
			// META smartglasses End: Print break errors to better track any MCU related issues.
		}
...
static void msm_geni_serial_shutdown(struct uart_port *uport)
{
	...
	// META smartglasses End: Track serial port shutdown.
	...
	if (!uart_console(uport)) {
		...
		if (pm_runtime_enabled(uport->dev)) {
			...
			/*
			 * Release shutdown's own power_on ref and force
			 * synchronous suspend. This ensures runtime_suspend
			 * runs (stop_rx, clocks off) before the next startup,
			 * preventing stale DMA state that causes framing errors.
			 */
			ret = pm_runtime_put_sync_suspend(uport->dev);
			usage_count = atomic_read(&uport->dev->power.usage_count);
			BUG_ON(usage_count < 0);
...
static int msm_geni_serial_startup(struct uart_port *uport)
{
	...
	// META smartglasses Begin: Track serial port startup.
	dev_err(uport->dev, "%s:%s:%s\n", __func__, msm_port->name, uport->name);
	// META smartglasses End: Track serial port startup.
...
static int msm_geni_serial_probe(struct platform_device *pdev)
{
	...
	fallback_hs = of_property_read_bool(pdev->dev.of_node, "meta,fallback_hs");

	// If the console is disabled and the fallback_hs flag is set,
	// the console device will function as an HS serial device.
	if (is_console && !con_enabled && fallback_hs) {
		is_console = false;
		drv = &msm_geni_serial_hs_driver;
		dev_info(&pdev->dev, "%s: Console disabled. Fallback to hs serial", __func__);
	}
