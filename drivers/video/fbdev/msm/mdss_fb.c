...
#if !defined(CONFIG_MDSS_FB_NUM)
#if defined(CONFIG_FB_MSM_TRIPLE_BUFFER)
#define CONFIG_MDSS_FB_NUM 3
#else
#define CONFIG_MDSS_FB_NUM 2
#endif
#endif
...
static int mdss_fb_probe(struct platform_device *pdev)
{
	...
	mfd->fb_page = CONFIG_MDSS_FB_NUM;
...
static bool check_not_supported_ioctl(u32 cmd)
{
	return((cmd == MSMFB_OVERLAY_SET) || (cmd == MSMFB_OVERLAY_UNSET) ||
		...
		(cmd == MSMFB_OVERLAY_PLAY) || (cmd == MSMFB_BUFFER_SYNC) ||
		(cmd == MSMFB_OVERLAY_QUEUE) || (cmd == MSMFB_NOTIFY_UPDATE));
}
