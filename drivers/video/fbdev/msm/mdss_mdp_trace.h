...
TRACE_EVENT(mdp_overlay_handle_vsync,
	TP_PROTO(u64 vsync_ticks),
	TP_ARGS(vsync_ticks),
	TP_STRUCT__entry(
			__field(u64, vsync_ticks)
	),
	TP_fast_assign(
			__entry->vsync_ticks = vsync_ticks;
	),
	TP_printk("vsync_ticks:%llu",
			__entry->vsync_ticks)
);

TRACE_EVENT(mdp_overlay_handle_lineptr,
	TP_PROTO(u64 lineptr_ticks, int lineptr_value),
	TP_ARGS(lineptr_ticks, lineptr_value),
	TP_STRUCT__entry(
			__field(u64, lineptr_ticks)
			__field(int, lineptr_value)
	),
	TP_fast_assign(
			__entry->lineptr_ticks = lineptr_ticks;
			__entry->lineptr_value = lineptr_value;
	),
	TP_printk("lineptr_ticks:%llu [line #%d]",
