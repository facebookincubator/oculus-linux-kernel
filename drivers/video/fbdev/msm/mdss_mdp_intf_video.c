...
static inline u32 mdss_mdp_video_frame_count(struct mdss_mdp_ctl *ctl)
{
	struct mdss_mdp_video_ctx *ctx;
	u32 frame_cnt = 0;

	if (!ctl || !ctl->intf_ctx[MASTER_CTX])
		goto frame_count_exit;
	ctx = ctl->intf_ctx[MASTER_CTX];
	frame_cnt = mdp_video_read(ctx, MDSS_MDP_REG_INTF_FRAME_COUNT);
frame_count_exit:
	return frame_cnt;
}
...
int mdss_mdp_video_start(struct mdss_mdp_ctl *ctl)
{
	...
	ctl->ops.read_frame_cnt_fnc = mdss_mdp_video_frame_count;
