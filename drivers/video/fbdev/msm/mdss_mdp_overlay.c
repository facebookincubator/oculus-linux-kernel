						...
						ktime_t t)
{
	...
	trace_mdp_overlay_handle_vsync(ktime_to_ns(t));

	mdp5_data->vsync_time = t;
	sysfs_notify_dirent(mdp5_data->vsync_event_sd);

	if (mdp5_data->freerun_histogram_enable) {
		mdata = mdp5_data->mdata;

		spin_lock(&mdata->hist_intr.lock);
		mask = readl_relaxed(
			mdata->mdp_base + MDSS_MDP_REG_HIST_INTR_EN);
		for (i = 0; i < mdata->ndspp; i++)
			mask |= (1 << (4 * i + 12));
		writel_relaxed(
			mask, mdata->mdp_base + MDSS_MDP_REG_HIST_INTR_EN);

		if (ctl->mixer_left) {
			mixer = mdata->mixer_intf + ctl->mixer_left->num;
			/* PA_DSPP_BLOCK_REG_OFF + PA_HIST_REG_OFF */
			base_addr = mixer->dspp_base + 0x804;
			writel_relaxed(0, base_addr);
		}
		if (ctl->mixer_right) {
			mixer = mdata->mixer_intf + ctl->mixer_right->num;
			/* PA_DSPP_BLOCK_REG_OFF + PA_HIST_REG_OFF */
			base_addr = mixer->dspp_base + 0x804;
			writel_relaxed(0, base_addr);
		}
						...
						ktime_t t)
{
	...
	trace_mdp_overlay_handle_lineptr(
		ktime_to_ns(t), ctl->mfd->panel_info->te.wr_ptr_irq);
...
int mdss_mdp_overlay_vsync_ctrl(struct msm_fb_data_type *mfd, int en)
{
	...
	/*
	 * XXX: VR compositor relies on vsync_event timestamps so we must
	 * keep this enabled
	 */
	if (!en)
		return 0;
...
static ssize_t mdss_mdp_freerun_hist_show_event(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
	struct mdss_overlay_private *mdp5_data = mfd_to_mdp5_data(mfd);
	struct mdss_data_type *mdata = mdp5_data->mdata;
	struct mdss_mdp_mixer *mixer;
	u32 dspp_num;
	int i, bin;
	char __iomem *base_addr;

	u32 expected_sum = 0, sum = 0, bin_value = 0;
	u32 hist_cnt, mixer_id[MDSS_MDP_INTF_MAX_LAYERMIXER];

	struct mdss_mdp_freerun_histogram *hist =
		(struct mdss_mdp_freerun_histogram *)buf;

	hist_cnt = mdss_mdp_get_ctl_mixers(mfd->index, mixer_id);
	for (i = 0; i < hist_cnt; i++) {
		dspp_num = mixer_id[i];
		mixer = mdata->mixer_intf + dspp_num;
		expected_sum += (mdata->mixer_intf[dspp_num].width *
				 mdata->mixer_intf[dspp_num].height);

		/* PA_DSPP_BLOCK_REG_OFF + PA_HIST_RAM_REG_OFF */
		base_addr = mixer->dspp_base + 0xC00;
		for (bin = 0; bin < HIST_V_SIZE; bin++) {
			/* HIST_DATA_MASK */
			bin_value = readl_relaxed(base_addr) & 0xFFFFFF;
			hist->histogram[bin] = bin_value +
				((i > 0) ? hist->histogram[bin] : 0);
			sum += bin_value;
			base_addr += 0x4;
		}
	}

	hist->event_time = ktime_to_ns(mdp5_data->hist_time);
	hist->expected_sum = expected_sum;
	hist->sum = sum;

	pr_debug("fb%d histogram=%llu\n", mfd->index, hist->event_time);

	return sizeof(struct mdss_mdp_freerun_histogram);
}

static ssize_t mdss_mdp_freerun_hist_set_value(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
	...
	struct mdp_histogram_start_req start_req = {
		.block = MDP_LOGICAL_BLOCK_DISP_0 + mfd->index,
		.frame_cnt = 1,
		.num_bins = HIST_V_SIZE,
	};

	ret = kstrtoint(buf, 10, &freerun);
	if (ret) {
		pr_err("Invalid input for freerun\n");
		return -EINVAL;
	}

	if (freerun) {
		pr_debug("mdss_mdp_hist_start\n");
		mdss_mdp_hist_stop(start_req.block);
		mdss_mdp_hist_start(&start_req);

		/* Kick off the histogram start ASAP */
		mutex_lock(&mfd->mdp_sync_pt_data.sync_mutex);
		mfd->msm_fb_backup.info = *fbi;
		...
		pr_debug("mdss_mdp_hist_stop\n");
		mdss_mdp_hist_stop(start_req.block);
	}
	mdp5_data->freerun_histogram_enable = freerun;
...
static ssize_t mdss_mdp_frame_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
	...
	current_frame =  ctl->ops.read_frame_cnt_fnc(ctl);
	ret = scnprintf(buf, PAGE_SIZE, "%d\n", current_frame);
	return ret;
}

static ssize_t mdss_mdp_line_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
...
static ssize_t mdss_mdp_partial_vsync_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
	...
	int current_frame, current_line, vtotal, partial_vsync;
	int ret;

	ktime_t t;
	u64 current_time;

	if (!ctl->ops.read_frame_cnt_fnc || !ctl->ops.read_line_cnt_fnc)
		return -EINVAL;
	...
	vtotal = mdss_panel_get_vtotal(pinfo);

	t = ktime_get();
	current_frame = ctl->ops.read_frame_cnt_fnc(ctl);
	current_line = ctl->ops.read_line_cnt_fnc(ctl);
	current_time = ktime_to_ns(t);

	if (current_line >= vtotal || current_line < 0 || current_frame < 0)
		return -EINVAL;

	/* Multiplier is 10000 to match the %04d below. */
	partial_vsync = (current_line * 10000) / vtotal;
	ret = scnprintf(buf, PAGE_SIZE, "%d.%04d@t=%llu\n",
		current_frame, partial_vsync, current_time);
	return ret;
}
...
static DEVICE_ATTR(freerun_histogram, S_IRUGO | S_IWUSR | S_IWGRP,
	mdss_mdp_freerun_hist_show_event, mdss_mdp_freerun_hist_set_value);
...
static DEVICE_ATTR(mdp_frame, S_IRUGO, mdss_mdp_frame_show, NULL);
static DEVICE_ATTR(mdp_line, S_IRUGO, mdss_mdp_line_show, NULL);
static DEVICE_ATTR(partial_vsync, S_IRUGO, mdss_mdp_partial_vsync_show, NULL);
...
static struct attribute *mdp_overlay_sysfs_attrs[] = {
	...
	&dev_attr_freerun_histogram.attr,
	&dev_attr_bl_event.attr,
	&dev_attr_ad_event.attr,
	&dev_attr_ad_bl_event.attr,
	&dev_attr_mdp_frame.attr,
	&dev_attr_mdp_line.attr,
	&dev_attr_partial_vsync.attr,
	NULL,
};
				...
				void __user *argp)
{
	...
	case MSMFB_HISTOGRAM_START:
		...
		if (mdp5_data && mdp5_data->freerun_histogram_enable)
			break;
