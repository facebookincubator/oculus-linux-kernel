...
static struct pp_hist_col_info *dspp_hist;

static DEFINE_MUTEX(mdss_pp_mutex);
static struct mdss_pp_res_type *mdss_pp_res_left;
static struct mdss_pp_res_type *mdss_pp_res_right;
...
static int pp_ppb_setup(struct mdss_mdp_mixer *mixer,
				struct mdss_pp_res_type *mdss_pp_res_ptr);
				...
				struct mdss_pp_res_type *mdss_pp_res_ptr)
{
	...
	if (disp_num < MDSS_BLOCK_DISP_NUM)
		flags = mdss_pp_res_ptr->pp_disp_flags[disp_num];
	else
		flags = 0;
	...
	pp_sts = &mdss_pp_res_ptr->pp_disp_sts[disp_num];
	...
	if (flags & PP_FLAGS_DIRTY_ARGC) {
		if (mdata->pp_block_off.lm_pgc_off == U32_MAX) {
			...
			pp_ops[GC].pp_set_config(addr, pp_sts,
			   &mdss_pp_res_ptr->argc_disp_cfg[disp_num], LM);
		}
...

{
	...
	if (disp_num < MDSS_BLOCK_DISP_NUM) {
		...
		if (pp_program_mask & PP_PROGRAM_HIST) {
			ret = pp_hist_setup(&opmode, MDSS_PP_DSPP_CFG |
					dspp_num, mixer, pp_sts);
			if (ret)
				goto dspp_exit;
		}
		flags = mdss_pp_res_ptr->pp_disp_flags[disp_num];
	...
	if ((flags & PP_FLAGS_DIRTY_PA) &&
		...
		pp_ops[PA].pp_set_config(base, pp_sts,
				&mdss_pp_res_ptr->pa_v2_disp_cfg[disp_num],
				DSPP);
	}
	if ((flags & PP_FLAGS_DIRTY_PCC) &&
		...
		if (mdata->pp_block_off.dspp_pcc_off == U32_MAX) {
			...
			pp_ops[PCC].pp_set_config(addr, pp_sts,
				&mdss_pp_res_ptr->pcc_disp_cfg[disp_num],
				DSPP);
		}
	}
	...
	if ((flags & PP_FLAGS_DIRTY_IGC) &&
		...
		mdss_pp_res_ptr->igc_disp_cfg[disp_num].block = dspp_num;
		pp_ops[IGC].pp_set_config(addr, pp_sts,
			&mdss_pp_res_ptr->igc_disp_cfg[disp_num],
			DSPP);
	...
	if ((flags & PP_FLAGS_DIRTY_ENHIST) &&
		...
		/* Pass dspp num using block */
		mdss_pp_res_ptr->enhist_disp_cfg[disp_num].block = dspp_num;
		pp_ops[HIST_LUT].pp_set_config(base, pp_sts,
			&mdss_pp_res_ptr->enhist_disp_cfg[disp_num], DSPP);
	}
	...
	if ((flags & PP_FLAGS_DIRTY_DITHER) &&
		...
		pp_ops[DITHER].pp_set_config(addr, pp_sts,
			&mdss_pp_res_ptr->dither_disp_cfg[disp_num], DSPP);
	}
	if ((flags & PP_FLAGS_DIRTY_GAMUT) &&
		...
		if (mdata->pp_block_off.dspp_gamut_off == U32_MAX) {
			...
			addr = base + mdata->pp_block_off.dspp_gamut_off;
			pp_ops[GAMUT].pp_set_config(addr, pp_sts,
				&mdss_pp_res_ptr->gamut_disp_cfg[disp_num],
				DSPP);
		}
	}
	...
	if ((flags & PP_FLAGS_DIRTY_PGC) &&
		...
		pgc_config = &mdss_pp_res_ptr->pgc_disp_cfg[disp_num];
		if (mdata->pp_block_off.dspp_pgc_off == U32_MAX) {
			...
			pp_ops[GC].pp_set_config(addr, pp_sts,
				&mdss_pp_res_ptr->pgc_disp_cfg[disp_num],
				DSPP);
		}
	}
	if (flags & PP_FLAGS_DIRTY_PA_DITHER &&
		...
		pp_ops[PA_DITHER].pp_set_config(base, pp_sts,
				&mdss_pp_res_ptr->pa_dither_cfg[disp_num],
				DSPP);
	}
...
int mdss_mdp_pp_setup(struct mdss_mdp_ctl *ctl)
{
	...
	if ((!ctl->mfd) || (!mdss_pp_res_left) || (!mdss_pp_res_right))
		return -EINVAL;
				...
				struct mdss_mdp_pp_program_info *info)
{
	...
	if ((!ctl) || (!ctl->mfd) || (!mdss_pp_res_left) ||
			(!mdss_pp_res_right) || (!ctl->mdata))
		return -EINVAL;
	if (!info) {
	...
	if (info->pp_program_mask & PP_DEFER_PROGRAM_MASK) {
		flags = (disp_num < MDSS_BLOCK_DISP_NUM) ?
				mdss_pp_res_left->pp_disp_flags[disp_num] : 0;
		pa_v2_flags = mdss_pp_res_left->pa_v2_disp_cfg[disp_num].flags;
		max_bw_needed = (IS_PP_RESUME_COMMIT(flags) &&
					(IS_PP_LUT_DIRTY(flags) ||
					IS_SIX_ZONE_DIRTY(flags, pa_v2_flags)));

		flags = (disp_num < MDSS_BLOCK_DISP_NUM) ?
				mdss_pp_res_right->pp_disp_flags[disp_num] : 0;
		pa_v2_flags = mdss_pp_res_right->pa_v2_disp_cfg[disp_num].flags;
		max_bw_needed |= (IS_PP_RESUME_COMMIT(flags) &&
					(IS_PP_LUT_DIRTY(flags) ||
					IS_SIX_ZONE_DIRTY(flags, pa_v2_flags)));
	...
	if (ctl->mixer_left &&
	    (mdss_pp_res_left->pp_disp_flags[disp_num] != 0)) {
		if (info->pp_program_mask & PP_DEFER_PROGRAM_MASK) {
			pp_mixer_setup(ctl->mixer_left, mdss_pp_res_left);
			pp_dspp_setup(disp_num, ctl->mixer_left,
				info->pp_program_mask, &info->pp_opmode_left,
				mdss_pp_res_left);
			pp_ppb_setup(ctl->mixer_left, mdss_pp_res_left);
		} else {
			pp_dspp_setup(disp_num, ctl->mixer_left,
				info->pp_program_mask, &info->pp_opmode_left,
				mdss_pp_res_left);
	...
	if (ctl->mixer_right &&
	    (mdss_pp_res_right->pp_disp_flags[disp_num] != 0)) {
		if (info->pp_program_mask & PP_DEFER_PROGRAM_MASK) {
			pp_mixer_setup(ctl->mixer_right, mdss_pp_res_right);
			pp_dspp_setup(disp_num, ctl->mixer_right,
				info->pp_program_mask, &info->pp_opmode_right,
				mdss_pp_res_right);
			pp_ppb_setup(ctl->mixer_right, mdss_pp_res_right);
		} else {
			pp_dspp_setup(disp_num, ctl->mixer_right,
				info->pp_program_mask, &info->pp_opmode_right,
				mdss_pp_res_right);
	...
	if (info->pp_program_mask & PP_DEFER_PROGRAM_MASK) {
		...
		if (IS_PP_RESUME_COMMIT(flags)) {
			mdss_pp_res_left->pp_disp_flags[disp_num] &=
				~PP_FLAGS_RESUME_COMMIT;
			mdss_pp_res_right->pp_disp_flags[disp_num] &=
				~PP_FLAGS_RESUME_COMMIT;
		}
	}
 ...
 * reconfigured
 */
void mdss_pp_res_set_bits(struct msm_fb_data_type *mfd,
	struct mdss_pp_res_type *mdss_pp_res_ptr)
{
	u32 flags = 0;
	u32 disp_num = mfd->index;
	struct pp_sts_type pp_sts = mdss_pp_res_ptr->pp_disp_sts[disp_num];
	struct mdp_pa_v2_cfg_data *pa_v2_cache_cfg = NULL;

	if (pp_sts.pa_sts & PP_STS_ENABLE) {
		flags |= PP_FLAGS_DIRTY_PA;
		pa_v2_cache_cfg = &mdss_pp_res_ptr->pa_v2_disp_cfg[disp_num];
	...
	if (pp_sts.pcc_sts & PP_STS_ENABLE) {
		...
		if (!(mdss_pp_res_ptr->pcc_disp_cfg[disp_num].ops
					& MDP_PP_OPS_DISABLE))
			mdss_pp_res_ptr->pcc_disp_cfg[disp_num].ops |=
				MDP_PP_OPS_WRITE;
	}
	if (pp_sts.igc_sts & PP_STS_ENABLE) {
		flags |= PP_FLAGS_DIRTY_IGC;
		if (!(mdss_pp_res_ptr->igc_disp_cfg[disp_num].ops
					& MDP_PP_OPS_DISABLE))
			mdss_pp_res_ptr->igc_disp_cfg[disp_num].ops |=
				MDP_PP_OPS_WRITE;
	}
	if (pp_sts.argc_sts & PP_STS_ENABLE) {
		flags |= PP_FLAGS_DIRTY_ARGC;
		if (!(mdss_pp_res_ptr->argc_disp_cfg[disp_num].flags
					& MDP_PP_OPS_DISABLE))
			mdss_pp_res_ptr->argc_disp_cfg[disp_num].flags |=
				MDP_PP_OPS_WRITE;
	}
	if (pp_sts.enhist_sts & PP_STS_ENABLE) {
		flags |= PP_FLAGS_DIRTY_ENHIST;
		if (!(mdss_pp_res_ptr->enhist_disp_cfg[disp_num].ops
					& MDP_PP_OPS_DISABLE))
			mdss_pp_res_ptr->enhist_disp_cfg[disp_num].ops |=
				MDP_PP_OPS_WRITE;
	}
	if (pp_sts.dither_sts & PP_STS_ENABLE) {
		flags |= PP_FLAGS_DIRTY_DITHER;
		if (!(mdss_pp_res_ptr->dither_disp_cfg[disp_num].flags
					& MDP_PP_OPS_DISABLE))
			mdss_pp_res_ptr->dither_disp_cfg[disp_num].flags |=
				MDP_PP_OPS_WRITE;
	}
	if (pp_sts.gamut_sts & PP_STS_ENABLE) {
		flags |= PP_FLAGS_DIRTY_GAMUT;
		if (!(mdss_pp_res_ptr->gamut_disp_cfg[disp_num].flags
					& MDP_PP_OPS_DISABLE))
			mdss_pp_res_ptr->gamut_disp_cfg[disp_num].flags |=
				MDP_PP_OPS_WRITE;
	}
	if (pp_sts.pgc_sts & PP_STS_ENABLE) {
		flags |= PP_FLAGS_DIRTY_PGC;
		if (!(mdss_pp_res_ptr->pgc_disp_cfg[disp_num].flags
					& MDP_PP_OPS_DISABLE))
			mdss_pp_res_ptr->pgc_disp_cfg[disp_num].flags |=
				MDP_PP_OPS_WRITE;
	}
	if (pp_sts.pa_dither_sts & PP_STS_ENABLE) {
		flags |= PP_FLAGS_DIRTY_PA_DITHER;
		if (!(mdss_pp_res_ptr->pa_dither_cfg[disp_num].flags
			& MDP_PP_OPS_DISABLE))
			mdss_pp_res_ptr->pa_dither_cfg[disp_num].flags |=
				MDP_PP_OPS_WRITE;
	}
	mdss_pp_res_ptr->pp_disp_flags[disp_num] |= flags;
	mdss_pp_res_ptr->pp_disp_flags[disp_num] |= PP_FLAGS_RESUME_COMMIT;
}
...
int mdss_mdp_pp_resume(struct msm_fb_data_type *mfd)
{
	...
	mdss_pp_res_set_bits(mfd, mdss_pp_res_left);
	mdss_pp_res_set_bits(mfd, mdss_pp_res_right);
...
int mdss_mdp_pp_init(struct device *dev)
{
	...
	mutex_lock(&mdss_pp_mutex);
	if (!mdss_pp_res_left || !mdss_pp_res_right) {
		mdss_pp_res_left = devm_kzalloc(dev, sizeof(*mdss_pp_res_left),
				GFP_KERNEL);
		mdss_pp_res_right = devm_kzalloc(dev,
				sizeof(*mdss_pp_res_right), GFP_KERNEL);
		if (mdss_pp_res_left == NULL || mdss_pp_res_right == NULL) {
...
hist_exit:
	devm_kfree(dev, hist);
pp_exit:
	devm_kfree(dev, mdss_pp_res_left);
	devm_kfree(dev, mdss_pp_res_right);
	mutex_unlock(&mdss_pp_mutex);
	return ret;
}
...
void mdss_mdp_pp_term(struct device *dev)
{
	...
	if (mdss_pp_res_left || mdss_pp_res_right) {
		...
		devm_kfree(dev, mdss_pp_res_left);
		devm_kfree(dev, mdss_pp_res_right);
		devm_kfree(dev, dspp_hist);
		mdss_pp_res_left = NULL;
		mdss_pp_res_right = NULL;
		dspp_hist = NULL;
		mutex_unlock(&mdss_pp_mutex);
	}
...
pa_clk_off:
		...
		if ((flags & MDSS_PP_SPLIT_MASK) != MDSS_PP_SPLIT_LEFT_ONLY) {
			res_cache.mdss_pp_res = mdss_pp_res_right;
		...
		if ((flags & MDSS_PP_SPLIT_MASK) != MDSS_PP_SPLIT_RIGHT_ONLY) {
			res_cache.mdss_pp_res = mdss_pp_res_left;
			ret = pp_pa_cache_params(config, &res_cache);
			if (ret) {
				pr_err("PA config failed version %d ret %d\n",
					config->version, ret);
...
pcc_clk_off:
		...
		if ((config->ops & MDSS_PP_SPLIT_MASK) !=
		    MDSS_PP_SPLIT_LEFT_ONLY) {
			res_cache.mdss_pp_res = mdss_pp_res_right;
					...
					u32 *copyback, u32 copy_from_kernel)
{
	...
	if (config->ops & MDP_PP_OPS_READ) {
		...
		if (mdata->has_no_lut_read) {
			pr_err("%s, no LUT read available\n",
				__func__);
			goto clock_off;
		}
				...
				u32 *copyback)
{
	...
	case MDSS_PP_LM_CFG:
		 ...
		 */
		if (((mdss_pp_res_left->pp_disp_sts[disp_num].argc_sts &
		     PP_STS_ENABLE) && !(config->flags & MDP_PP_OPS_DISABLE)) ||
		     ((mdss_pp_res_right->pp_disp_sts[disp_num].argc_sts &
		     PP_STS_ENABLE) && !(config->flags & MDP_PP_OPS_DISABLE))) {
...
clock_off:
		...
		if ((config->flags & MDSS_PP_SPLIT_MASK) !=
		    MDSS_PP_SPLIT_LEFT_ONLY) {
			ret = pp_pgc_lut_cache_params(config, mdss_pp_res_right,
				((is_lm) ? LM : DSPP));
			if (ret) {
		...
		if ((config->flags & MDSS_PP_SPLIT_MASK) !=
		    MDSS_PP_SPLIT_RIGHT_ONLY) {
			ret = pp_pgc_lut_cache_params(config, mdss_pp_res_left,
				((is_lm) ? LM : DSPP));
			if (ret) {
				pr_err("pgc cache params failed, ret %d\n",
					ret);
					...
					int copy_from_kernel)
{
	...
	if ((config->flags & MDSS_PP_SPLIT_MASK) !=
	    MDSS_PP_SPLIT_LEFT_ONLY) {
		ret = pp_dither_cache_params(config, mdss_pp_res_right,
				copy_from_kernel);
		if (ret) {
		...
		mdss_pp_res_right->pp_disp_flags[disp_num] |=
				PP_FLAGS_DIRTY_DITHER;
	}
	if ((config->flags & MDSS_PP_SPLIT_MASK) !=
	    MDSS_PP_SPLIT_RIGHT_ONLY) {
		ret = pp_dither_cache_params(config, mdss_pp_res_left,
				copy_from_kernel);
		if (ret) {
			pr_err("dither config failed version %d ret %d\n",
				config->version, ret);
...
gamut_clk_off:
		...
		if ((config->flags & MDSS_PP_SPLIT_MASK) !=
		    MDSS_PP_SPLIT_LEFT_ONLY) {
			ret = pp_gamut_cache_params(config, mdss_pp_res_right);
			if (ret) {
			...
			mdss_pp_res_right->pp_disp_flags[disp_num] |=
					PP_FLAGS_DIRTY_GAMUT;
		}
		if ((config->flags & MDSS_PP_SPLIT_MASK) !=
		    MDSS_PP_SPLIT_RIGHT_ONLY) {
			ret = pp_gamut_cache_params(config, mdss_pp_res_left);
			if (ret) {
				pr_err("gamut config failed version %d ret %d\n",
					config->version, ret);
				ret = -EFAULT;
...
int mdss_mdp_hist_stop(u32 block)
{
	...
	if (PP_LOCAT(block) == MDSS_PP_SSPP_CFG) {
		...
		for (i = 0; i < mdata->ndspp; i++) {
			hist_info = &dspp_hist[i];
			if (disp_num != hist_info->disp_num)
				continue;
			ret = pp_hist_disable(hist_info);
			if (ret)
				goto hist_stop_clk;
			mdss_pp_res_left->pp_disp_flags[i] |=
							PP_FLAGS_DIRTY_HIST_COL;
			mdss_pp_res_right->pp_disp_flags[i] |=
							PP_FLAGS_DIRTY_HIST_COL;
		}
	}
hist_stop_clk:
...
void mdss_mdp_hist_intr_done(u32 isr)
{
	...
	while (isr != 0) {
		...
		if (hist_info->ctl)
			mdp5_data = mfd_to_mdp5_data(hist_info->ctl->mfd);
		...
		if (hist_info && is_hist_done && hist_info->col_en &&
			(hist_info->col_state == HIST_IDLE ||
			 (mdp5_data && mdp5_data->freerun_histogram_enable))) {
			hist_info->col_state = HIST_READY;
...
{
	if (!mdss_pp_res_left || !mdss_pp_res_right || !mfd)
		return -EINVAL;
	mutex_lock(&mdss_pp_mutex);
...
static void mdss_mdp_hist_intr_notify(u32 disp)
{
	...
	mdp5_data->hist_time = mdp5_data->vsync_time;
...
static int pp_mfd_release_all(struct msm_fb_data_type *mfd)
{
	...
	memset(&mdss_pp_res_left->pp_disp_sts[mfd->index], 0,
			sizeof(mdss_pp_res_left->pp_disp_sts[mfd->index]));
	memset(&mdss_pp_res_right->pp_disp_sts[mfd->index], 0,
			sizeof(mdss_pp_res_right->pp_disp_sts[mfd->index]));
	mfd->bl_scale = 1024;
...
static int pp_get_driver_ops(struct mdp_pp_driver_ops *ops)
{
	...
	mdss_pp_res_left->pp_data_v1_7 = NULL;
	mdss_pp_res_left->pp_data_v3 = NULL;
	mdss_pp_res_right->pp_data_v1_7 = NULL;
	mdss_pp_res_right->pp_data_v3 = NULL;
	...
	case MDSS_MDP_HW_REV_116:
		...
		if (IS_ERR_OR_NULL(pp_cfg))
			...
			mdss_pp_res_left->pp_data_v1_7 = pp_cfg;
			mdss_pp_res_right->pp_data_v1_7 = pp_cfg;
		}
		break;
	...
	case MDSS_MDP_HW_REV_330:
		...
		if (IS_ERR_OR_NULL(pp_cfg)) {
			...
			mdss_pp_res_left->pp_data_v3 = pp_cfg;
			mdss_pp_res_right->pp_data_v3 = pp_cfg;
		}
		break;
				...
				struct mdss_pp_res_type *mdss_pp_res_ptr)
{
	...
	if ((flags & PP_FLAGS_DIRTY_DITHER)) {
		...
		ret = pp_ops[DITHER].pp_set_config(addr, pp_sts,
			&mdss_pp_res_ptr->dither_disp_cfg[disp_num], PPB);
		if (!ret) {
			switch (mixer->num) {
