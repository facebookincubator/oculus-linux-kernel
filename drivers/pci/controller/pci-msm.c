...
#define PCIE20_PARF_SRIS_MODE_ENABLE BIT(0)
...
static void msm_pcie_show_status(struct msm_pcie_dev_t *dev)
{
	...
	PCIE_DBG_FS(dev, "Refclk separate is %s\n",
		    dev->separate_refclk ? "on" : "off");
...
static int msm_pcie_enable(struct msm_pcie_dev_t *dev)
{
	...
	if (dev->separate_refclk) {
		/* enable independent clock support for SRIS and SRNS. */
		msm_pcie_write_mask(dev->parf + PCIE20_PARF_SRIS_MODE, 0,
				    PCIE20_PARF_SRIS_MODE_ENABLE);
	}
...
static int msm_pcie_probe(struct platform_device *pdev)
{
	...
	pcie_dev->separate_refclk =
		of_property_read_bool(of_node, "qcom,separate-refclk");
	PCIE_DBG(pcie_dev, "Refclk separate is %s.\n",
		 pcie_dev->separate_refclk ? "on" : "off");
