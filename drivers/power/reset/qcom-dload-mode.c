...
module_param_call(dump_mode, param_set_int, param_get_int,
			&dump_mode, 0644);
MODULE_PARM_DESC(dump_mode,
		"select either fulldump (0x10), minidump (0x20), or both (0x30)");

static bool emmc_dload = false;
module_param_call(emmc_dload, param_set_bool, param_get_bool,
			&emmc_dload, 0644);
MODULE_PARM_DESC(emmc_dload,
		"boolean to select whether ramdump is written to flash");
