...
static struct power_supply_attr power_supply_attrs[] = {
	...
	POWER_SUPPLY_ATTR(VOLTAGE_REP),
	POWER_SUPPLY_ATTR(VOLTAGE_AVG),
	POWER_SUPPLY_ATTR(VOLTAGE_OCV),
	POWER_SUPPLY_ATTR(VOLTAGE_BOOT),
	POWER_SUPPLY_ATTR(CURRENT_MAX),
	POWER_SUPPLY_ATTR(CURRENT_NOW),
	POWER_SUPPLY_ATTR(CURRENT_REP),
	...
	POWER_SUPPLY_ATTR(VOUT_ENABLE),
	POWER_SUPPLY_ATTR(INPUT_SUSPEND),
	/* Properties of type `const char *' */
	POWER_SUPPLY_ATTR(MODEL_NAME),
	POWER_SUPPLY_ATTR(MANUFACTURER),
	POWER_SUPPLY_ATTR(SERIAL_NUMBER),
	POWER_SUPPLY_ATTR(BATTERY_CHGSTAT),
	POWER_SUPPLY_ATTR(BATT_STATUS),
	POWER_SUPPLY_ATTR(PROT_STATUS),
	POWER_SUPPLY_ATTR(PROT_ALRT),
	POWER_SUPPLY_ATTR(FET_STATUS),
	POWER_SUPPLY_ATTR(BATT_CONFIG),
	POWER_SUPPLY_ATTR(BATT_CONFIG2),
	POWER_SUPPLY_ATTR(COMM_STATUS),
	POWER_SUPPLY_ATTR(SLACK),
	POWER_SUPPLY_ATTR(INI_VERSION),
};
...
static ssize_t power_supply_show_property(struct device *dev,
	...
	case POWER_SUPPLY_PROP_USB_TYPE:
		ret = power_supply_show_usb_type(dev, psy->desc,
						&value, buf);
		break;
	case POWER_SUPPLY_PROP_MODEL_NAME ... POWER_SUPPLY_PROP_BMU_SMT_DATE:
		ret = sprintf(buf, "%s\n", value.strval);
		break;
	default:
		ret = sprintf(buf, "%d\n", value.intval);
...
void power_supply_init_attrs(struct device_type *dev_type)
{
	...
	for (i = 0; i < ARRAY_SIZE(power_supply_attrs); i++) {
		...
		if (!power_supply_attrs[i].prop_name) {
			pr_info("%s: Property %d skipped because is is missing from power_supply_attrs\n",
			 	__func__, i);
