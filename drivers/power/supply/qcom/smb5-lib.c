...
static void smblib_uusb_removal(struct smb_charger *chg)
{
	...
	vote(chg->usb_icl_votable, FULL_BATT_SUSPEND_INPUT_VOTER, false, 0);
				...
				union power_supply_propval *val)
{
	...
	if (!usb_online && !dc_online) {
		/*
		 * USB and/or DC may appear offline becuse 'full batt' input suspend is
		 * presently in effect. If so, report the battery as full.
		 */
		if (((input_present & INPUT_PRESENT_USB) &&
				is_client_vote_enabled(chg->usb_icl_votable, FULL_BATT_SUSPEND_INPUT_VOTER)) ||
		     ((input_present & INPUT_PRESENT_DC) &&
				is_client_vote_enabled(chg->dc_suspend_votable, FULL_BATT_SUSPEND_INPUT_VOTER))) {
	...
	/*
	 * If full battery suspend input is active and has suspended charging, then
	 * continue reporting charging status as FULL.
	 */
	if (is_client_vote_enabled(chg->usb_icl_votable, FULL_BATT_SUSPEND_INPUT_VOTER) ||
	    is_client_vote_enabled(chg->dc_suspend_votable, FULL_BATT_SUSPEND_INPUT_VOTER)) {
		val->intval = POWER_SUPPLY_STATUS_FULL;
		return 0;
	}
				...
				int *total_current_ua)
{
	...
	if (chg->pd_active) {
		*total_current_ua = get_client_vote(chg->usb_icl_votable, PD_VOTER);
		return rc;
	}
...
#define BATT_FULL_SUSPEND_INPUT_ENTRY_MS	300000		/* 5 min */
...
static void smblib_eval_chg_termination(struct smb_charger *chg, u8 batt_status)
{
	...
	if (chg->suspend_input_on_full_batt && batt_status == TERMINATE_CHARGE &&
	    pval.intval == 100) {
		alarm_start_relative(&chg->full_batt_suspend_input_alarm,
				ms_to_ktime(BATT_FULL_SUSPEND_INPUT_ENTRY_MS));
	}
...
irqreturn_t chg_state_change_irq_handler(int irq, void *data)
{
	...
	if (chg->wa_flags & CHG_TERMINATION_WA || chg->suspend_input_on_full_batt)
...
irqreturn_t dc_plugin_irq_handler(int irq, void *data)
{
	...
	if (dcin_present && !vbus_present) {
		...
		vote(chg->dc_suspend_votable, FULL_BATT_SUSPEND_INPUT_VOTER, false, 0);
...
#define BATT_FULL_SUSPEND_INPUT_PERIOD_MS	1800000		/* 30 min */
static void smblib_full_batt_suspend_input_work(struct work_struct *work)
{
	union power_supply_propval pval;
	struct smb_charger *chg = container_of(work, struct smb_charger,
						full_batt_suspend_input_work);
	int rc, input_present, delay = BATT_FULL_SUSPEND_INPUT_PERIOD_MS;

	/*
	 * Hold awake votable to prevent pm_relax being called prior to
	 * completion of this work.
	 */
	vote(chg->awake_votable, FULL_BATT_SUSPEND_INPUT_VOTER, true, 0);

	/*
	 * Check the battery hasn't dropped to the auto_recharge_soc level. If it has,
	 * make sure input is not suspended to we can recharge.
	 */
	rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_REAL_CAPACITY, &pval);
	if (rc < 0 || pval.intval <= chg->auto_recharge_soc) {
		vote(chg->usb_icl_votable, FULL_BATT_SUSPEND_INPUT_VOTER, false, 0);
		vote(chg->dc_suspend_votable, FULL_BATT_SUSPEND_INPUT_VOTER, false, 0);
		goto out;
	}
	...
	/*
	 * Suspend charger inputs for power savings.
	 */
	if (input_present & INPUT_PRESENT_USB)
		vote(chg->usb_icl_votable, FULL_BATT_SUSPEND_INPUT_VOTER, true, 0);
	if (input_present & INPUT_PRESENT_DC)
		vote(chg->dc_suspend_votable, FULL_BATT_SUSPEND_INPUT_VOTER, true, 0);

	/*
	 * Periodically re-evaluate our decision to keep input suspended. This is a
	 * safety-net to ensure we don't leave input suspended indefinitely, as that
	 * would be bad- the device would not be able to charge again without
	 * reconnecting the charger.
	 */
	alarm_start_relative(&chg->full_batt_suspend_input_alarm, ms_to_ktime(delay));
out:
	vote(chg->awake_votable, FULL_BATT_SUSPEND_INPUT_VOTER, false, 0);
}

static enum alarmtimer_restart full_batt_suspend_input_alarm_cb(struct alarm *alarm,
								ktime_t now)
{
	struct smb_charger *chg = container_of(alarm, struct smb_charger,
					       full_batt_suspend_input_alarm);

	smblib_dbg(chg, PR_MISC, "Full Batt Suspend Input: alarm triggered %lld\n",
			ktime_to_ms(now));

	/* Atomic context, cannot use voter */
	pm_stay_awake(chg->dev);
	schedule_work(&chg->full_batt_suspend_input_work);
...
int smblib_init(struct smb_charger *chg)
{
	...
	if (chg->suspend_input_on_full_batt) {
		INIT_WORK(&chg->full_batt_suspend_input_work,
					smblib_full_batt_suspend_input_work);

		if (alarmtimer_get_rtcdev()) {
			alarm_init(&chg->full_batt_suspend_input_alarm, ALARM_BOOTTIME,
						full_batt_suspend_input_alarm_cb);
		} else {
			smblib_err(chg, "Couldn't get rtc device\n");
			return -ENODEV;
		}
...
int smblib_deinit(struct smb_charger *chg)
{
	...
	case PARALLEL_MASTER:
		...
		if (chg->suspend_input_on_full_batt) {
			alarm_cancel(&chg->full_batt_suspend_input_alarm);
			cancel_work_sync(&chg->full_batt_suspend_input_work);
		}
		del_timer_sync(&chg->apsd_timer);
