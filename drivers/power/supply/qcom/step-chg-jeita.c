...
#define MAX_PROFILE_ENTRIES	4
#define MAX_CYCLE_ENTRIES	4
#define MAX_STEP_CHG_ENTRIES	8
#define STEP_CHG_VOTER		"STEP_CHG_VOTER"
#define JEITA_VOTER		"JEITA_VOTER"
#define CYCLE_VOTER		"CYCLE_VOTER"

#define MAX_CYCLES		(1 << 16)
...
struct step_chg_cfg {
	...
	char			*prop_name;
	int			hysteresis;
	struct range_data	fcc_cfg[MAX_PROFILE_ENTRIES][MAX_PROFILE_ENTRIES][MAX_STEP_CHG_ENTRIES];
};
...
struct cycle_fv_cfg {
	...
	char			*prop_name;
	int			hysteresis;
	struct range_data	fv_cfg[MAX_PROFILE_ENTRIES][MAX_STEP_CHG_ENTRIES];
};
...
struct jeita_fcc_cfg {
	...
	char			*prop_name;
	int			hysteresis;
	struct range_data	fcc_cfg[MAX_PROFILE_ENTRIES][MAX_STEP_CHG_ENTRIES];
};
...
static struct step_chg_cfg step_chg_config = {
	...
	.fcc_cfg	= {
		...
		/* PROFILE_1 */
		{
			...
			{
				/* VBAT_LOW	VBAT_HIGH	FCC */
				{3600000,	4060000,	1900000},
				{4061000,	4300000,	1000000},
			},
			/* CYCLE_MID */
			{
				/* VBAT_LOW	VBAT_HIGH	FCC */
				{3600000,	4060000,	1900000},
				{4061000,	4300000,	800000},
			},
			/* CYCLE_HIGH */
			{
				/* VBAT_LOW	VBAT_HIGH	FCC */
				{3600000,	4060000,	1900000},
				{4061000,	4300000,	600000},
			},
		},
	},
	/*
...
static struct cycle_fv_cfg cycle_fv_config = {
	...
	.fv_cfg		= {
		/* PROFILE_0 */
		{
			/* CYCLE_LOW	CYCLE_HIGH	FV */
			{0,		300,		4300000},
			{301,		500,		4250000},
			{501,		MAX_CYCLES,	4200000},
			{-1,		-1,		0},
			{-1,		-1,		0},
			{-1,		-1,		0},
			{-1,		-1,		0},
...
static bool is_charger_available(struct step_chg_info *chip)
{
	if (!chip->chg_psy)
		chip->chg_psy = power_supply_get_by_name("bms");
...
static int handle_chg_profile(struct step_chg_info *chip)
{
	int rc = 0;
	...
	if (pval.intval < 0 || pval.intval > MAX_PROFILE_ENTRIES) {
		pr_warn("Invalid profile ID: %d, defaulting to 0!\n",
				pval.intval);
		pval.intval = 0;
	}
	chip->profile_index = pval.intval;

	return 0;
}

static int handle_cycle_fv_config(struct step_chg_info *chip)
{
	union power_supply_propval pval = {0, };
	...
	rc = get_val(cycle_fv_config.fv_cfg[chip->profile_index],
			cycle_fv_config.hysteresis,
			chip->cycle_index,
			cycle_count,
			&chip->cycle_index,
			&fv_uv);
	if (rc < 0) {
		...
		if (chip->fv_votable)
			vote(chip->fcc_votable, CYCLE_VOTER, false, 0);
	}
	...
	vote(chip->fv_votable, CYCLE_VOTER, true, fv_uv);

	pr_debug("%s = %d Cycle-FV = %duV\n",
		cycle_fv_config.prop_name, cycle_count, fv_uv);
...
static int handle_step_chg_config(struct step_chg_info *chip)
{
	...
	rc = get_val(
		step_chg_config.fcc_cfg[chip->profile_index][chip->cycle_index],
...
static int handle_jeita(struct step_chg_info *chip)
{
	...
	rc = get_val(jeita_fcc_config.fcc_cfg[chip->profile_index],
	...
	rc = get_val(jeita_fv_config.fv_cfg[chip->profile_index],
...
int qcom_step_chg_init(bool step_chg_enable, bool sw_jeita_enable)
{
	...
	if (step_chg_enable && (!cycle_fv_config.psy_prop ||
				!cycle_fv_config.prop_name)) {
		/* fail if step-chg configuration is invalid */
		pr_err("Cycle count configuration not defined - fail\n");
		rc = -ENODATA;
		goto release_wakeup_source;
	}
