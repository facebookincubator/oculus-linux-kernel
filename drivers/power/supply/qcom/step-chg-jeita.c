...
struct multilvl_jeita_fcc_cfg {
	u32			num_batt_lvls;
	int			batt_lvl_index;
	struct range_data	batt_lvl_cfg[MAX_STEP_CHG_ENTRIES];
	struct jeita_fcc_cfg	*jeita_cfgs;
};
...
struct step_chg_info {
	...
	struct multilvl_jeita_fcc_cfg	multi_jeita_fcc_config;
...
#define MAX_MULTI_JEITA_BATT_LVLS	4
#define MULTI_JEITA_BATT_HYSTERESIS_MV	25
#define MAX_DT_PROP_NAME_LEN		32
...
static int get_multilvl_step_chg_jeita_setting(
	...
	u32 max_fcc_ma)
{
	...
	int i;
	u32 num_lvls;
	struct range_data *ranges;
	char prop_name[MAX_DT_PROP_NAME_LEN];

	rc  = of_property_read_u32(
		profile_node,
		"oculus,num-multilevel-battery-lvls",
		&num_lvls);
	if (rc) {
		pr_debug("No multilevel jeita config found\n");
		return rc;
	}

	if (!num_lvls || num_lvls > MAX_MULTI_JEITA_BATT_LVLS) {
		pr_err("Invalid num-multilevel-battery-lvls\n");
		return -EINVAL;
	}
	...
	rc = read_range_data_from_node(profile_node,
				       "oculus,multilevel-battery-ranges",
				       ranges,
				       max_fv_uv,
				       MAX_MULTI_JEITA_BATT_LVLS);
	if (rc < 0) {
		pr_err("Failed to read battery lvl ranges, rc=%d\n", rc);
		return rc;
	}

	for (i = 0; i < MAX_STEP_CHG_ENTRIES; i++) {
		if (ranges[i].value >= num_lvls) {
			pr_err("Invalid battery range config. %u\n",
			       ranges[i].value);
			return -EINVAL;
		}
	}
	...
	chip->multi_jeita_fcc_config.jeita_cfgs = devm_kcalloc(
		chip->dev, num_lvls, sizeof(struct jeita_fcc_cfg), GFP_KERNEL);
	if (!chip->multi_jeita_fcc_config.jeita_cfgs)
		return -ENOMEM;

	for (i = 0; i < num_lvls; i++) {
		struct jeita_fcc_cfg *jeita_cfg;

		rc = scnprintf(prop_name,
			       sizeof(prop_name),
			       "oculus,jeita-fcc-ranges-%d", i);
		if (rc < 0) {
			pr_err("Failed to build fcc_cfgs name.\n");
			return -ENOMEM;
		}

		jeita_cfg = &chip->multi_jeita_fcc_config.jeita_cfgs[i];
		ranges = jeita_cfg->fcc_cfg;
		rc = read_range_data_from_node(
			...
			max_fcc_ma * 1000);
		if (rc < 0) {
			pr_err("Failed to read multi fcc data, rc=%d\n", rc);
			return rc;
		}

		jeita_cfg->psy_prop = POWER_SUPPLY_PROP_TEMP;
		jeita_cfg->prop_name = "BATT_TEMP";
		jeita_cfg->hysteresis = 10;
	}
...
static int handle_multilvl_jeita(struct step_chg_info *chip, int *fcc_ua)
{
	int rc;
	int fcc_idx;
	struct jeita_fcc_cfg *fcc_cfg;
	union power_supply_propval pval = {0, };
	...
	if (rc < 0) {
		pr_err("Couldn't read VBATT property rc=%d\n", rc);
		return rc;
	}

	/* Get fcc config index to use based on battery level. */
	rc = get_val(chip->multi_jeita_fcc_config.batt_lvl_cfg,
		     MULTI_JEITA_BATT_HYSTERESIS_MV,
		     chip->multi_jeita_fcc_config.batt_lvl_index,
		     pval.intval,
		     &chip->multi_jeita_fcc_config.batt_lvl_index,
		     &fcc_idx);
	if (rc < 0) {
		pr_err("Failed to get batt_lvl index\n");
		return rc;
	}

	fcc_cfg = &chip->multi_jeita_fcc_config.jeita_cfgs[fcc_idx];
	...
	if (rc < 0) {
		pr_err("Couldn't read %s property rc=%d\n",
		       fcc_cfg->prop_name, rc);
		return rc;
	}

	/* Get fcc value with given temperature */
	rc = get_val(fcc_cfg->fcc_cfg,
		     fcc_cfg->hysteresis,
...
static int handle_jeita(struct step_chg_info *chip)
{
	...
	if (chip->multi_jeita_fcc_config.num_batt_lvls) {
		/* Multilevel JEITA step charging support */
		rc = handle_multilvl_jeita(chip, &fcc_ua);
		if (rc < 0) {
			pr_err("Failed to read multilvl jeita data. rc = %d",
			       rc);
		}
		goto skip_single_lvl_jeita;
	}
