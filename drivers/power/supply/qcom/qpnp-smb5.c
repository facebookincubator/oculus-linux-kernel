...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	if (chg->auto_recharge_soc) {
		chg->suspend_input_on_full_batt = of_property_read_bool(node,
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_FORCE_RECHARGE:
			if (chg->suspend_input_on_full_batt) {
				/* unsuspend input if suspended due to full charge */
				vote(chg->usb_icl_votable, FULL_BATT_SUSPEND_INPUT_VOTER,
						false, 0);
				vote(chg->dc_suspend_votable, FULL_BATT_SUSPEND_INPUT_VOTER,
						false, 0);
			}
