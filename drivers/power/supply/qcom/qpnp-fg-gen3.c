...
#define MSOC_LOW_THR_v2_WORD        13
#define MSOC_LOW_THR_v2_OFFSET      1
...
static void fg_set_usb_charging_state(struct fg_chip *chip,
	enum fg_usb_charging_state state);
...
static struct fg_sram_param pmi8998_v2_sram_params[] = {
	...
	PARAM(MSOC_LOW_THR, MSOC_LOW_THR_v2_WORD, MSOC_LOW_THR_v2_OFFSET,
		1, 256, 100, 0, fg_encode_default, NULL),
...
static int fg_get_msoc(struct fg_chip *chip, int *msoc)
{
	...
	if (*msoc > chip->dt.suspend_charging_cap_thr) {
		int batt_temp = 0;
		...
		else if (batt_temp > chip->dt.suspend_charging_temp_thr)
			fg_set_usb_charging_state(chip, SUSPEND_CHARGING);
	} else
		fg_set_usb_charging_state(chip, RESUME_CHARGING);
...
static bool is_debug_batt_id(struct fg_chip *chip)
{
...
#ifdef CONFIG_FAKE_BATTERY_WITH_TEMPERATURE
	int batt_temp = 0;
#endif
	...
	rc = fg_get_battery_temp(chip, &batt_temp);
	if (rc < 0) {
		pr_err("Error in getting battery temperature\n");
	} else {
		// Real battery is not present that's why the temperature is
		// reported as a big negative value.
		if (batt_temp <= -200)
			return true;
	}
#endif
...
static inline const char *usb_charging_state(enum fg_usb_charging_state state)
{
	switch (state) {
	case RESUME_CHARGING:
		return "charging resumed";
	case SUSPEND_CHARGING:
	...
	enum fg_usb_charging_state state)
{
	...
	if (!batt_psy_initialized(chip) || !usb_psy_initialized(chip))
		return;

	rc = power_supply_get_property(chip->batt_psy,
		POWER_SUPPLY_PROP_INPUT_SUSPEND, &prop);
	if (rc < 0) {
		pr_err("Error in getting battery charging state, rc=%d\n", rc);
		return;
	}

	/* Skip resume if battery input suspend is set */
	if (prop.intval == 1 && state == RESUME_CHARGING)
		return;

	rc = power_supply_get_property(chip->usb_psy,
		POWER_SUPPLY_PROP_INPUT_SUSPEND, &prop);
	if (rc < 0) {
		pr_err("Error in getting usb charging state, rc=%d\n", rc);
		return;
	}

	if (state == prop.intval) {
		fg_dbg(chip, FG_STATUS,
			"Current usb charging state is already \"%s\"\n",
	...
	rc = power_supply_set_property(chip->usb_psy,
		POWER_SUPPLY_PROP_INPUT_SUSPEND, &prop);
	if (rc < 0) {
		pr_err("Error in setting usb charging state, rc=%d\n", rc);
		return;
	}

	fg_dbg(chip, FG_STATUS,
		"Set usb charging state to \"%s\" successfully.\n",
		usb_charging_state(state));
}
...
static DEVICE_ATTR(cycle_counts_buckets, 0660,
				       ...
				       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_ESR_ACTUAL:
		rc = fg_get_sram_prop(chip, FG_SRAM_ESR, &pval->intval);
		break;
...
static int fg_hw_init(struct fg_chip *chip)
{
	...
	if (chip->dt.msoc_low_thr > 0) {
		fg_encode(chip->sp, FG_SRAM_MSOC_LOW_THR,
			chip->dt.msoc_low_thr, buf);
		rc = fg_sram_write(chip,
				chip->sp[FG_SRAM_MSOC_LOW_THR].addr_word,
				chip->sp[FG_SRAM_MSOC_LOW_THR].addr_byte,
				buf, chip->sp[FG_SRAM_MSOC_LOW_THR].len,
				FG_IMA_DEFAULT);
		if (rc < 0) {
			pr_err("Error in writing msoc_low_thr, rc = %d\n", rc);
			return rc;
		}
	}
...
#define DEFAULT_MSOC_LOW_THR        10
...
static int fg_parse_dt(struct fg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,fg-msoc-low-thr", &temp);
	if (rc < 0)
		chip->dt.msoc_low_thr = DEFAULT_MSOC_LOW_THR;
	else
		chip->dt.msoc_low_thr = temp;
	...
	rc = of_property_read_u32(node,
		"qcom,suspend-charging-cap-thr", &temp);
	if (!rc)
		chip->dt.suspend_charging_cap_thr = temp;

	rc = of_property_read_u32(node,
		"qcom,suspend-charging-temp-thr", &temp);
	if (!rc)
		chip->dt.suspend_charging_temp_thr = temp;
...
static int fg_gen3_probe(struct platform_device *pdev)
{
	...
	rc = device_create_file(chip->dev, &dev_attr_cycle_counts_buckets);
	if (rc != 0) {
		dev_err(chip->dev,
