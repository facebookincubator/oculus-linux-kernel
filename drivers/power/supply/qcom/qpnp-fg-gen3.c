...
static struct fg_sram_param pmi8998_v1_sram_params[] = {
	...
	/* Entries below are non-standard entries */
	PARAM(CUTOFF_SOC, CUTOFF_SOC_WORD, CUTOFF_SOC_OFFSET, 2, 1, 1, 0, NULL,
		fg_decode_default),
	PARAM(SYSTEM_SOC, SYSTEM_SOC_WORD, SYSTEM_SOC_OFFSET, 2, 1, 1, 0, NULL,
		fg_decode_default),
};
...
static int fg_get_battery_resistance_slow(struct fg_chip *chip, int *val)
{
	int rc;
...
static int fg_get_batt_soc(struct fg_chip *chip, int *val)
{
	int rc, batt_soc;
		...
		struct fg_cycle_counts *counts)
{
	...
	for (i = 0; i < BUCKET_COUNT; i++) {
		...
		bucket = counts->buckets[i].id;
		rc = fg_sram_write(chip,
				CYCLE_COUNT_WORD + (bucket / 2),
				CYCLE_COUNT_OFFSET + (bucket % 2) * 2,
				(u8 *)&counts->buckets[i].count, sizeof(u16),
				FG_IMA_DEFAULT);
		if (rc)
...
static int fg_get_cycle_count(struct fg_chip *chip)
{
	...
	mutex_unlock(&chip->cyc_ctr.lock);
  fg_dbg(chip, FG_STATUS, "Battery charge-discharge cycle count : %d\n", count);
	return count;
}
...
static int fg_get_aggregate_cycle_count(struct fg_chip *chip)
{
	...
	mutex_unlock(&chip->cyc_ctr.lock);
	fg_dbg(chip, FG_STATUS, "Aggregate charge-discharge cycle count: %d\n",
			count / BUCKET_COUNT);
				       ...
				       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_RESISTANCE_SLOW:
		rc = fg_get_battery_resistance_slow(chip, &pval->intval);
		break;
  ...
  case POWER_SUPPLY_PROP_FAKE_AGGREGATE_CYCLE_COUNT:
		pval->intval = chip->fake_battery_cycle_count;
		break;
	case POWER_SUPPLY_PROP_AGGREGATE_CYCLE_COUNT:
		if (chip->fake_battery_cycle_count >= 0)
			pval->intval = chip->fake_battery_cycle_count;
		else
			pval->intval = fg_get_aggregate_cycle_count(chip);
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_BATTERY:
		rc = fg_get_batt_soc(chip, &pval->intval);
		break;
	case POWER_SUPPLY_PROP_CHARGE_CUTOFF:
		rc = fg_get_sram_prop(chip, FG_SRAM_CUTOFF_SOC, &pval->intval);
		break;
	case POWER_SUPPLY_PROP_CHARGE_MONOTONIC:
		rc = fg_get_msoc_raw(chip, &pval->intval);
		break;
	case POWER_SUPPLY_PROP_CHARGE_SYSTEM:
		rc = fg_get_sram_prop(chip, FG_SRAM_SYSTEM_SOC, &pval->intval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_PREDICTED:
		rc = fg_get_sram_prop(chip, FG_SRAM_VOLTAGE_PRED,
			&pval->intval);
		break;
				  ...
				  const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		...
		break;
	case POWER_SUPPLY_PROP_AGGREGATE_CYCLE_COUNT:
		chip->fake_battery_cycle_count = pval->intval;
		break;
	case POWER_SUPPLY_PROP_CALIBRATE:
...
static int fg_parse_dt(struct fg_chip *chip)
{
	...
	if (of_property_count_elems_of_size(node, "qcom,fg-jeita-thresholds",
      ...
      fg_dbg(chip, FG_STATUS, "JEITA Thresholds read as COLD: %d, COOL: %d, WARM: %d, HOT: %d\n",
          chip->dt.jeita_thresholds[JEITA_COLD], chip->dt.jeita_thresholds[JEITA_COOL],
          chip->dt.jeita_thresholds[JEITA_WARM], chip->dt.jeita_thresholds[JEITA_HOT]);
	}
