			...
			char *buf)
{
	...
	if (ret < BUCKET_COUNT*2) {
		dev_err(chip->dev, "Error in showing cycle counts, rc=%d\n", ret);
		return 0;
	}
			...
			const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d %d %d %d %d %d %d %d",
		...
		!= BUCKET_COUNT) {
		dev_err(chip->dev, "Error storing cycle count arguments.\n");
		return -EINVAL;
	}
	...
	if (rc < 0) {
		dev_err(chip->dev, "Error in restoring cycle counts, rc=%d\n", rc);
		return rc;
	}
...
#define DEFAULT_DEBUG_BATT_ID_LOW	6000
#define DEFAULT_DEBUG_BATT_ID_HIGH	8500
static bool is_debug_batt_id(struct qpnp_qg *chip)
{
	if (is_between(chip->debug_batt_id_low, chip->debug_batt_id_high,
					chip->batt_id_ohm))
		return true;
...
static int qg_process_fifo(struct qpnp_qg *chip, u32 fifo_length)
{
	...
	for (i = 0; i < fifo_length * 2; i = i + 2, j++) {
		...
		power = (chip->last_fifo_v_uv / 1000) * (chip->last_fifo_i_ua / 1000);
		kfifo_put(&chip->power_fifo, power);
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_FG_TYPE:
		...
		break;
	case POWER_SUPPLY_PROP_QG_POWER_FIFO_POP:
		mutex_lock(&chip->data_lock);

		/* return 0 if the fifo queue is empty */
		if (!kfifo_is_empty(&chip->power_fifo))
			kfifo_get(&chip->power_fifo, &pval->intval);
		else
			pval->intval = 0;
...
static enum power_supply_property qg_psy_props[] = {
	...
	POWER_SUPPLY_PROP_QG_POWER_FIFO_POP,
};
...
static int qg_parse_dt(struct qpnp_qg *chip)
{
	...
	if (of_property_read_u32_index(node, "qcom,debug-batt-id-range", 0, &chip->debug_batt_id_low) < 0)
		chip->debug_batt_id_low = DEFAULT_DEBUG_BATT_ID_LOW;
	if (of_property_read_u32_index(node, "qcom,debug-batt-id-range", 1, &chip->debug_batt_id_high) < 0)
		chip->debug_batt_id_high = DEFAULT_DEBUG_BATT_ID_HIGH;
...
static int qpnp_qg_probe(struct platform_device *pdev)
{
	...
	chip->qg_version = (u32)(uintptr_t)of_device_get_match_data(&pdev->dev);
