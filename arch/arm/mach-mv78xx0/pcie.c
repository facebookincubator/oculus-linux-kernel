...
{
	*dev = unknown5_pcie_dev_id(PCIE00_VIRT_BASE);
	*rev = unknown5_pcie_rev(PCIE00_VIRT_BASE);
}
...
static int __init mv78xx0_pcie_setup(int nr, struct pci_sys_data *sys)
{
	 ...
	 */
	unknown5_pcie_set_local_bus_nr(pp->base, sys->busnr);
	unknown5_pcie_setup(pp->base);
			...
			int size, u32 *val)
{
	...
	ret = unknown5_pcie_rd_conf(pp->base, bus, devfn, where, size, val);
			...
			int where, int size, u32 val)
{
	...
	spin_lock_irqsave(&pp->conf_lock, flags);
	ret = unknown5_pcie_wr_conf(pp->base, bus, devfn, where, size, val);
	spin_unlock_irqrestore(&pp->conf_lock, flags);
...
void __init mv78xx0_pcie_init(int init_port0, int init_port1)
{
	...
	if (init_port0) {
		...
		if (!unknown5_pcie_x4_mode(PCIE00_VIRT_BASE)) {
	...
	if (init_port1) {
		...
		if (!unknown5_pcie_x4_mode((void __iomem *)PCIE10_VIRT_BASE)) {
