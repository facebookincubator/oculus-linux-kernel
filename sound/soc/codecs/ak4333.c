...
struct ak4333_priv {
	...
	unsigned int *hp_irq_gpios;  /* headset irq GPIO pin */
	int *hp_irqs;
	int hp_count;
	bool hp_inserted;
...
)
{
	...
	for (i = 0; i <= 500; i++) {
		val = snd_soc_component_read(component,
			AK4333_09_IMPEDANCE_DET_STATUS);
		if (val & 0x08)
			ret = -EIO;
		if (ret < 0 || (val & 0x01))
			break;
	...
	if (!ret) {
		/* get IMP */
		val = snd_soc_component_read(component,
			AK4333_13_IMPEDANCE_DET_RESULT);
...
static int ak4333_set_dai_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	struct snd_soc_component *component = dai->component;
	...
	if (mute == 0) {
		...
		/* DAC power up*/
		snd_soc_component_update_bits(component,
				AK4333_02_POWER_MANAGEMENT3, 0x01, 0x01);
		/* AMP power up*/
		snd_soc_component_update_bits(component,
				AK4333_03_POWER_MANAGEMENT4, 0x03, 0x03);
	} else {
		/* AMP power down*/
		snd_soc_component_update_bits(component,
				AK4333_03_POWER_MANAGEMENT4, 0x03, 0x00);
		/* DAC power down*/
		snd_soc_component_update_bits(component,
				AK4333_02_POWER_MANAGEMENT3, 0x01, 0x00);
	}
...
static struct snd_soc_dai_ops ak4333_dai_ops = {
	...
	.mute_stream = ak4333_set_dai_mute,
};
...
static int ak4333_init_reg(struct snd_soc_component *component)
{
	...
	DeviceID = snd_soc_component_read(component, AK4333_15_AUDIO_IF_FORMAT);
	...
	/*
	 * Before PMPLL set to 1, PLLMD,CM,FS,LDACL,RDACR,HP
	 * registers need to be configured. Among them,
	 * PLLMD and HPMD use default value.
	 *
	 * Before PMTIM set to 1, PLS,PLD,PLM,MDIV,DIF,MS
	 * registers need to be configured. Among them,
	 * MDIV,MS,DIF use default value.
	 *
	 */
	/* Set CM */
	snd_soc_component_update_bits(component,
		AK4333_05_CLOCK_MODE_SELECT, 0x60, 0x20);
	/* Set FS */
	snd_soc_component_update_bits(component,
		AK4333_05_CLOCK_MODE_SELECT, 0x1F, 0x0a);
	/* Set LDACL */
	snd_soc_component_update_bits(component,
		AK4333_07_DAC_MONO_MIXING, 0x01, 0x01);
	/* Set RDACR */
	snd_soc_component_update_bits(component,
		AK4333_07_DAC_MONO_MIXING, 0x20, 0x20);
	...
	/* Set PLD */
	snd_soc_component_update_bits(component,
		AK4333_10_PLL_REF_CLK_DIVIDER2, 0xFF, 0x03);
	/* Set PLM */
	snd_soc_component_update_bits(component,
		AK4333_12_PLL_FB_CLK_DIVIDER2, 0xFF, 0x1f);
...
static int set_volume_according_impedance(struct ak4333_priv *ak4333)
{
	int ret;
	unsigned int i, val, impedance;

	ret = 0;

#if IS_ENABLED(CONFIG_SND_SOC_IMPEDANCE_DETECTION)
	/* AK4333 power up*/
	if (!ak4333->nDACOn) {
		int pll = ((ak4333->PLLMode > 0) ? 0x01 : 0x00);

		/* PMPLL */
		regmap_update_bits(ak4333->regmap,
			AK4333_00_POWER_MANAGEMENT1, 0x01, pll);
		/* wait 2ms */
		msleep(2);
		/* PMTIM = 1 */
		regmap_update_bits(ak4333->regmap,
	...
	akdbgprt("%s: the detected headphone impedance is 0x%x\n",
		__func__, impedance);
#else
	akdbgprt("%s: overwriting detected impedance with 0x%x\n",
		__func__, 0x20);
	i = 0;
	val = 0;
	impedance = 0x20;
	...
	/*Get AK4333_0D_HP_VOLUME_CONTROL value*/

	if (impedance < 0x2E) {
		/* If impedance < 46ohm, set digital volume 0db*/
		regmap_write(ak4333->regmap,
			AK4333_0B_LCH_OUTPUT_VOLUME, 0x19);

		/* Set Analog volume -6db*/
		regmap_update_bits(ak4333->regmap,
			AK4333_0D_HP_VOLUME_CONTROL, 0x07, 0x02);
	} else if ((impedance >= 0x2E) && (impedance < 0x33)) {
		/* If 46ohm <= impedance < 51ohm, set digital volume -1.5db*/
		regmap_write(ak4333->regmap,
			AK4333_0B_LCH_OUTPUT_VOLUME, 0x16);

		/* Set Analog volume -4db*/
		regmap_update_bits(ak4333->regmap,
			AK4333_0D_HP_VOLUME_CONTROL, 0x07, 0x03);
	} else if ((impedance >= 0x33) && (impedance < 0x3D)) {
		/* If 51ohm <= impedance < 61ohm, set digital volume -0.5db*/
		regmap_write(ak4333->regmap,
			AK4333_0B_LCH_OUTPUT_VOLUME, 0x18);

		/* Set Analog volume -4db*/
		regmap_update_bits(ak4333->regmap,
			AK4333_0D_HP_VOLUME_CONTROL, 0x07, 0x03);
	} else if ((impedance >= 0x3D) && (impedance < 0x47)) {
		/* If 61ohm <= impedance < 71ohm, set digital volume -1db*/
		regmap_write(ak4333->regmap,
			AK4333_0B_LCH_OUTPUT_VOLUME, 0x17);

		/* Set Analog volume -2db*/
		regmap_update_bits(ak4333->regmap,
			AK4333_0D_HP_VOLUME_CONTROL, 0x07, 0x04);
	} else if ((impedance >= 0x47) && (impedance < 0x51)) {
		/* If 71ohm <= impedance < 81ohm, set digital volume 0db*/
		regmap_write(ak4333->regmap,
			AK4333_0B_LCH_OUTPUT_VOLUME, 0x19);
		...
		regmap_update_bits(ak4333->regmap,
			AK4333_0D_HP_VOLUME_CONTROL, 0x07, 0x04);
	} else if ((impedance >= 0x51) && (impedance < 0x5B)) {
		/* If 81ohm <= impedance < 91ohm, set digital volume -0.5db*/
		regmap_write(ak4333->regmap,
			AK4333_0B_LCH_OUTPUT_VOLUME, 0x18);

		/* Set Analog volume 0db*/
		regmap_update_bits(ak4333->regmap,
			AK4333_0D_HP_VOLUME_CONTROL, 0x07, 0x05);
	} else if ((impedance >= 0x5B) && (impedance < 0x64)) {
		/* If 91ohm <= impedance < 100ohm, set digital volume -1.5db*/
		regmap_write(ak4333->regmap,
			AK4333_0B_LCH_OUTPUT_VOLUME, 0x16);

		/* Set Analog volume 2db*/
		regmap_update_bits(ak4333->regmap,
			AK4333_0D_HP_VOLUME_CONTROL, 0x07, 0x06);
	} else {
		/* If impedance >= 100ohm, set digital volume -0.5db*/
		regmap_write(ak4333->regmap,
			AK4333_0B_LCH_OUTPUT_VOLUME, 0x18);
...
static void ak4333_hp_det_report(struct work_struct *work)
{
	...
	for (i = 0; i < ak4333->hp_count; i++) {
		value |= !gpio_get_value(ak4333->hp_irq_gpios[i]);
		akdbgprt("\t[AK4333] %s gpio value = %d\n", __func__, value);
	}

	if (ak4333->hp_inserted && !value) {
		snd_soc_jack_report(&ak4333->jack, 0, AK4333_JACK_MASK);
		ak4333->hp_inserted = false;
	} else if (!ak4333->hp_inserted && value) {
		snd_soc_jack_report(&ak4333->jack,
			SND_JACK_HEADPHONE, AK4333_JACK_MASK);
		ak4333->hp_inserted = true;
	}

	if(value) {
		/* Add for 3.5mm headphone impedance detection */
		akdbgprt("%s :set digital volume for different impedance.\n",
			__func__);
		set_volume_according_impedance(ak4333);
	}
}
...
static int ak4333_parse_dt_hp_det(struct ak4333_priv *ak4333)
{
	...
	ak4333->hp_count = of_gpio_named_count(dev->of_node,
		"ak4333,headset-det-gpio");
	if (ak4333->hp_count < 0) {
		akdbgprt("\t[ak4333] %s Audio jack detection is not supported\n", __func__);
		return 0;
	}

	ak4333->hp_irq_gpios = devm_kcalloc(dev,
		ak4333->hp_count, sizeof(*ak4333->hp_irq_gpios),
		GFP_KERNEL);
	if (!ak4333->hp_irq_gpios) {
		dev_err(dev, "Cannot allocate hp irq gpios array");
		rc = -ENOMEM;
		goto error;
	}

	ak4333->hp_irqs = devm_kcalloc(dev,
		ak4333->hp_count, sizeof(*ak4333->hp_irqs),
		GFP_KERNEL);
	if (!ak4333->hp_irqs) {
	...
	for (i = 0; i < ak4333->hp_count; i++) {
		rc = of_get_named_gpio(np, "ak4333,headset-det-gpio", i);
		if (!gpio_is_valid(rc)) {
			dev_err(dev, "gpio %d is invalid\n", i);
			rc = -EINVAL;
			goto error;
		}
		ak4333->hp_irq_gpios[i] = rc;

		rc = devm_gpio_request(dev, ak4333->hp_irq_gpios[i],
				"ak4333-hp-det");
		if (rc) {
			dev_err(dev, "ak4333-hp-det gpio request fail\n");
			rc = -EINVAL;
			goto error;
		}

		rc = gpio_direction_input(ak4333->hp_irq_gpios[i]);
		if (rc) {
			dev_err(dev, "ak4333-hp-det input fail\n");
			rc = EINVAL;
			goto error;
		}

		ak4333->hp_irqs[i] =
			gpio_to_irq(ak4333->hp_irq_gpios[i]);
	}

	/*
	 * Request IRQs only after all GPIOs are configured, since the IRQ
	 * handler will queue work that iterates over all GPIOs.
	 */
	for (i = 0; i < ak4333->hp_count; i++) {
		rc = devm_request_threaded_irq(dev, ak4333->hp_irqs[i],
			NULL, ak4333_headset_det_irq_thread,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING |
			IRQF_ONESHOT, "ak4333-hp-det", ak4333);
		if (rc != 0) {
		...
		rc = enable_irq_wake(ak4333->hp_irqs[i]);
		if (rc) {
			dev_err(dev, "Failed to set wake interrupt on IRQ %d:%d\n",
					ak4333->hp_irqs[i], rc);
			rc = -EINVAL;
			goto error;
		}
...
static void ak4333_remove(struct snd_soc_component *component)
{
	...
	ret = regulator_disable(ak4333->vdd);
	if (ret < 0)
		dev_err(dev, "VDD regulator_disable failed\n");
}
...
static int ak4333_resume(struct snd_soc_component *component)
{
	...
	if (ak4333->vdd_voltage != 0) {
		ret = regulator_set_voltage(ak4333->vdd,
			ak4333->vdd_voltage, ak4333->vdd_voltage);
		if (ret < 0) {
			dev_err(dev, "VDD regulator_set_voltage failed\n");
			return ret;
		}
	}

	if (ak4333->vdd_current != 0) {
		ret = regulator_set_load(ak4333->vdd,
			ak4333->vdd_current);
		if (ret < 0) {
			dev_err(dev, "VDD regulator_set_load failed\n");
			return ret;
		}
	}

	ret = regulator_enable(ak4333->vdd);
	if (ret < 0) {
		dev_err(dev, "VDD regulator_enable failed\n");
		return ret;
	}
...
)
{
	...
	ak4333->vdd = devm_regulator_get(&i2c->dev, "ak4333,vdd");
	if (IS_ERR(ak4333->vdd)) {
		ret = PTR_ERR(ak4333->vdd);
		dev_err(&i2c->dev, "Unable to get vdd:%d\n", ret);
		return ret;
	}

	ret = of_property_read_u32(np, "ak4333,vdd-voltage",
				&ak4333->vdd_voltage);
	if (ret) {
		dev_err(&i2c->dev,
			"%s:Looking up %s property in node %s failed\n",
			__func__, "ak4333,vdd-voltage",
			np->full_name);
	}

	ret = of_property_read_u32(np, "ak4333,vdd-current",
				&ak4333->vdd_current);
	if (ret) {
		dev_err(&i2c->dev,
			"%s:Looking up %s property in node %s failed\n",
			__func__, "ak4333,vdd-current",
			np->full_name);
	}
	...
	if (ak4333->vdd_voltage != 0) {
		ret = regulator_set_voltage(ak4333->vdd,
			ak4333->vdd_voltage, ak4333->vdd_voltage);
		if (ret < 0) {
			dev_err(&i2c->dev, "VDD regulator_set_voltage failed\n");
			return ret;
		}
	}
	...
	if (ak4333->vdd_current != 0) {
		ret = regulator_set_load(ak4333->vdd,
			ak4333->vdd_current);
		if (ret < 0) {
			dev_err(&i2c->dev, "VDD regulator_set_load failed\n");
			return ret;
		}
	}
	...
	ret = regulator_enable(ak4333->vdd);
	if (ret < 0) {
		dev_err(&i2c->dev, "VDD regulator_enable failed\n");
		return ret;
	}
