...
static struct reg_default max98388_reg[] = {
	...
	{MAX98388_R2059_PCM_RX_SRC2, 0x00},
	{MAX98388_R205C_PCM_TX_DRIVE_STRENGTH, 0x01},
	...
	{MAX98388_R2094_SPK_AMP_ER_CTRL, 0x0F},
...
static const char * const max98388_spkmon_duration_text[] = {
	"10ms", "25ms", "50ms", "75ms", "100ms", "200ms",
	"300ms", "400ms", "500ms", "600ms", "700ms", "800ms",
	"900ms", "1000ms", "1100ms", "1200ms"
};
...
static const struct snd_kcontrol_new max98388_snd_controls[] = {
	...
	SOC_SINGLE("PCM Invert", MAX98388_R2091_SPK_CH_CFG,
		   MAX98388_SPK_CFG_INV_SHIFT, 1, 0),
	...
	for (cnt = 0 ; cnt < MAX_NUM_SLOTS ; cnt++, mask >>= 1) {
		if (mask & 0x1) {
			if (slot_found == 0)
				regmap_update_bits(max98388->regmap,
					MAX98388_R2059_PCM_RX_SRC2,
					MAX98388_RX_SRC_CH0_MASK,
					cnt << MAX98388_RX_SRC_CH0_SHIFT);
			else
				regmap_update_bits(max98388->regmap,
					MAX98388_R2059_PCM_RX_SRC2,
					MAX98388_RX_SRC_CH1_MASK,
					cnt << MAX98388_RX_SRC_CH1_SHIFT);
			slot_found++;
...
static bool max98388_writeable_register(struct device *dev,
					unsigned int reg)
{
	switch (reg) {
	...
	case MAX98388_R20E0_BP_ALC_THRESH
		... MAX98388_R20E4_BP_ALC_MUTE:
	case MAX98388_R20EE_BP_INF_HOLD_REL
		... MAX98388_R20EF_BP_ALC_EN:
...
static int max98388_suspend(struct device *dev)
{
	...
	if (max98388->vdd) {
		ret = regulator_disable(max98388->vdd);
		if (ret < 0)
...
static int max98388_resume(struct device *dev)
{
	...
	if (max98388->vdd) {
		ret = regulator_enable(max98388->vdd);
		if (ret < 0) {
...
static const struct regmap_config max98388_regmap = {
	...
	.writeable_reg = max98388_writeable_register,
				    ...
				    struct max98388_priv *max98388)
{
	...
	if (!device_property_read_string(dev, "device-name", &name)) {
		dev_set_name(dev, name);
	} else {
		dev_set_name(dev, "max98388");
	}
...
static int max98388_set_pinctrl(struct device *dev,
	struct max98388_priv *max98388)
{
	int ret;

	max98388->pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR_OR_NULL(max98388->pinctrl)) {
		dev_err(dev, "Failed to look up pinctrl");
		return -EINVAL;
	}

	max98388->pin_default = pinctrl_lookup_state(max98388->pinctrl,
			"default");
	if (IS_ERR_OR_NULL(max98388->pin_default)) {
		dev_err(dev, "Failed to look up default pin state");
		devm_pinctrl_put(max98388->pinctrl);
		return -EINVAL;
	}

	ret = pinctrl_select_state(max98388->pinctrl,
			max98388->pin_default);
	if (ret != 0) {
			      ...
			      const struct i2c_device_id *id)
{
	...
	max98388->vdd = devm_regulator_get_optional(&i2c->dev, "vdd");
	if (IS_ERR(max98388->vdd)) {
		ret = PTR_ERR(max98388->vdd);
		dev_err(&i2c->dev, "Unable to get vdd regulator: %d\n", ret);
		return ret;
	}
	if (max98388->vdd) {
		ret = regulator_enable(max98388->vdd);
		if (ret < 0) {
			dev_err(&i2c->dev, "vdd enable failed: %d\n", ret);
			return ret;
		}
	}
	...
	if (device_property_read_bool(&i2c->dev, "adi,interleave-mode"))
	...
	ret = max98388_set_pinctrl(&i2c->dev, max98388);
	if (ret)
		dev_err(&i2c->dev, "%s: Failed to set_pinctrl ret=%d\n",
			__func__, ret);
...
static int max98388_i2c_remove(struct i2c_client *i2c)
{
