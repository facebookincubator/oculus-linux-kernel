...
/*
 * This driver is based on kernel-4.19 development.

 * Release note:
 * v0.1: (2019/12/03) Add AEC / ENC control parameters for DSP and ALSA
 * v0.2: (2019/12/05) Merge v0.1 code into Goertek cm710x driver
 * v0.3: (2019/12/12) Fix AMIC recording glitch bug
 * v0.4: (2019/12/30) Fix SPK EQ Gain set and get value error bug
...
struct cm710x_mic_calibration {
	const struct firmware *mic_cal_fw;
	bool mic_calibrated;
	u32 mic_calibrated_reg_val;
	s32 mic1_volume_step_count;
	s32 mic2_volume_step_count;
};
...
struct EQParam {
	...
	unsigned long HPF : 1; // is HPF, ignored if Q != 0
};
...
struct cm710x_codec_priv {
	...
	struct EQParam MICEQParam[8];

	const char *fw_parsing;

	struct notifier_block pm_nb;
	struct cm710x_mic_calibration mic_calib;
};
