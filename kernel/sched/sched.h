...
struct task_group {
	...
	/* Wake affine to task's previous cpu flag for a task group */
	unsigned int		wake_affine;
...
static inline bool uclamp_wake_affine(struct task_struct *p)
{
	struct cgroup_subsys_state *css = task_css(p, cpu_cgrp_id);
	struct task_group *tg;
