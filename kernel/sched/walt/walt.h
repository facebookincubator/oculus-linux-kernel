...
extern int sched_dynamic_tp_handler(struct ctl_table *table, int write,
			void __user *buffer, size_t *lenp, loff_t *ppos);
extern bool uclamp_task_background(struct task_struct *p);
...
extern unsigned int __read_mostly sysctl_sched_lb_rebalance_bg;
extern unsigned int __read_mostly sysctl_sched_adaptive_busy_factor;
