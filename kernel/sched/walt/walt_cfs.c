					...
					bool *force_energy_eval)
{
	...
	unsigned int cpu_rq_mvp_runnable_cnt = UINT_MAX;
	...
	if (((capacity_orig_of(prev_cpu) == capacity_orig_of(start_cpu)) ||
		...
		if (uclamp_wake_affine(p) &&
		    !__cpu_overutilized(prev_cpu,
					p->state == TASK_WAKING ?
					task_util(p) : 0)) {
			fbt_env->fastpath = PREV_CPU_WAKE_AFFINE;
			cpumask_set_cpu(prev_cpu, candidates);
			visited_clusters[cpu_cluster(prev_cpu)->id] = true;
			goto out;
		}
...
do_expand_search:
		for_each_cpu(i, &visit_cpus) {
			...
			if (!expand_search && wrq->num_mvp_tasks > 0) {
				cpumask_set_cpu(i, &visit_cpus);
				continue;
			}
			 ...
			 * number of runnables.
			 *
			 * When the cpu_utils are same (happens when they are
			 * overutilized at 1024), make sure we select a rq with
			 * less mvp_tasks.
			 */
			if (cpu_rq(i)->nr_running < cpu_rq_runnable_cnt) {
				cpu_rq_runnable_cnt = cpu_rq(i)->nr_running;
				cpu_rq_mvp_runnable_cnt = wrq->num_mvp_tasks;
				least_nr_cpu = i;
			} else if (cpu_rq(i)->nr_running == cpu_rq_runnable_cnt &&
				   wrq->num_mvp_tasks < cpu_rq_mvp_runnable_cnt) {
				cpu_rq_mvp_runnable_cnt = wrq->num_mvp_tasks;
				least_nr_cpu = i;
			}
		...
		} else if (cpumask_weight(&visit_cpus)) {
			expand_search = true;
			goto do_expand_search;
		}
				     ...
				     int sync, int sibling_count_hint)
{
	...
	bool wake_affine = uclamp_wake_affine(p);
...
unlock:
	...
	trace_sched_task_util(p, cpumask_bits(candidates)[0], best_energy_cpu,
			...
			start_t, uclamp_boost, start_cpu, wake_affine);
