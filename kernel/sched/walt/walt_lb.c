...
static void walt_find_busiest_group(void *unused, struct sched_group *busiest,
				    struct rq *dst_rq, int *out_balance)
{
	struct rq *src_rq;

	if (sysctl_sched_lb_rebalance_bg == 0)
		return;

	if (uclamp_task_background(dst_rq->curr)) {
		src_rq = cpu_rq(cpumask_first(sched_group_span(busiest)));
		if (!uclamp_task_background(src_rq->curr))
			*out_balance = 0;
	}
}

static void walt_sched_rebalance_domains(void *unused, struct rq *rq,
					 int *continue_balancing)
{
	struct sched_domain *sd;

	if (!sysctl_sched_adaptive_busy_factor)
		return;

	/* rq has SCHED_IDLE tasks only */
	if (unlikely(rq->nr_running == rq->cfs.idle_h_nr_running &&
	    rq->nr_running))
		return;

	if (uclamp_task_background(rq->curr)) {
		rcu_read_lock();
		sd = rcu_dereference(rq->sd);
		if (sd->min_interval > sd->busy_factor)
			sd->balance_interval = (sd->min_interval / sd->busy_factor);
		else
			sd->balance_interval = 1;
		rcu_read_unlock();
	}

	/* keep the *continue_balancing = 1 */
}
...
void walt_lb_init(void)
{
	...
	register_trace_android_rvh_sched_rebalance_domains(walt_sched_rebalance_domains, NULL);
}
