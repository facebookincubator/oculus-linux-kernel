...
static int create_image(int platform_mode)
{
 ...
 Platform_finish:
	...
	bootstat_record_kernel2_event(HIBEVENT_KERN2_START_DEVICE_RESUME);
...
static int load_image_and_restore(void)
{
	...
	bootstat_record_kernel1_event(HIBEVENT_KERN1_LOAD_MEM_FROM_DISK_STARTED);
	...
	bootstat_record_kernel1_event(HIBEVENT_KERN1_LOAD_MEM_FROM_DISK_FINISHED);
	if (!error)
...
int hibernate(void)
{
	...
	*/
	...
	if (error) {
		...
		hibernate_stats.failed_notifier++;
		hib_save_failed_step(HIBERNATE_NOTIFIER);
		goto Restore;
	}
	...
	if (disk_write_skip) {
		pr_info("Skipping hibernate image save but mark swap file\n");
		swsusp_mark_swapfile();
		power_down();
	}
...
static int software_resume(void)
{
 ...
 Check_image:
	...
	*/
	...
	error = load_image_and_restore();
	thaw_processes();
	bootstat_record_kernel1_event(HIBEVENT_KERN1_HIBERNATION_RESTORE_FAILED);
 Finish:
...
static ssize_t disk_write_skip_show(struct kobject *kobj, struct kobj_attribute *attr,
			   char *buf)
{
	return sprintf(buf, "%d\n", disk_write_skip);
}

static ssize_t disk_write_skip_store(struct kobject *kobj, struct kobj_attribute *attr,
			    const char *buf, size_t n)
{
	int error = 0;
	bool temp;

	error = kstrtobool(buf, &temp);
	if (error < 0)
		return error;

	disk_write_skip = temp;
	return n;
}

power_attr(disk_write_skip);
