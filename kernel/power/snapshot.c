...
unsigned int stats_preallocation_us;
unsigned int stats_preallocation_pages;
...
int hibernate_preallocate_memory(void)
{
	...
	unsigned long alloc = 0, save_highmem, pages_highmem = 0, avail_normal;
	ktime_t start, stop;
	int error;
 ...
 out:
	...
	stats_preallocation_us = ktime_to_us(ktime_sub(stop, start));
	stats_preallocation_pages = pages;

	return 0;

 err_out:
	hibernate_stats.failed_preallocate++;
	hibernate_stats.failed_preallocate_nr_pages = alloc - pages_highmem;
	hib_save_failed_step(HIBERNATE_PREALLOCATE);
	swsusp_free();
	return -ENOMEM;
}
...
EXPORT_SYMBOL(snapshot_get_image_size);
