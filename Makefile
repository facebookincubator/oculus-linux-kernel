...
ifdef CONFIG_DEBUG_INFO_BTF
  ifeq ($(wildcard $(RESOLVE_BTFIDS)),)
    # only try to build from kernel when RESOLVE_BTFIDS doesn't exist
    ifeq ($(has_libelf),1)
...
ifeq ($(KBUILD_EXTMOD),)
	...
	$(Q)$(MAKE) $(hdr-inst)=$(hdr-prefix)drivers/staging/oculus/include/uapi dst=usr/oculus/include
	$(Q)for d in $(techpack-dirs); do \
		$(MAKE) $(hdr-inst)=$$d/include/uapi dst=usr/$$d/include; \
	done
