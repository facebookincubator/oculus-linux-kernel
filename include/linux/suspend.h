...
enum hibernate_stat_step {
	HIBERNATE_NOTIFIER = 1,
	HIBERNATE_FREEZE,
	HIBERNATE_PREALLOCATE,
	HIBERNATE_FREEZE_KERNEL,
	HIBERNATE_PREPARE,
	HIBERNATE_SUSPEND,
	HIBERNATE_SWAP,
};

struct hibernate_stats {
	int	success;
	int	fail;
	int	failed_notifier;
	int	failed_swap;
	...
	int	failed_freeze;
	int	failed_freeze_kernel;
	char	failed_freeze_task[TASK_COMM_LEN];
	char	failed_notifier_func[KSYM_SYMBOL_LEN];

	int	failed_preallocate;
	unsigned long failed_preallocate_nr_pages;
	int	failed_suspend;
	int	failed_prepare;
	...
	int	last_failed_step;
	enum hibernate_stat_step failed_steps[REC_FAILED_NUM];
};

extern struct hibernate_stats hibernate_stats;

static inline void hib_save_failed_dev(const char *name)
{
	strlcpy(hibernate_stats.failed_devs[hibernate_stats.last_failed_dev],
		name,
		sizeof(hibernate_stats.failed_devs[0]));
	hibernate_stats.last_failed_dev++;
	hibernate_stats.last_failed_dev %= REC_FAILED_NUM;
}

static inline void hib_save_failed_errno(int err)
{
	hibernate_stats.errno[hibernate_stats.last_failed_errno] = err;
	hibernate_stats.last_failed_errno++;
	hibernate_stats.last_failed_errno %= REC_FAILED_NUM;
}

static inline void hib_save_failed_step(enum hibernate_stat_step step)
{
	hibernate_stats.failed_steps[hibernate_stats.last_failed_step] = step;
	hibernate_stats.last_failed_step++;
	hibernate_stats.last_failed_step %= REC_FAILED_NUM;
}
...
extern bool hibernate_ongoing(void);
...
static inline bool hibernate_ongoing(void) { return false; }
...
#define PM_FREEZE_PREPARE	0x0007 /* Going to freeze processes */
#define PM_POST_THAW		0x0008 /* Process thaw finished */
