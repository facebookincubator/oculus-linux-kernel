...
TRACE_EVENT(mark_victim,
	...
	TP_fast_assign(
		...
		if (exe_file) {
			strncpy(__entry->exec_name,
				file_dentry(exe_file)->d_name.name, 63);
			__entry->exec_name[63] = '\0';
			fput(exe_file);
		} else {
			__entry->exec_name[0] = '\0';
		}
	),

	TP_printk("pid=%d comm=%s total-vm=%lukB anon-rss=%lukB file-rss:%lukB shmem-rss:%lukB uid=%u pgtables=%lukB oom_score_adj=%hd exec_name=%s",
		__entry->pid,
		__get_str(comm),
		__entry->total_vm,
		__entry->anon_rss,
